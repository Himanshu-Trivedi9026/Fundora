    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-geist text-base font-semibold">
            Business Verification
          </h3>
        </div>
        <span
          className={`px-2 py-0.5 rounded-full text-xs font-bold bg-${config.color}/10 text-${config.color}`}
        >
          {config.label}
        </span>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm font-inter">
          ...
          <div>
            ...
            <p className="text-on-surface font-medium capitalize">
              {verification.business_type?.replace(/_/g, " ") || "—"}
            </p>
          </div>
          {verification.gst_status && (
            <div>
              ...
              <p
                className={`font-medium text-xs ${verification.gst_status === "verified" ? "text-success" : "text-on-surface-variant"}`}
              >
                {verification.gst_status}
              </p>
            </div>
          ...
          {verification.pan_status && (
            <div>
              ...
              <p
                className={`font-medium text-xs ${verification.pan_status === "verified" ? "text-success" : "text-on-surface-variant"}`}
              >
                {verification.pan_status}
              </p>
            </div>

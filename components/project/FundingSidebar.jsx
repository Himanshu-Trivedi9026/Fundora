 ...
 */
export default function FundingSidebar({ project, isOwner, saved, onSave, onEdit, onDelete, creatorVerification }) {
  ...
  const backersCount = project?.backers_count || Math.floor(pledged / 1500) || 0;
    ...
    >
      ...
      <div>
        ...
        <div className="text-on-surface-variant text-xs mt-1">
          pledged of <span className="text-on-surface font-semibold">₹{goal.toLocaleString("en-IN")}</span> goal
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider">
          ...
          <span className="text-on-surface-variant">
            {daysLeft !== null
              ? isEnding ? `🔥 ${daysLeft} days left` : `${daysLeft} days left`
              : "—"}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 bg-surface-container-high/40 rounded-xl border border-white/5">
          <div className="text-lg font-bold">{backersCount.toLocaleString()}</div>
          <div className="text-[8px] uppercase font-bold text-on-surface-variant tracking-widest mt-0.5">Backers</div>
        </div>
        <div className="p-3 bg-surface-container-high/40 rounded-xl border border-white/5">
          <div className="text-lg font-bold">{formatViews(viewsCount)}</div>
          <div className="text-[8px] uppercase font-bold text-on-surface-variant tracking-widest mt-0.5">Views</div>
        </div>
      </div>
        ...
        <div className="p-3 bg-surface-container-high/30 rounded-xl border border-white/5">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <div className="flex items-center gap-1.5">
                <span className={`material-symbols-outlined text-[10px] ${
                  creatorVerification.identity_verified ? "text-success" : "text-on-surface-variant/40"
                }`} style={creatorVerification.identity_verified ? { fontVariationSettings: "'FILL' 1" } : {}} aria-hidden="true">
                  {creatorVerification.identity_verified ? "check_circle" : "pending"}
                </span>
                <span className="text-[10px] text-on-surface-variant font-inter">Identity</span>
              </div>
            ...
            {creatorVerification.verification_level >= 2 && (
              <span className="material-symbols-outlined text-success text-[16px]" style={{ fontVariationSettings: "'FILL' 1" }} aria-label="Verified creator" role="img">
                verified
              </span>
            )}
          </div>
        ...
        <>
          ...
          <div className="flex items-center justify-center gap-6 pt-1">
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest">{saved ? "Watching" : "Watchlist"}</span>
            </button>
          </div>
        </>
      )}
      ...
      <p className="text-[10px] text-center text-on-surface-variant leading-tight font-inter">
        By funding, you agree to Fundora&apos;s Investor Terms. All payments secured via Razorpay Enterprise.
      </p>
    </motion.div>
  );
}

  ...
  const phases = [
    {
      ...
      status: "Completed",
      dotClass:
        "bg-primary ring-6 ring-primary/20 shadow-[0_0_12px_rgba(208,188,255,0.4)]",
    ...
    },
    {
      ...
      description:
        "Post-funding development, prototype refinement, and delivery to backers. Scaling operations and market expansion.",
      status: "Upcoming",
      ...
      <div className="relative ml-3 pl-10 space-y-8 before:absolute before:left-0 before:top-2 before:bottom-2 before:w-px before:bg-outline-variant/30">
        {phases.map((phase, i) => (
          <div key={i} className={`relative ${phase.opacity}`}>
            <div
              className={`absolute -left-[46px] top-1 w-2.5 h-2.5 rounded-full ${phase.dotClass}`}
            />
            <div className="flex items-center gap-3 mb-1.5">
              <span
                className={`font-bold uppercase text-[9px] tracking-widest px-2 py-0.5 border rounded ${phase.badgeClass}`}
              >
                {phase.status}
              </span>
            ...
            <p className="text-on-surface-variant text-sm max-w-xl font-inter">
              {phase.description}
            </p>
          </div>
        ))}

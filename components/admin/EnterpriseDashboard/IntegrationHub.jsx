...
const AVAILABLE_PROVIDERS = [
  { id: "slack", name: "Slack", icon: "💬", description: "Messaging & collaboration" },
  { id: "teams", name: "Microsoft Teams", icon: "📋", description: "Team chat & meetings" },
  { id: "discord", name: "Discord", icon: "🎮", description: "Community communication" },
  { id: "google_workspace", name: "Google Workspace", icon: "📧", description: "Docs, Drive, Gmail" },
  { id: "github", name: "GitHub", icon: "🐙", description: "Code & project management" },
  { id: "jira", name: "Jira", icon: "📊", description: "Issue & project tracking" },
  ...
  async function handleAdd(provider) {
    ...
    try {
      ...
      if (json.success) {
        ...
        setFormData({ provider: "", label: "", webhookUrl: "", config: "", credentials: "" });
      }
    } catch (err) {
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Add and manage external integrations</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {AVAILABLE_PROVIDERS.map((provider) => (
          <div
            ...
            onClick={() => { setFormData({ ...formData, provider: provider.id }); setShowAddModal(true); }}
          >
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-xl p-6 w-full max-w-md border border-gray-800">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-gray-400 text-sm block mb-1">Config (JSON, optional)</label>
                <textarea
              ...
              <div>
                <label className="text-gray-400 text-sm block mb-1">Credentials (JSON)</label>
                <textarea
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => { setShowAddModal(false); setFormData({ provider: "", label: "", webhookUrl: "", config: "", credentials: "" }); }}
                className="flex-1 px-4 py-2 bg-gray-800 text-gray-300 rounded-lg hover:bg-gray-700"
              >
                Cancel
              </button>
              <button
                onClick={() => handleAdd(AVAILABLE_PROVIDERS.find(p => p.id === formData.provider))}

  ...
  const stats = [
    {
      ...
      value: analytics?.metrics ? Object.keys(analytics.metrics).length : 0,
      color: "text-blue-400",
    },
    {
      label: "Total Alerts",
    ...
    {
      label: "Total Backups",
      value: analytics?.backups?.total || 0,
      color: "text-green-400",
    },
    {
      label: "Backup Size",
      value: analytics?.backups?.totalSizeBytes
        ? `${(analytics.backups.totalSizeBytes / 1048576).toFixed(1)} MB`
        : "0 MB",
    ...
    <div className="space-y-6">
      ...
      {analytics?.alerts?.byStatus && (
        <div className="bg-gray-900/40 border border-gray-800 rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(analytics.alerts.byStatus).map(
              ([status, count]) => (
                <div
                  key={status}
      ...
      {analytics?.backups?.byStatus && (
        <div className="bg-gray-900/40 border border-gray-800 rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(analytics.backups.byStatus).map(
              ([status, count]) => (
                <div
                  key={status}
                  className="text-center p-3 bg-gray-800/50 rounded-lg"

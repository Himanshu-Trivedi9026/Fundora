...
export default function ComplianceDashboard() {
  ...
  async function handleCreateCase(e) {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) {
        setCreateForm({ type: "", description: "", priority: "medium", relatedEntityType: "", relatedEntityId: "" });
        fetchDashboard();
      } else {
    ...
    <div className="space-y-6">
      ...
      {activeTab === "overview" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              { label: "Total Cases", value: summary.totalCases || 0, color: "text-white" },
              { label: "Open", value: summary.open || 0, color: "text-blue-400" },
              { label: "Investigating", value: summary.investigating || 0, color: "text-yellow-400" },
              { label: "Resolved", value: summary.resolved || 0, color: "text-green-400" },
              { label: "Escalation Queue", value: summary.escalationQueue || 0, color: "text-orange-400" },
            ].map((stat) => (
          ...
          {summary.byPriority && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <h3 className="text-white font-semibold text-sm mb-3">By Priority</h3>
              <div className="flex flex-wrap gap-3">
                {Object.entries(summary.byPriority).map(([priority, count]) => (
                  <span key={priority} className={`px-3 py-1 rounded-full text-xs ${priorityColors[priority] || "bg-gray-800 text-gray-300"}`}>
                    {priority}: {count}
                  </span>
                ))}
              </div>
          ...
          {data?.recentCases?.length > 0 && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl">
              <div className="px-5 py-3 border-b border-gray-800">
                <h3 className="text-white font-semibold text-sm">Recent Cases</h3>
              </div>
              <div className="divide-y divide-gray-800/50">
                {data.recentCases.map((c) => (
                  ...
                  >
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs ${priorityColors[c.priority] || "bg-gray-800 text-gray-300"}`}>
                      {c.priority}
                    </span>
                  </button>
                ))}
          ...
          {selectedCase ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-3">
                      <h3 className="text-white font-bold text-lg">{selectedCase.type}</h3>
                      <span className={`px-3 py-1 rounded-full text-xs ${statusColors[selectedCase.status] || "bg-gray-800 text-gray-300"}`}>
                        {selectedCase.status}
                      </span>
                      <span className={`px-3 py-1 rounded-full text-xs ${priorityColors[selectedCase.priority] || "bg-gray-800 text-gray-300"}`}>
                ...
                <p className="text-gray-300 text-sm mb-4">{selectedCase.description || "No description provided."}</p>

                {selectedCase.related_entity_type && (
                  <p className="text-gray-500 text-xs">
                    Related: {selectedCase.related_entity_type} → {selectedCase.related_entity_id?.substring(0, 8)}...
                  </p>
                )}
                ...
                <div className="flex gap-2 mt-4 flex-wrap">
                  {selectedCase.status !== "resolved" && (
                    <>
                      <button
                        onClick={() => handleCaseAction(selectedCase.id, "assign", { assignee: "current_admin" })}
                      ...
                      <button
                        onClick={() => handleCaseAction(selectedCase.id, "resolve")}
                        disabled={actionLoading}
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl">
                <div className="px-5 py-3 border-b border-gray-800">
                  <h3 className="text-white font-semibold text-sm">Events Timeline</h3>
                </div>
                  ...
                  <div className="px-5 py-3 space-y-3">
                    {selectedCase.events.map((ev, i) => (
                      <div key={i} className="flex gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-blue-400 text-xs font-medium">{ev.event_type || ev.action}</span>
                            <span className="text-gray-600 text-xs">{formatDate(ev.created_at || ev.timestamp)}</span>
                          </div>
                          <p className="text-gray-400 text-xs mt-0.5">{ev.description || ev.details || "—"}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="p-8 text-center text-gray-500 text-sm">No events recorded</div>
                )}
              </div>
            </div>
          ) : (
            ...
            <>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl">
                <div className="px-5 py-3 border-b border-gray-800">
                  <h3 className="text-white font-semibold text-sm">Cases ({filteredCases.length})</h3>
                </div>
                {filteredCases.length > 0 ? (
                  ...
                  <div className="p-8 text-center text-gray-500 text-sm">No cases match filters</div>
                )}
              </div>
            </>
          )}
      ...
      {activeTab === "create" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <form onSubmit={handleCreateCase} className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
            <h3 className="text-white font-semibold text-sm">Create New Compliance Case</h3>
            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-gray-400 text-xs mb-1">Related Entity Type</label>
                <input
                  type="text"
                  value={createForm.relatedEntityType}
                  onChange={(e) => setCreateForm({ ...createForm, relatedEntityType: e.target.value })}
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-500"
                  placeholder="e.g. user, campaign"
                />
              </div>
              <div>
                <label className="block text-gray-400 text-xs mb-1">Related Entity ID</label>
                <input
                  type="text"
                  value={createForm.relatedEntityId}
                  onChange={(e) => setCreateForm({ ...createForm, relatedEntityId: e.target.value })}
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-500"
                  placeholder="UUID"
                />
              </div>

...
export default function PortfolioHealthCard({ health, className = "" }) {
  const safe = health && health.score != null
    ? health
    : { score: 0, breakdown: { diversification: 0, fundingProgress: 0, categorySpread: 0, consistency: 0 } };
    ...
    <GlassCard className={className}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-on-surface font-geist">Portfolio Health</h3>
        </div>
        <div className="text-right">
          <p className="text-3xl font-bold text-on-surface font-geist leading-none">
            {score}
            <span className="text-base text-on-surface-variant font-inter">/100</span>
          </p>
        ...
        {sections.map((s) => (
          <div key={s.label}>
            <div className="flex items-center justify-between text-xs mb-1">
              <span className="text-on-surface-variant font-inter">{s.label}</span>
              <span className="text-on-surface font-inter font-medium">{s.value}</span>
            </div>
            <div

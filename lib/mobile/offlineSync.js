...
async function assertOwnership(table, rowId, userId) {
  ...
  const owned = config.ownership.some(
    (col) => String(row[col]) === String(userId),
  );
      ...
      case SYNC_OPERATIONS.CREATE:
        if (!config.operations.create) {
          return {
            success: false,
            operation: op,
            error: `Create is not allowed for '${table}'`,
          };
        }
      ...
      case SYNC_OPERATIONS.UPDATE:
        if (!config.operations.update) {
          return {
            success: false,
            operation: op,
            error: `Update is not allowed for '${table}'`,
          };
        }
      ...
      case SYNC_OPERATIONS.DELETE:
        if (!config.operations.delete) {
          return {
            success: false,
            operation: op,
            error: `Delete is not allowed for '${table}'`,
          };
        }
...
async function handleCreate(table, data, userId) {
  ...
  if (sanitized.error)
    return { success: false, operation: "create", error: sanitized.error };
  ...
  if (error)
    return { success: false, operation: "create", error: error.message };
...
async function handleUpdate(table, data, conflictStrategy, userId) {
  const id = data.id;
  if (!id)
    return { success: false, operation: "update", error: "No ID provided" };

  const sanitized = sanitizeWriteColumns(table, data);
  if (sanitized.error)
    return { success: false, operation: "update", error: sanitized.error };

  // Ownership must be verified BEFORE the service role is used.
  const ownership = await assertOwnership(table, id, userId);
  if (ownership.error)
    return { success: false, operation: "update", error: ownership.error };
...
async function handleDelete(table, data, userId) {
  const id = data.id;
  if (!id)
    return { success: false, operation: "delete", error: "No ID provided" };

  // Ownership must be verified BEFORE the service role is used.
  const ownership = await assertOwnership(table, id, userId);
  if (ownership.error)
    return { success: false, operation: "delete", error: ownership.error };
...
export async function getChangesSince(userId, timestamp, options = {}) {
  try {
    ...
    if (unknown.length > 0) {
      return {
        success: false,
        error: `Unknown table(s): ${unknown.join(", ")}`,
      };
    }
    ...
    for (const table of tables) {
      ...
      if (config.ownership.length === 1) {
        ...
        const orString = config.ownership
          .map((c) => `${c}.eq.${userId}`)
          .join(",");
    ...
    const totalChanges = Object.values(results).reduce(

 ...
 */
export async function processRefund({ escrowAccountId, amount, reason, refundedBy, donationId = null }) {
  try {
    // Input validation
    ...
    const newRefundedBalance = Math.round((account.refunded_balance + amount) * 100) / 100;
    const newStatus = newLockedBalance === 0 ? "refunded" : "partially_released";
    ...
    if (updateError) {
      logError("RefundEngine", "Update error", { error: updateError.message, escrowAccountId });
    ...
    if (!ledgerResult.success) {
      logError("RefundEngine", "Ledger entry failed", { error: ledgerResult.error, escrowAccountId });
    ...
    logError("RefundEngine", "Refund error", { error: err.message, escrowAccountId });
 ...
 * @returns {Promise<{success: boolean, refund?: Object, error?: string}>}
 */
export async function partialRefund({ escrowAccountId, amount, reason, refundedBy, donationId = null }) {
  try {
    if (!escrowAccountId || !amount || !reason || !refundedBy) {
      return {
        success: false,
    ...
    if (amount >= lockedBalance && lockedBalance > 0) {
      return {
        success: false,
        error: "Amount equals or exceeds locked balance. Use fullRefund() for full refund.",
      };
    }

    // Delegate to processRefund
    return processRefund({ escrowAccountId, amount, reason, refundedBy, donationId });
  } catch (err) {
...
export async function fullRefund({ escrowAccountId, reason, refundedBy }) {
  try {
    if (!escrowAccountId || !reason || !refundedBy) {
      return { success: false, error: "escrowAccountId, reason, and refundedBy are required" };
    }

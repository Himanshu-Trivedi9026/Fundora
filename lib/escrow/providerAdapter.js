...
export class BasePayoutProvider {
  constructor(name) {
    ...
    if (new.target === BasePayoutProvider) {
      throw new Error("BasePayoutProvider is abstract and cannot be instantiated directly.");
    }
  }
...
export class MockPayoutProvider extends BasePayoutProvider {
  ...
  async createPayout(params) {
    ...
    const mockReference = reference || `mock_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;

    logInfo("PayoutProvider", "Mock payout created", {
      amount,
      recipientId: recipientId ? recipientId.substring(0, 8) + "..." : "unknown",

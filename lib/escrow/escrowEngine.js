...
export async function initializeEscrow({ campaignId, creatorId }) {
  try {
    ...
    if (insertError) {
      logError("EscrowEngine", "Create escrow error", { error: insertError.message });
    ...
    logInfo("EscrowEngine", "Escrow initialized", { escrowAccountId: escrow.id, campaignId });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function recordDonation({ escrowAccountId, campaignId, amount, donationId, idempotencyKey }) {
  try {
    if (!escrowAccountId || !campaignId || !amount || !donationId) {
      return { success: false, error: "escrowAccountId, campaignId, amount, and donationId are required" };
    }
    ...
    if (idempotencyKey) {
      ...
      if (existingEntry) {
        logInfo("EscrowEngine", "Duplicate donation detected (idempotent skip)", {
          idempotencyKey,
        });
        return { success: true, data: { alreadyRecorded: true, ledgerEntryId: existingEntry.id } };
      }
    }
    ...
    if (escrow.status !== ESCROW_STATUSES.ACTIVE) {
      return { success: false, error: `Escrow account is not active (status: ${escrow.status})` };
    }
    ...
    if (updateError) {
      logError("EscrowEngine", "Update escrow balance error", { error: updateError.message });
      return { success: false, error: "Failed to update escrow balance. Please retry." };
    }
    ...
    if (ledgerError) {
      logError("EscrowEngine", "Create ledger entry error", { error: ledgerError.message });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function releaseMilestoneFunds({ milestoneId, escrowAccountId, amount, releasedBy }) {
  try {
    if (!milestoneId || !escrowAccountId || !amount || !releasedBy) {
      return { success: false, error: "milestoneId, escrowAccountId, amount, and releasedBy are required" };
    }
    ...
    const { data: escrow, error: fetchError } = await supabaseAdmin
      .from("escrow_accounts")
      .select("id, status, available_balance, locked_balance, released_balance, total_released, creator_id")
      .eq("id", escrowAccountId)
      .single();
    ...
    if (updateError) {
      logError("EscrowEngine", "Update escrow balance for release error", { error: updateError.message });
      return { success: false, error: "Failed to update escrow balance. Please retry." };
    }
    ...
    if (ledgerError) {
      logError("EscrowEngine", "Create release ledger entry error", { error: ledgerError.message });
    }
    ...
    logError("EscrowEngine", "Release milestone funds error", { error: err.message });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function requestPayout({ creatorId, escrowAccountId, bankAccountId, amount }) {
  try {
    if (!creatorId || !escrowAccountId || !bankAccountId || !amount) {
      return { success: false, error: "creatorId, escrowAccountId, bankAccountId, and amount are required" };
    }
    ...
    const result = await delegatePayout({ creatorId, escrowAccountId, bankAccountId, amount });

    if (!result.success) {
      logError("EscrowEngine", "Payout delegation failed", { error: result.error });
      return result;
    }
...
export async function getEscrowSummary(campaignId) {
  try {
    ...
    const { data: milestones } = await supabaseAdmin
      .from("campaign_milestones")
      .select("id, title, status, target_amount, release_amount, approval_percentage")
      .eq("campaign_id", campaignId)
    ...
    logError("EscrowEngine", "Get escrow summary error", { error: err.message });
...
export async function getCreatorEarnings(creatorId) {
  try {
    ...
    if (escrowError) {
      logError("EscrowEngine", "Fetch creator escrows error", { error: escrowError.message });
    ...
    logError("EscrowEngine", "Get creator earnings error", { error: err.message });
    return { success: false, error: "Failed to fetch creator earnings" };
  }
}

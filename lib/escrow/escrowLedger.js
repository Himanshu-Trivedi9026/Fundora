  ...
  try {
    if (
      ...
      return {
        success: false,
        error:
          "escrowAccountId, campaignId, entryType, amount, and balanceAfter are required",
      };
    }
    ...
    if (idempotencyKey) {
      ...
      if (existing) {
        ...
        return {
          success: false,
          error: "Duplicate entry: idempotency key already exists",
        };
      }
    }
    ...
    if (error) {
      logError("EscrowLedger", "Insert error", {
        error: error.message,
        escrowAccountId,
      });
...
export async function getLedgerEntries({
  ...
  try {
    ...
    if (error) {
      logError("EscrowLedger", "Query error", {
        error: error.message,
        escrowAccountId,
      });
...
export async function getLedgerBalance(escrowAccountId) {
  try {
    ...
    if (error) {
      logError("EscrowLedger", "Balance calculation error", {
        error: error.message,
        escrowAccountId,
      });
...
export async function getLedgerSummary(escrowAccountId) {
  try {
    ...
    if (error) {
      logError("EscrowLedger", "Summary query error", {
        error: error.message,
        escrowAccountId,
      });
...
export async function validateLedgerIntegrity(escrowAccountId) {
  try {
    ...
    const expectedBalance =
      ...
      (account.released_balance || 0) -
      (account.refunded_balance || 0);

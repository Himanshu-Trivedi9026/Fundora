...
export async function createEscrowAccount({
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Fee percentage must be a number between 0 and 100",
      };
    }
    ...
    if (error) {
      logError("EscrowAccount", "Create error", {
        error: error.message,
        campaignId,
      });
...
export async function getEscrowAccount(escrowAccountId) {
  try {
    ...
    if (error) {
      ...
      logError("EscrowAccount", "Fetch error", {
        error: error.message,
        escrowAccountId,
      });
...
export async function getEscrowAccountByCampaign(campaignId) {
  try {
    ...
    if (error) {
      ...
      logError("EscrowAccount", "Fetch by campaign error", {
        error: error.message,
        campaignId,
      });
      return { success: false, error: "Failed to fetch escrow account" };
 ...
 */
export async function getEscrowAccountsByCreator(
  creatorId,
  ...
  try {
    ...
    if (error) {
      logError("EscrowAccount", "List by creator error", {
        error: error.message,
        creatorId,
      });
...
export async function updateEscrowStatus(
  ...
  try {
    if (!escrowAccountId || !status) {
      return {
        success: false,
        error: "Escrow account ID and status are required",
      };
    }
    ...
    if (error) {
      logError("EscrowAccount", "Update status error", {
        error: error.message,
        escrowAccountId,
      });
 ...
 * @returns {Promise<{success: boolean, account?: Object, error?: string}>}
 */
export async function freezeEscrowAccount(
  escrowAccountId,
  reason,
  performedBy,
) {
  try {
    if (!escrowAccountId || !reason || !performedBy) {
      return {
        success: false,
        error: "Escrow account ID, reason, and performedBy are required",
      };
    }
...
export async function unfreezeEscrowAccount(escrowAccountId, performedBy) {
  try {
    if (!escrowAccountId || !performedBy) {
      return {
        success: false,
        error: "Escrow account ID and performedBy are required",
      };
    }
...
export async function closeEscrowAccount(escrowAccountId) {
  try {
    ...
    if (error) {
      logError("EscrowAccount", "Close error", {
        error: error.message,
        escrowAccountId,
      });

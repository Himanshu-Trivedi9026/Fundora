...
export async function recordEscrowEvent({
  ...
  try {
    ...
    if (error) {
      logError("EscrowEvents", "Record error", { error: error.message, escrowAccountId });
...
export async function getEscrowEvents({
  ...
  try {
    if (!escrowAccountId && !campaignId) {
      return { success: false, error: "At least one of escrowAccountId or campaignId is required" };
    }
...
export async function getEscrowEventSummary(campaignId, days = 30) {
  try {
    ...
    if (error) {
      logError("EscrowEvents", "Summary query error", { error: error.message, campaignId });

...
export function canRelease(escrowAccount, amount) {
  try {
    ...
    if (TERMINAL_STATUSES.includes(escrowAccount.status)) {
      return { allowed: false, reason: `Cannot release from '${escrowAccount.status}' status` };
    }
...
export function canRefund(escrowAccount, amount) {
  try {
    ...
    if (escrowAccount.status === "fully_released") {
      return { allowed: false, reason: "Cannot refund a fully released account" };
    }
...
export function canPayout(escrowAccount, amount) {
  try {
    ...
    // Check if in terminal state
    if (TERMINAL_STATUSES.includes(escrowAccount.status)) {
      return { allowed: false, reason: `Cannot payout from '${escrowAccount.status}' status` };
    }
...
export function calculatePlatformFee(amount, feePercentage) {
  try {
    ...
    if (typeof feePercentage !== "number" || isNaN(feePercentage) || feePercentage < 0 || feePercentage > 100) {
...
export function getEscrowStatus(escrowAccount) {
  try {
    ...
    if (["frozen", "closed", "cancelled", "refunded"].includes(escrowAccount.status)) {

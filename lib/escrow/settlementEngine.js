 ...
 * @type {string[]}
 */
const BATCH_STATUSES = ["pending", "processing", "completed", "failed", "partial"];
...
export async function createSettlementBatch({ initiatedBy }) {
  try {
    ...
    if (error) {
      logError("SettlementEngine", "Create batch error", { error: error.message });
...
export async function addToSettlementBatch(batchId, payoutRequestId) {
  try {
    if (!batchId || !payoutRequestId) {
      return { success: false, error: "batchId and payoutRequestId are required" };
    }
    ...
    if (batch.status !== "pending") {
      return { success: false, error: `Batch is in '${batch.status}' status. Only pending batches accept new items.` };
    }
    ...
    if (payoutRequest.status !== "approved") {
      return { success: false, error: `Payout request is in '${payoutRequest.status}' status. Must be approved.` };
    }
    ...
    if (existingItem) {
      return { success: false, error: "Payout request is already in a settlement batch" };
    }
    ...
    if (insertError) {
      logError("SettlementEngine", "Add to batch error", { error: insertError.message, batchId });
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        ...
        total_amount: Math.round((batch.total_amount + (payoutRequest.amount || 0)) * 100) / 100,
    ...
    if (updateError) {
      logError("SettlementEngine", "Batch update error", { error: updateError.message, batchId });
...
export async function processSettlementBatch(batchId) {
  try {
    ...
    if (batch.status !== "pending") {
      return { success: false, error: `Batch is in '${batch.status}' status. Only pending batches can be processed.` };
    }
    ...
    if (statusError) {
      logError("SettlementEngine", "Batch status update error", { error: statusError.message, batchId });
    ...
    if (itemsError) {
      logError("SettlementEngine", "Fetch items error", { error: itemsError.message, batchId });
      await supabaseAdmin
        .from("settlement_batches")
    ...
    for (const item of payoutItems) {
      try {
        ...
        if (!payoutRequest) {
          ...
          await supabaseAdmin
            .from("settlement_batch_items")
            .update({ status: "failed", error_message: "Payout request not found" })
            .eq("id", item.id);
          failedCount++;
          continue;
        }
    ...
    if (finalError) {
      logError("SettlementEngine", "Batch final update error", { error: finalError.message, batchId });
    }
    ...
    logError("SettlementEngine", "Batch processing error", { error: err.message, batchId });
 ...
 */
export async function getSettlementBatches({ status, limit = 50, offset = 0 } = {}) {
  try {
    ...
    if (error) {
      logError("SettlementEngine", "List batches error", { error: error.message });

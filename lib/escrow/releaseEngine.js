...
export async function releaseFunds({
  ...
  try {
    ...
    const newReleasedBalance =
      Math.round((account.released_balance + amount) * 100) / 100;
    const newCreatorEarnings =
      Math.round(
        (account.creator_earnings +
          amount * (1 - (account.fee_percentage || 0) / 100)) *
          100,
      ) / 100;
    const newStatus =
      newLockedBalance === 0 ? "fully_released" : "partially_released";
    ...
    if (updateError) {
      logError("ReleaseEngine", "Update error", {
    ...
    if (!ledgerResult.success) {
      logError("ReleaseEngine", "Ledger entry failed", {
    ...
    logError("ReleaseEngine", "Release error", {
      error: err.message,
      escrowAccountId,
    });
...
export async function emergencyFreeze(escrowAccountId, reason, performedBy) {
  try {
    if (!escrowAccountId || !reason || !performedBy) {
      return {
        success: false,
        error: "escrowAccountId, reason, and performedBy are required",
      };
    }
...
export async function emergencyCancel(escrowAccountId, reason, performedBy) {
  try {
    ...
    if (["cancelled", "closed"].includes(account.status)) {
      return {
        success: false,
        error: `Cannot cancel an account in '${account.status}' status`,
      };
    }
    ...
    const { data: updatedAccount, error: updateError } = await supabaseAdmin
      ...
      .update({
        status: "cancelled",
        refunded_balance:
          Math.round((account.refunded_balance + lockedBalance) * 100) / 100,
        locked_balance: 0,
        updated_at: new Date().toISOString(),
      })
      .eq("id", escrowAccountId)

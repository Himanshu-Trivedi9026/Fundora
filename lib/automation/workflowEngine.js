...
const FORBIDDEN_UPDATE_COLUMNS = new Set([
  // identity / ownership
  "id", "owner_id", "creator_id", "user_id", "organization_id", "org_id",
  "created_by", "created_at", "updated_at", "updated_by",
  // role & permissions
  "role", "permissions", "is_admin", "is_platform_admin", "is_staff",
  // auth & credentials
  "email", "phone", "mobile", "password", "password_hash", "token",
  "api_key", "secret", "otp", "verification_code", "session_id",
  // security & verification
  "is_verified", "verified_at", "risk_score", "fraud_status", "banned",
]);
...
export function validateActionConfig(action) {
  ...
  switch (action.type) {
    case ACTION_TYPES.UPDATE_ENTITY: {
      ...
      if (!entityType) return { valid: false, error: "update_entity requires entityType" };
      if (!ALLOWED_ENTITY_TABLES.includes(entityType)) {
        return { valid: false, error: `Table '${entityType}' is not allowlisted for workflow updates` };
      }
      if (!entityId) return { valid: false, error: "update_entity requires entityId" };
      if (!updates || typeof updates !== "object" || Array.isArray(updates) || Object.keys(updates).length === 0) {
        return { valid: false, error: "update_entity requires a non-empty updates object" };
      }
      const bad = Object.keys(updates).filter((k) => !isColumnAllowed(entityType, k));
      if (bad.length > 0) {
        return { valid: false, error: `Columns not allowlisted for ${entityType}: ${bad.join(", ")}` };
      }
      return { valid: true };
    }
    ...
    case ACTION_TYPES.UPDATE_STATUS: {
      ...
      if (!entityType) return { valid: false, error: "update_status requires entityType" };
      if (!ALLOWED_ENTITY_TABLES.includes(entityType)) {
        return { valid: false, error: `Table '${entityType}' is not allowlisted for workflow updates` };
      }
      if (!entityId) return { valid: false, error: "update_status requires entityId" };
      if (!statusField) return { valid: false, error: "update_status requires statusField" };
      if (!isColumnAllowed(entityType, statusField)) {
        return { valid: false, error: `Status field '${statusField}' is not allowlisted for ${entityType}` };
      }
  ...
  try {
    ...
    for (const { address } of records) {
      if (isPrivateIpAddress(address)) {
        return { safe: false, reason: "Hostname resolves to a private/loopback IP address" };
      }
    }
  } catch {
    return { safe: false, reason: "Unable to resolve hostname" };
...
export async function verifyWorkflowOwnership({ workflowId, userId }) {
  try {
    ...
    logError("WorkflowEngine", "Ownership verification error", { error: err.message, workflowId });
...
function validateWorkflowInput({ name, triggerType }) {
  ...
  if (!triggerType || !VALID_TRIGGER_TYPES.includes(triggerType)) {
    return { valid: false, error: `triggerType must be one of: ${VALID_TRIGGER_TYPES.join(", ")}` };
  }
  return { valid: true };
}
  ...
  switch (type) {
    ...
    case ACTION_TYPES.UPDATE_ENTITY: {
      ...
      if (!entityType || !entityId || !updates) {
        return { status: "error", output: { error: "entityType, entityId, and updates are required" } };
      }
    ...
    case ACTION_TYPES.CALL_API: {
      ...
      if (!url) {
        return { status: "error", output: { error: "URL is required for API call" } };
      }
    ...
    case ACTION_TYPES.RUN_AI: {
      ...
      return {
        status: "success",
        output: { message: "AI action recorded — processing in background", taskType: config.taskType },
      };
    }
    ...
    case ACTION_TYPES.SEND_WEBHOOK: {
      ...
      if (!url) {
        return { status: "error", output: { error: "Webhook URL is required" } };
      }
    ...
    case ACTION_TYPES.UPDATE_STATUS: {
      ...
      if (!entityType || !entityId || !statusField || !statusValue) {
        return { status: "error", output: { error: "entityType, entityId, statusField, and statusValue are required" } };
      }
      ...
      return { status: "success", output: { updated: true, entityId, [statusField]: statusValue } };
    }
    ...
    default:
      return { status: "error", output: { error: `Unknown action type: ${type}` } };
  }
}
  ...
  try {
    ...
    for (const action of actions) {
      if (!action.type || !VALID_ACTION_TYPES.includes(action.type)) {
        return { success: false, error: `Invalid action type: ${action.type}. Must be one of: ${VALID_ACTION_TYPES.join(", ")}` };
      }
      const configResult = validateActionConfig(action);
      if (!configResult.valid) {
        return { success: false, error: `Invalid action config for ${action.type}: ${configResult.error}` };
      }
    }
    ...
    const validateConditions = (conds) => {
      for (const cond of conds) {
        ...
        if (cond.type === CONDITION_TYPES.AND || cond.type === CONDITION_TYPES.OR) {
    ...
    if (error) {
      logError("WorkflowEngine", "Create workflow error", { error: error.message, name });
      return { success: false, error: "Failed to create workflow" };
    }

    logInfo("WorkflowEngine", "Workflow created", { workflowId: data.id, name, triggerType });
    ...
    logError("WorkflowEngine", "Create workflow error", { error: err.message, name });
    return { success: false, error: "Failed to create workflow" };
  }
}
...
export async function updateWorkflow(workflowId, updates, performedBy) {
  try {
    ...
    const allowedFields = [
      "name", "description", "trigger_type", "conditions", "actions",
      "retry_config", "schedule_config", "enabled",
    ];
    ...
    if (error) {
      ...
      logError("WorkflowEngine", "Update workflow error", { error: error.message, workflowId });
      return { success: false, error: "Failed to update workflow" };
    }

    logInfo("WorkflowEngine", "Workflow updated", { workflowId, fields: Object.keys(sanitizedUpdates) });
    ...
    logError("WorkflowEngine", "Update workflow error", { error: err.message, workflowId });
    return { success: false, error: "Failed to update workflow" };
  }
}
...
export async function deleteWorkflow(workflowId, performedBy) {
  try {
    ...
    if (error) {
      logError("WorkflowEngine", "Delete workflow error", { error: error.message, workflowId });
      return { success: false, error: "Failed to delete workflow" };
    }

    logInfo("WorkflowEngine", "Workflow deleted", { workflowId, name: existing.name });
    ...
    logError("WorkflowEngine", "Delete workflow error", { error: err.message, workflowId });
    return { success: false, error: "Failed to delete workflow" };
  }
}
...
export async function enableWorkflow(workflowId, performedBy) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("workflows")
      .update({ enabled: true, updated_at: new Date().toISOString(), updated_by: performedBy })
      .eq("id", workflowId)
      .select()
      .single();
    ...
    if (error) {
      ...
      logError("WorkflowEngine", "Enable workflow error", { error: error.message, workflowId });
    ...
    logError("WorkflowEngine", "Enable workflow error", { error: err.message, workflowId });
    return { success: false, error: "Failed to enable workflow" };
  }
}
...
export async function disableWorkflow(workflowId, performedBy) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("workflows")
      .update({ enabled: false, updated_at: new Date().toISOString(), updated_by: performedBy })
      .eq("id", workflowId)
      .select()
      .single();
    ...
    if (error) {
      ...
      logError("WorkflowEngine", "Disable workflow error", { error: error.message, workflowId });
    ...
    logError("WorkflowEngine", "Disable workflow error", { error: err.message, workflowId });
    return { success: false, error: "Failed to disable workflow" };
  }
}
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function listWorkflows({ organizationId, enabled, triggerType, limit = 20, offset = 0, userId }) {
  try {
    ...
    if (error) {
      logError("WorkflowEngine", "List workflows error", { error: error.message });
...
export async function getWorkflow(workflowId) {
  try {
    ...
    if (error) {
      ...
      logError("WorkflowEngine", "Get workflow error", { error: error.message, workflowId });
      return { success: false, error: "Failed to get workflow" };
    }

    return { success: true, data };
  } catch (err) {
    logError("WorkflowEngine", "Get workflow error", { error: err.message, workflowId });
    return { success: false, error: "Failed to get workflow" };
  }
}
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function triggerWorkflow({ workflowId, triggerEvent, input = {}, triggeredBy }) {
  try {
    if (!workflowId) {
      return { success: false, error: "workflowId is required" };
    }
    ...
    if (runError) {
      logError("WorkflowEngine", "Create workflow run error", { error: runError.message, workflowId });
    ...
    if (workflow.conditions && workflow.conditions.length > 0) {
      const condResult = await evaluateConditions({ conditions: workflow.conditions, context });
      if (!condResult.success) {
        status = "failed";
        output = { error: condResult.error };
      } else if (!condResult.data.matched) {
        status = "skipped";
        output = { message: "Conditions not met", details: condResult.data.details };
      }
    }
    ...
    if (status === "running" || status === "completed") {
      if (workflow.actions && workflow.actions.length > 0) {
        const actionResult = await executeActions({ actions: workflow.actions, context });
        if (!actionResult.success) {
          ...
          const failedSteps = actionResult.data.results.filter((r) => r.status === "error");
    ...
    logError("WorkflowEngine", "Trigger workflow error", { error: err.message, workflowId });
...
export async function evaluateConditions({ conditions, context }) {
  try {
    ...
    logError("WorkflowEngine", "Evaluate conditions error", { error: err.message });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function getWorkflowRuns({ workflowId, limit = 20, offset = 0, status }) {
  try {
    if (!workflowId) {
      return { success: false, error: "workflowId is required" };
    }
    ...
    if (error) {
      logError("WorkflowEngine", "Get workflow runs error", { error: error.message, workflowId });
    ...
    logError("WorkflowEngine", "Get workflow runs error", { error: err.message, workflowId });
    return { success: false, error: "Failed to get workflow runs" };
  }
}
...
export async function getWorkflowRun(runId) {
  try {
    ...
    if (runError) {
      ...
      logError("WorkflowEngine", "Get workflow run error", { error: runError.message, runId });
    ...
    logError("WorkflowEngine", "Get workflow run error", { error: err.message, runId });
    return { success: false, error: "Failed to get workflow run" };
  }
}
...
export async function retryWorkflowRun(runId, performedBy) {
  try {
    ...
    if (originalRun.status !== "failed" && originalRun.status !== "partial_failure") {
      return { success: false, error: `Cannot retry a run with status: ${originalRun.status}. Only failed or partial_failure runs can be retried.` };
    }
    ...
    logInfo("WorkflowEngine", "Workflow run retried", { originalRunId: runId, newRunId: retryResult.data.runId });
    ...
    logError("WorkflowEngine", "Retry workflow run error", { error: err.message, runId });
...
export async function createWorkflowTemplate({
  ...
  try {
    ...
    if (error) {
      logError("WorkflowEngine", "Create template error", { error: error.message, name });
      return { success: false, error: "Failed to create workflow template" };
    }

    logInfo("WorkflowEngine", "Workflow template created", { templateId: data.id, name });
    ...
    logError("WorkflowEngine", "Create template error", { error: err.message, name });
    return { success: false, error: "Failed to create workflow template" };
  }
}
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function instantiateFromTemplate({ templateId, customizations = {}, createdBy }) {
  try {
    if (!templateId) {
    ...
    logError("WorkflowEngine", "Instantiate from template error", { error: err.message, templateId });
    return { success: false, error: "Failed to instantiate workflow from template" };
  }
}
...
export async function processScheduledWorkflows() {
  try {
    ...
    if (fetchError) {
      logError("WorkflowEngine", "Fetch scheduled workflows error", { error: fetchError.message });
    ...
    logInfo("WorkflowEngine", "Scheduled workflows processed", { processed, triggered });
    ...
    logError("WorkflowEngine", "Process scheduled workflows error", { error: err.message });

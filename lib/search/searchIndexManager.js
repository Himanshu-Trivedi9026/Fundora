...
export async function rebuildIndex(entityType, options = {}) {
  try {
    ...
    processIndexRebuild(data.id).catch((err) => {
      secureLogger.error("Index rebuild failed", { indexId: data.id, error: err.message });
    });
...
export async function listIndexes(options = {}) {
  try {
    let query = supabaseAdmin.from("search_indexes").select("*", { count: "exact" });
...
export async function queueIndexUpdate(entityType, entityId, operation = "upsert") {
  try {

...
const SEARCH_ENTITIES = {
  projects: { table: "projects", searchFields: ["title", "description", "tags"], defaultLimit: 20 },
  users: { table: "profiles", searchFields: ["full_name", "username", "bio"], defaultLimit: 20 },
  campaigns: { table: "campaigns", searchFields: ["title", "description"], defaultLimit: 20 },
  plugins: { table: "plugins", searchFields: ["name", "description", "tags"], defaultLimit: 20 },
};
...
export async function search(options = {}) {
  try {
    ...
    if (!entityConfig) return { success: false, error: `Unknown search entity: ${entity}` };
    ...
    if (query.trim()) {
      ...
      const conditions = entityConfig.searchFields.map(
        (field) => terms.map((t) => `${field}.ilike.%${t}%`).join(",")
      );
...
export async function globalSearch(query, options = {}) {
  try {
    ...
    for (const entity of entities) {
      const result = await search({ ...options, query, entity, limit: options.perEntityLimit || 5 });
      if (result.success) {

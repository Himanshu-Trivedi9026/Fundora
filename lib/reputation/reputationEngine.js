...
export const REPUTATION_DIMENSIONS = {
  CREATOR: {
    ...
    community: { weight: 0.10, label: "Community" },
  ...
  DONOR: {
    ...
    feedback_quality: { weight: 0.30, label: "Feedback Quality" },
    campaign_adherence: { weight: 0.20, label: "Campaign Adherence" },
  },
  CAMPAIGN: {
    funding_progress: { weight: 0.20, label: "Funding Progress" },
    milestone_adherence: { weight: 0.20, label: "Milestone Adherence" },
    transparency: { weight: 0.20, label: "Transparency" },
    creator_reputation: { weight: 0.15, label: "Creator Reputation" },
    donor_sentiment: { weight: 0.15, label: "Donor Sentiment" },
    update_frequency: { weight: 0.10, label: "Update Frequency" },
  },
};
...
export async function calculateCreatorReputation(creatorId) {
  try {
    ...
    if (campaignError) {
      logError("ReputationEngine", "Fetch campaigns error", { error: campaignError.message });
    ...
    if (campaignIds.length > 0) {
      const { data: milestoneData, error: milestoneError } = await supabaseAdmin
        ...
        .select("id, campaign_id, status, target_amount, release_amount, approval_percentage")
        .in("campaign_id", campaignIds);

      if (milestoneError) {
        logWarn("ReputationEngine", "Fetch milestones error", { error: milestoneError.message });
      } else {
    ...
    if (campaignIds.length > 0) {
      ...
      if (reviewError) {
        logWarn("ReputationEngine", "Fetch reviews error", { error: reviewError.message });
      } else {
        reviews = reviewData || [];
      }
    }
    ...
    if (campaignIds.length > 0) {
      ...
      if (donationError) {
        logWarn("ReputationEngine", "Fetch donations error", { error: donationError.message });
      } else {
    ...
    const overallScore = calculateWeightedScore(scores, REPUTATION_DIMENSIONS.CREATOR);
    ...
    const totalRaised = allCampaigns.reduce((sum, c) => sum + (c.current_amount || 0), 0);
    const completedMilestones = milestones.filter((m) => m.status === "completed").length;
    const totalMilestones = milestones.length;
    const approvalRate =
      reviews.length > 0
        ? Math.round(
            (reviews.filter((r) => r.decision === "approved").length / reviews.length) * 100,
          )
        : 0;
    const totalDonations = donations.length;
    const averageDonation =
      totalDonations > 0
        ? Math.round(donations.reduce((sum, d) => sum + (d.amount || 0), 0) / totalDonations)
        : 0;
    ...
    logError("ReputationEngine", "Calculate creator reputation error", { error: err.message });
...
export async function getCreatorReputation(creatorId) {
  try {
    ...
    logError("ReputationEngine", "Get creator reputation error", { error: err.message });
...
export async function calculateDonorReputation(donorId) {
  try {
    ...
    if (donationError) {
      logError("ReputationEngine", "Fetch donor donations error", { error: donationError.message });
    ...
    const backedCampaignIds = [...new Set(allDonations.map((d) => d.campaign_id))];
    ...
    // Compute dimension scores
    const scores = {
      engagement: scoreDonorEngagement(allDonations, allReviews, backedCampaigns.length),
      generosity: scoreDonorGenerosity(allDonations),
      feedback_quality: scoreDonorFeedbackQuality(allReviews),
      campaign_adherence: scoreDonorCampaignAdherence(allDonations, backedCampaigns),
    };

    // Calculate weighted overall score
    const overallScore = calculateWeightedScore(scores, REPUTATION_DIMENSIONS.DONOR);
    ...
    const completionRate =
      totalDonations > 0 ? Math.round((completedDonations / totalDonations) * 100) : 0;
    const totalReviews = allReviews.length;
    const approvalReviews = allReviews.filter((r) => r.decision === "approved").length;
    ...
    logError("ReputationEngine", "Calculate donor reputation error", { error: err.message });
...
export async function getDonorReputation(donorId) {
  try {
    ...
    logError("ReputationEngine", "Get donor reputation error", { error: err.message });
...
export async function calculateCampaignReputation(campaignId) {
  try {
    ...
    logInfo("ReputationEngine", "Calculating campaign reputation", { campaignId });

    // Fetch campaign data
    const { data: campaign, error: campaignError } = await supabaseAdmin
      .from("campaigns")
      .select("id, creator_id, status, goal_amount, current_amount, created_at, updated_at")
      .eq("id", campaignId)
      .single();
    ...
    const { data: milestones, error: milestoneError } = await supabaseAdmin
      .from("campaign_milestones")
      .select("id, status, target_amount, release_amount, approval_percentage, created_at")
    ...
    if (allMilestones.length > 0) {
      const { data: reviewData, error: reviewError } = await supabaseAdmin
        ...
        .in("milestone_id", allMilestones.map((m) => m.id));
    ...
    const scores = {
      ...
      update_frequency: scoreCampaignUpdateFrequency(updateCount, campaign.created_at),
    };

    // Calculate weighted overall score
    const overallScore = calculateWeightedScore(scores, REPUTATION_DIMENSIONS.CAMPAIGN);

    // Compute stats
    const totalRaised = campaign.current_amount || 0;
    const goalAmount = campaign.goal_amount || 0;
    const fundingPercentage = goalAmount > 0 ? Math.round((totalRaised / goalAmount) * 100) : 0;
    ...
    const averageDonation =
      totalDonations > 0
        ? Math.round(allDonations.reduce((sum, d) => sum + (d.amount || 0), 0) / totalDonations)
        : 0;
    ...
    logError("ReputationEngine", "Calculate campaign reputation error", { error: err.message });
...
export async function getCampaignReputation(campaignId) {
  try {
    ...
    logError("ReputationEngine", "Get campaign reputation error", { error: err.message });
...
export async function updateReputationPenalty(userId, penaltyCount, reason) {
  try {
    if (!userId || !penaltyCount || !reason) {
      return { success: false, error: "userId, penaltyCount, and reason are required" };
    }
    ...
    if (!table) {
      return { success: false, error: "No reputation record found for this user" };
    }
    ...
    if (error) {
      logError("ReputationEngine", "Update reputation penalty error", { error: error.message });
    ...
    logError("ReputationEngine", "Apply reputation penalty error", { error: err.message });
 ...
 */
export async function getReputationLeaderboard({ type = "creator", limit = 20, offset = 0 } = {}) {
  try {
    if (type !== "creator" && type !== "donor") {
      return { success: false, error: 'type must be "creator" or "donor"' };
    }

    const table = type === "creator" ? "creator_reputation" : "donor_reputation";
    ...
    if (error) {
      logError("ReputationEngine", "Fetch leaderboard error", { error: error.message });
      return { success: false, error: "Failed to fetch reputation leaderboard" };
    }

    return { success: true, data: data || [] };
  } catch (err) {
    logError("ReputationEngine", "Get reputation leaderboard error", { error: err.message });
    return { success: false, error: "Failed to fetch reputation leaderboard" };
  }
}
...
function scoreCreatorQuality(campaigns, milestones, donations) {
  ...
  const completedMilestones = milestones.filter((m) => m.status === "completed").length;
  const milestoneRate = milestones.length > 0 ? completedMilestones / milestones.length : 0;
...
function scoreCreatorReliability(campaigns, milestones) {
  ...
  const activeCampaigns = campaigns.filter(
    (c) => c.status === "active" || c.status === "funded" || c.status === "completed",
  );
...
function scoreCreatorCommunication(reviews, campaigns) {
  ...
  const positiveReviews = reviews.filter((r) => r.decision === "approved").length;
...
function scoreDonorFeedbackQuality(reviews) {
  ...
  const validReviews = reviews.filter((r) => r.decision === "approved" || r.decision === "rejected");
...
function scoreCampaignMilestoneAdherence(milestones) {
  ...
  const active = milestones.filter((m) => m.status === "active" || m.status === "submitted").length;
  const failed = milestones.filter((m) => m.status === "rejected" || m.status === "cancelled").length;
...
function scoreCampaignUpdateFrequency(updateCount, createdAt) {
  ...
  const updateRate = expectedUpdates > 0 ? updateCount / expectedUpdates : updateCount > 0 ? 1 : 0;

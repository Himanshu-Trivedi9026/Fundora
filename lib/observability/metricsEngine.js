...
export async function recordMetric(metricName, value, metricType = "counter", tags = {}, options = {}) {
  try {
    const entry = {
...
export async function flushMetrics() {
  ...
  try {
    ...
    if (error) {
      secureLogger.error("Failed to flush metrics", { error: error.message, count: batch.length });
    }
  } catch (err) {
...
export async function getMetricSummary(metricName, options = {}) {
  try {
    const { data, error } = await supabaseAdmin
      ...
      .eq("metric_name", metricName)
      .gte("recorded_at", options.since || new Date(Date.now() - 86400000).toISOString())
    ...
    if (values.length === 0) {
      return { success: true, data: { count: 0, sum: 0, avg: 0, min: 0, max: 0, last: 0 } };
    }

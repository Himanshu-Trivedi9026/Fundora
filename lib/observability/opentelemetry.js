...
import { logInfo, logWarn, logError, logDebug } from "../verification/secureLogger.js";
...
export function exportActiveTraces() {
  return Array.from(_traceContext.keys()).map((id) => exportTrace(id)).filter(Boolean);
}
...
export async function runErrorHooks(error, context = {}) {
  ...
  for (const hook of _errorHooks) {
    try {
      ...
      logError("Error hook failed", { original: error.message, hookError: err.message });
    }
  }
  return results;
}

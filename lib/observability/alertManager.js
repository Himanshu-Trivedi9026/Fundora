...
export const ALERT_SEVERITIES = { CRITICAL: "critical", WARNING: "warning", INFO: "info", DEBUG: "debug" };
export const ALERT_STATUSES = { ACTIVE: "active", ACKNOWLEDGED: "acknowledged", RESOLVED: "resolved", SILENCED: "silenced" };
export const ALERT_TYPES = { THRESHOLD: "threshold", ANOMALY: "anomaly", HEARTBEAT: "heartbeat", CUSTOM: "custom" };
...
export async function checkThresholdAlert(metricName, value, threshold, options = {}) {
  if (value > threshold) {
    return createAlert({
...
export async function silenceAlert(alertId) {
  try {
    const { error } = await supabaseAdmin
      .from("alerts")
      .update({ status: ALERT_STATUSES.SILENCED, updated_at: new Date().toISOString() })
      .eq("id", alertId);

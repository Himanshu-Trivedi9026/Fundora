 ...
 * @type {string[]}
 */
export const COMPLIANCE_PRIORITIES = ["low", "medium", "high", "urgent"];
...
export async function createComplianceCase({
  ...
  try {
    ...
    if (!COMPLIANCE_CASE_TYPES.includes(caseType)) {
      return {
        success: false,
        error: `Invalid caseType: ${caseType}. Must be one of: ${COMPLIANCE_CASE_TYPES.join(", ")}`,
      };
    }
    ...
    if (!COMPLIANCE_PRIORITIES.includes(priority)) {
      return {
        success: false,
        error: `Invalid priority: ${priority}. Must be one of: ${COMPLIANCE_PRIORITIES.join(", ")}`,
      };
    }
    ...
    if (!subjectUserId && !subjectCampaignId) {
      return {
        success: false,
        error: "At least one of subjectUserId or subjectCampaignId is required",
      };
    }
    ...
    if (error) {
      logError("ComplianceEngine", "Create case error", {
        error: error.message,
        caseType,
      });
...
export async function getComplianceCase(caseId) {
  try {
    ...
    if (error) {
      ...
      logError("ComplianceEngine", "Fetch case error", {
        error: error.message,
        caseId,
      });
...
export async function getComplianceCaseByNumber(caseNumber) {
  try {
    ...
    if (error) {
      ...
      logError("ComplianceEngine", "Fetch case by number error", {
        error: error.message,
        caseNumber,
      });
      return { success: false, error: "Failed to fetch compliance case" };
...
export async function getComplianceCases({
  ...
  try {
    ...
    if (error) {
      logError("ComplianceEngine", "List cases error", {
        error: error.message,
      });
...
export async function updateComplianceCase(
  ...
  try {
    ...
    if (error) {
      logError("ComplianceEngine", "Update case error", {
        error: error.message,
        caseId,
      });
...
export async function assignComplianceCase(caseId, assignTo, assignedBy) {
  try {
    if (!caseId || !assignTo || !assignedBy) {
      return {
        success: false,
        error: "caseId, assignTo, and assignedBy are required",
      };
    }
    ...
    if (current.status === "resolved" || current.status === "closed") {
      return {
        success: false,
        error: `Cannot assign a case in '${current.status}' status`,
      };
    }
    ...
    if (error) {
      logError("ComplianceEngine", "Assign case error", {
        error: error.message,
        caseId,
      });
  ...
  try {
    if (!caseId || !resolutionType || !resolution || !performedBy) {
      return {
        success: false,
        error:
          "caseId, resolutionType, resolution, and performedBy are required",
      };
    }
    ...
    if (current.status === "closed") {
      return {
        success: false,
        error: "Cannot resolve a closed case. Reopen it first.",
      };
    }
    ...
    if (error) {
      logError("ComplianceEngine", "Resolve case error", {
        error: error.message,
        caseId,
      });
...
export async function reopenComplianceCase(caseId, reason, performedBy) {
  try {
    ...
    if (current.status !== "resolved" && current.status !== "closed") {
      return {
        success: false,
        error: `Cannot reopen a case in '${current.status}' status. Must be resolved or closed.`,
      };
    }
    ...
    if (error) {
      logError("ComplianceEngine", "Reopen case error", {
        error: error.message,
        caseId,
      });
...
export async function escalateComplianceCase(caseId, reason, performedBy) {
  try {
    ...
    if (current.status === "resolved" || current.status === "closed") {
      return {
        success: false,
        error: `Cannot escalate a case in '${current.status}' status`,
      };
    }
    ...
    if (error) {
      logError("ComplianceEngine", "Escalate case error", {
        error: error.message,
        caseId,
      });

...
export async function checkPlatformAdmin(userId) {
  try {
    ...
    if (error) {
      logError("RBAC", "checkPlatformAdmin query error", {
        error: error.message,
      });
    ...
    logError("RBAC", "checkPlatformAdmin unexpected error", {
      error: err.message,
    });
...
export async function getUserPermissions(userId, organizationId) {
  try {
    ...
    if (customErr) {
      logWarn("RBAC", "getUserPermissions custom role query error", {
    ...
    logError("RBAC", "getUserPermissions unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }
...
export async function setOrganizationRole(
  ...
  try {
    if (!organizationId || !userId || !role || !performedBy) {
      return {
        success: false,
        error: "organizationId, userId, role, and performedBy are required",
      };
    }
    ...
    if (error) {
      logError("RBAC", "setOrganizationRole update error", {
        error: error.message,
      });
      return { success: false, error: error.message };
    }
    ...
    logError("RBAC", "setOrganizationRole unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }
  ...
  try {
    if (
      ...
      return {
        success: false,
        error:
          "organizationId, name, permissions (array), and performedBy are required",
      };
    }
    ...
    if (error) {
      logError("RBAC", "createCustomRole insert error", {
        error: error.message,
      });
      return { success: false, error: error.message };
    }
    ...
    logInfo("RBAC", "Custom role created", {
      organizationId,
      name,
      performedBy,
    });
    ...
    logError("RBAC", "createCustomRole unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }
...
export async function getOrganizationRoles(organizationId) {
  try {
    ...
    if (error) {
      logError("RBAC", "getOrganizationRoles query error", {
        error: error.message,
      });
      return { success: false, error: error.message };
    }
    ...
    const systemRoles = Object.entries(DEFAULT_ROLE_PERMISSIONS).map(
      ([name, permissions]) => ({
        name,
        permissions,
        is_system: true,
    ...
    logError("RBAC", "getOrganizationRoles unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }
...
export async function initializeOrganizationRoles(organizationId, performedBy) {
  try {
    ...
    if (error) {
      logError("RBAC", "initializeOrganizationRoles insert error", {
        error: error.message,
      });
      return { success: false, error: error.message };
    }

    logInfo("RBAC", "Organization roles initialized", {
      organizationId,
      count: data.length,
    });
    ...
    logError("RBAC", "initializeOrganizationRoles unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }

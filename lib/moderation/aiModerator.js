...
const POLICY_VIOLATIONS = {
  NO_PERSONAL_INFO: "Do not share personal information (addresses, phone numbers, IDs)",
  NO_MISLEADING_CLAIMS: "Campaign descriptions must be truthful and not make misleading claims",
  NO_ILLEGAL_ACTIVITY: "Campaigns must not promote or fund illegal activities",
  NO_HATE_SPEECH: "Content must not contain hate speech or discriminatory language",
  NO_HARASSMENT: "Content must not harass, bully, or intimidate individuals or groups",
  NO_SPAM: "Content must not be spam or deceptive promotional material",
  NO_GRAPHIC_CONTENT: "Content must not contain graphic violence, gore, or disturbing imagery",
  ...
  NO_SCAM_INDICATORS: "Campaign must not show common scam indicators (guaranteed returns, urgency pressure)",
};
...
function countSpamIndicators(text) {
  ...
  return SPAM_INDICATORS.filter((indicator) => lower.includes(indicator)).length;
}
...
function checkPersonalInfo(text) {
  const patterns = [
    ...
    { type: "address", regex: /\b\d{1,5}\s+\w+\s+(street|st|avenue|ave|road|rd|boulevard|blvd|drive|dr)\b/i },
 ...
 */
export async function classifyContent({ entityType, entityId, content, title }) {
  try {
    if (!entityType) {
    ...
    if (personalInfoTypes.length > 0) {
      category = category === CONTENT_CATEGORIES.CLEAN ? CONTENT_CATEGORIES.INAPPROPRIATE : category;
    ...
    const harassmentPatterns = /\b(you're stupid|you're an? idiot|go away|shut up|worthless)\b/i;
    ...
    const misinfoPatterns = /\b(cure for all|miracle|proven to reverse|doctors don't want you to know|secret cure)\b/i;
    ...
    logInfo("AIModerator", "Content classified", { entityType, entityId, category, severity, confidence });
    ...
    logError("AIModerator", "Classify content error", { error: err.message, entityType, entityId });
...
export async function detectSpam({ content, authorId, context = {} }) {
  try {
    ...
    const urgencyPatterns = /\b(urgent|act now|limited time|hurry|don't miss|last chance|expires?|today only)\b/gi;
    ...
    if (context.recentPostCount && context.recentPostCount > 20) {
      confidence += 0.1;
      factors.push(`Author has ${context.recentPostCount} recent posts — possible spam account`);
    }

    if (context.similarPostCount && context.similarPostCount > 3) {
      isSpam = true;
      confidence += 0.25;
      factors.push(`${context.similarPostCount} similar posts detected — possible spam repetition`);
    }
    ...
    if (isSpam) {
      recommendation = "Content appears to be spam — recommend removal or review";
    } else if (confidence >= 0.3) {
      recommendation = "Possible spam — recommend manual review";
    }

    logInfo("AIModerator", "Spam detection completed", { authorId, isSpam, confidence });
    ...
    logError("AIModerator", "Detect spam error", { error: err.message, authorId });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function detectDuplicateCampaign({ title, description, category, creatorId }) {
  try {
    if (!title) {
    ...
    if (fetchError) {
      logError("AIModerator", "Duplicate check fetch error", { error: fetchError.message });
      return { success: false, error: "Failed to fetch campaigns for duplicate check" };
    }
    ...
    for (const existing of existingCampaigns || []) {
      ...
      const descSimilarity = calculateTextSimilarity(description, existing.description);
    ...
    const maxSimilarity = similarCampaigns.length > 0 ? similarCampaigns[0].similarity : 0;
    ...
    logError("AIModerator", "Detect duplicate campaign error", { error: err.message, creatorId });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function detectSuspiciousDescription({ description, category, goal }) {
  try {
    if (!description) {
      return { success: false, error: "description is required" };
    }
    ...
    for (const pattern of SUSPICIOUS_PATTERNS) {
      if (pattern.test(description)) {
        flags.push({
          ...
          description: "Description contains a pattern commonly associated with fraudulent content",
          severity: "medium",
        });
        confidence += 0.1;
      }
    ...
    if (description.trim().length < 100) {
      flags.push({
        ...
        description: "Campaign description is very short — lacks detail about how funds will be used",
        severity: "low",
      });
      confidence += 0.1;
    }
    ...
    const unrealisticPatterns = /\b(100%|guaranteed|no risk|guaranteed return|guaranteed profit)\b/i;
    ...
    if (redirectPatterns.test(description)) {
      flags.push({
        type: "shortened_url",
        description: "Description contains shortened URLs — may be hiding destination",
        severity: "medium",
      });
      confidence += 0.15;
    }
    ...
    logInfo("AIModerator", "Suspicious description check completed", { suspicious, flagCount: flags.length });
    ...
    logError("AIModerator", "Detect suspicious description error", { error: err.message });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function analyzeMediaAuthenticity({ mediaUrls, campaignContext = {} }) {
  try {
    if (!mediaUrls || !Array.isArray(mediaUrls) || mediaUrls.length === 0) {
      return { success: false, error: "mediaUrls array is required and must not be empty" };
    }
    ...
    for (const url of mediaUrls) {
      ...
      const stockIndicators = /\b(shutterstock|istockphoto|getty|adobe stock|depositphoto|dreamstime)\b/i;
      ...
      const memeIndicators = /\b(meme|funny|viral|memeconomy|imgflip|knowyourmeme)\b/i;
      ...
      if (hasOtherExt) {
        indicators.push({
          ...
          confidence: 0.9,
          description: "Media URL points to a potentially executable file — security risk",
        });
        totalScore += 0.8;
      }
    ...
    if (mediaUrls.length === 1) {
      indicators.push({
        type: "single_media",
        confidence: 0.3,
        description: "Only one media item — consider adding more for credibility",
      });
    }

    const overallAuthenticity = clamp(1 - totalScore / Math.max(mediaUrls.length, 1), 0, 1);
    ...
    logError("AIModerator", "Analyze media authenticity error", { error: err.message });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function suggestPolicyViolation({ content, entityType, policyContext = {} }) {
  try {
    if (!content) {
      return { success: false, error: "content is required" };
    }
    ...
    const misleadingPatterns = /\b(guaranteed|100%|no risk|miracle|secret cure|doctors don't want)\b/i;
    ...
    const illegalPatterns = /\b(drug traffic|money laundering|gambling ring|ponzi|pyramid scheme)\b/i;
    ...
    // Hate speech
    const hatePatterns = /\b(hate|kill|destroy|eliminate)\s+(all|every)\s+(men|women|jews|muslims|christians|blacks|whites|asians|gays|lesbians|trans)\b/i;
    ...
    // Harassment
    const harassmentPatterns = /\b(you('re|\s+are)\s+(stupid|idiot|worthless|ugly|fat|pathetic|loser))\b/i;
    ...
    if (entityType === "campaign" && content.trim().length < 100) {
      violations.push({
        ...
        description: "Campaign description is below the minimum length requirement (100 characters)",
        severity: "low",
        confidence: 0.95,
      });
    }
    ...
    if (violations.length > 0) {
      ...
      if (criticalCount > 0) {
        recommendation = "Critical violations detected — immediate action required (content removal recommended)";
      } else if (highCount > 0) {
        recommendation = "Serious violations detected — content review required before approval";
      } else {
        recommendation = "Minor policy issues detected — consider requesting content modifications";
      }
    }

    logInfo("AIModerator", "Policy violation check completed", { entityType, violationCount: violations.length });
    ...
    logError("AIModerator", "Suggest policy violation error", { error: err.message, entityType });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function calculateModerationConfidence({ signals = {}, aiResults = {}, ruleResults = {} }) {
  try {
    ...
    const signalConfidence = signalCount > 0 ? clamp(signalScore / signalCount, 0, 1) : 0;
    ...
    if (ruleResults.length !== undefined && Array.isArray(ruleResults)) {
      ...
      ruleConfidence = ruleCount > 0 ? clamp(ruleConfidence / ruleCount, 0, 1) : 0;
    } else if (typeof ruleResults === "object") {
      const matchedRules = Object.values(ruleResults).filter((r) => r.matched);
      if (matchedRules.length > 0) {
        ruleConfidence = clamp(
          matchedRules.reduce((sum, r) => sum + (r.confidence || 0.5), 0) / matchedRules.length,
          0,
          1
        );
      }
    ...
    const overallConfidence = totalWeight > 0 ? clamp(weightedSum / totalWeight, 0, 1) : 0;

    logInfo("AIModerator", "Moderation confidence calculated", { overallConfidence });
    ...
    logError("AIModerator", "Calculate moderation confidence error", { error: err.message });
    return { success: false, error: "Failed to calculate moderation confidence" };
  }
}

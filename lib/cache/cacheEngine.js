...
export async function invalidatePattern(pattern, options = {}) {
  ...
  try {
    ...
    return { success: false, error: `Pattern invalidation not supported for ${backend}` };
  } catch (err) {
    return { success: false, error: err.message };
  }
}
...
async function getFromDatabase(key) {
  ...
  await supabaseAdmin
    .from("cache_metadata")
    .update({ hits: (data.hits || 0) + 1, last_accessed_at: new Date().toISOString() })
    .eq("cache_key", key);
...
export async function checkRateLimit(key, options = {}) {
  ...
  return {
    ...
    data: {
      ...
      remaining,
      resetAt,
      retryAfter: limiter.count > maxRequests ? Math.ceil((resetAt - now) / 1000) : 0,
    },
  };
}

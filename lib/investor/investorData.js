 ...
 */
export async function loadInvestorDonations(client, userId, { force = false } = {}) {
  ...
  for (const d of rows) {
    ...
    if (status === SETTLED_DONATION_STATUS) {
      ...
      if (d.created_at) {
        ...
        if (!firstSettledAt || new Date(d.created_at) < new Date(firstSettledAt)) {
          firstSettledAt = d.created_at;
        }
        if (!lastSettledAt || new Date(d.created_at) > new Date(lastSettledAt)) {
      ...
      if (d.project_id && project) {
        ...
        if (existing) {
          ...
          if (!existing.lastDonatedAt || new Date(d.created_at) > new Date(existing.lastDonatedAt)) {
...
export function computePortfolioHealth(stats, { now = Date.now() } = {}) {
  ...
  if (firstAt && activeMonths > 0) {
    const elapsedMs = Math.max(0, new Date(now).getTime() - new Date(firstAt).getTime());
    const monthsElapsed = Math.max(1, Math.round(elapsedMs / (1000 * 60 * 60 * 24 * 30.44)));
    consistency = Math.min(100, Math.round((activeMonths / monthsElapsed) * 100));
  }

  const score = Math.round(
    diversification * 0.3 + fundingProgress * 0.3 + categorySpread * 0.2 + consistency * 0.2,
  );

  return {
    score,
    breakdown: { diversification, fundingProgress, categorySpread, consistency },
  };
}
...
export async function loadInvestorPortfolio(client, userId) {
  ...
  return { stats: derivePortfolioStats(donations), savedProjects, followers, donations };
}
...
export function deriveAiInsights({ stats, donations, recommendations }) {
  ...
  const confidence = Math.max(
    10,
    Math.min(100, Math.round(20 + settledCount * 5 + topCategories.length * 8 + Math.min(spread, 20))),
  );
 ...
 */
export async function loadRecommendedProjects(client, userId, { limit = 6 } = {}) {
  ...
  let query = client.from("projects").select(RECOMMENDED_SELECT).eq("deleted", false);
  ...
  const candidates = (Array.isArray(data) ? data : []).filter((p) => !fundedIds.has(p.id));
  ...
  const scored = candidates.map((p) => {
    ...
    return { project: p, growthScore, rankScore: growthScore + overlap * 5, overlap, cats };
  });
  ...
  const ownerIds = [...new Set(recommendations.map((r) => r.project.owner_id).filter(Boolean))];
  let creatorMap = {};
 ...
 */
export function deriveAnalytics(donations, { range = "all", now = Date.now() } = {}) {
  const rows = Array.isArray(donations) ? donations : [];
  ...
  const filtered =
    cutoffMs == null
      ? rows
      : rows.filter((d) => d.created_at && new Date(d.created_at).getTime() >= cutoffMs);

  const settled = filtered.filter((d) => (d.status || "") === SETTLED_DONATION_STATUS);
  ...
  for (const d of settled) {
    ...
    const entry = byMonth.get(key) || { label: monthLabel(date), sum: 0, count: 0 };
  ...
  const monthlyInvestment = months.map((m) => ({ month: m.label, amount: m.sum }));
  ...
  for (const d of settled) {
    ...
    const p = d.projects;
    if (d.project_id) {
      const entry = projectAmounts.get(d.project_id) || { title: p?.title || "Unknown", sum: 0 };
      entry.sum += amount;
  ...
  const projectSorted = [...projectAmounts.values()].sort((a, b) => b.sum - a.sum);
  ...
  // ---- Performance metrics ----
  const totalInvested = settled.reduce((acc, d) => acc + (Number(d.amount) || 0), 0);
  const fundedIds = new Set(settled.map((d) => d.project_id).filter(Boolean));
  const projectsFunded = fundedIds.size;
  const avgPerProject = projectsFunded > 0 ? totalInvested / projectsFunded : 0;
  const averageDonation = settled.length > 0 ? totalInvested / settled.length : 0;
  const largestDonation = settled.reduce((m, d) => Math.max(m, Number(d.amount) || 0), 0);
  ...
  const successRate = terminal > 0 ? Math.round(((settled.length + refunded.length) / terminal) * 100) : 0;

  const activeProjects = new Set(
    settled.filter((d) => d.projects && d.projects.deleted === false).map((d) => d.project_id),
  ).size;
  ...
  const roi =
    progressList.length > 0
      ? Math.round(
          (progressList.reduce((acc, x) => acc + Math.min(x.pledged / x.goal, 1), 0) /
            progressList.length) *
            100,
        )
      : 0;
 ...
 */
export async function loadInvestorAnalytics(client, userId, { range = "all", now } = {}) {
  const donations = await loadInvestorDonations(client, userId);

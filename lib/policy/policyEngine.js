...
export async function createPolicy({
  ...
  try {
    ...
    if (existing) {
      return {
        success: false,
        error: `Policy with key '${policyKey}' already exists`,
      };
    }
    ...
    if (error) {
      logError("PolicyEngine", "Create policy error", {
        error: error.message,
        policyKey,
      });
...
export async function getPolicy(policyId) {
  try {
    ...
    if (error) {
      ...
      logError("PolicyEngine", "Fetch policy error", {
        error: error.message,
        policyId,
      });
...
export async function getPolicyByKey(policyKey) {
  try {
    ...
    if (error) {
      ...
      logError("PolicyEngine", "Fetch policy by key error", {
        error: error.message,
        policyKey,
      });
      return { success: false, error: "Failed to fetch policy" };
...
export async function updatePolicyValue(
  ...
  try {
    ...
    if (
      current.policy_type === "threshold" ||
      current.policy_type === "number"
    ) {
      if (typeof newValue !== "number") {
        return {
          success: false,
          error: "Value must be a number for this policy type",
        };
      }
    ...
    if (current.allowed_values && current.allowed_values.length > 0) {
      ...
      const allAllowed = valueToCheck.every((v) =>
        current.allowed_values.includes(v),
      );
      if (!allAllowed) {
        return {
          success: false,
    ...
    if (versionError) {
      logError("PolicyEngine", "Create version entry error", {
    ...
    if (error) {
      logError("PolicyEngine", "Update policy value error", {
        error: error.message,
        policyId,
      });
...
export async function getPolicyVersions(policyId) {
  try {
    ...
    if (error) {
      logError("PolicyEngine", "Fetch versions error", {
        error: error.message,
        policyId,
      });
...
export async function evaluatePolicy(policyKey, context = {}) {
  try {
    ...
    logError("PolicyEngine", "Evaluate policy error", {
      error: err.message,
      policyKey,
    });
...
export async function getActivePolicies() {
  try {
    ...
    if (error) {
      logError("PolicyEngine", "Fetch active policies error", {
        error: error.message,
      });
...
export async function initializeDefaultPolicies() {
  try {
    const defaultPolicies = [
      {
        ...
        description:
          "Minimum trust score required for a user to perform sensitive actions",
      ...
      },
      {
        ...
        description:
          "List of verification types required for full account access",
        category: "verification",
        policyType: "array",
      ...
      },
      {
        ...
        description:
          "Fraud score at or above which the user is automatically blocked",
        category: "fraud",
        policyType: "threshold",
        value: 75,
      },
      {
        ...
        description:
          "Fraud score at or above which the user is flagged for monitoring",
        category: "fraud",
        policyType: "threshold",
        value: 50,
      },
      ...
      },
      {
        ...
        description:
          "Review score at or above which milestones are auto-approved",
        category: "milestone",
        policyType: "threshold",
        value: 80,
      },
    ...
    for (const policy of defaultPolicies) {
      ...
      if (existing) {
        logInfo("PolicyEngine", "Default policy already exists", {
    ...
    logError("PolicyEngine", "Initialize default policies error", {
      error: err.message,
    });

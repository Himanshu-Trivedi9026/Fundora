...
export async function createNotification({
  ...
  try {
    if (!userId || !notificationType) {
      return { success: false, error: "userId and notificationType are required" };
    }
    ...
    if (error) {
      logError("NotificationEngine", "Failed to create notification", { error: error.message, userId, notificationType });
      return { success: false, error: "Failed to create notification" };
    }

    logInfo("NotificationEngine", "Notification created", { notificationId: notification.id, userId, notificationType });

    return { success: true, data: notification };
  } catch (error) {
    logError("NotificationEngine", "Error creating notification", { error: error.message });
    return { success: false, error: "Failed to create notification" };
  }
}
...
export async function getNotifications({
  ...
  try {
    ...
    if (error) {
      logError("NotificationEngine", "Failed to query notifications", { error: error.message, userId });
      return { success: false, error: "Failed to query notifications" };
    }

    return { success: true, data: data || [], total: count || 0 };
  } catch (error) {
    logError("NotificationEngine", "Error querying notifications", { error: error.message });
    return { success: false, error: "Failed to query notifications" };
  }
}
...
export async function getUnreadCount(userId) {
  try {
    ...
    if (error) {
      logError("NotificationEngine", "Failed to get unread count", { error: error.message, userId });
      return { success: false, error: "Failed to get unread count" };
    }

    return { success: true, data: count || 0 };
  } catch (error) {
    logError("NotificationEngine", "Error getting unread count", { error: error.message });
    return { success: false, error: "Failed to get unread count" };
  }
}
...
export async function markAsRead(notificationId, userId) {
  try {
    ...
    if (existing.user_id !== userId) {
      logWarn("NotificationEngine", "Unauthorized markAsRead attempt", { notificationId, userId });
      return { success: false, error: "You can only update your own notifications" };
    }
    ...
    if (error) {
      logError("NotificationEngine", "Failed to mark notification as read", { error: error.message, notificationId });
      return { success: false, error: "Failed to mark notification as read" };
    }

    return { success: true, data };
  } catch (error) {
    logError("NotificationEngine", "Error marking notification as read", { error: error.message });
    return { success: false, error: "Failed to mark notification as read" };
  }
}
...
export async function markAllAsRead(userId) {
  try {
    ...
    if (error) {
      logError("NotificationEngine", "Failed to mark all as read", { error: error.message, userId });
      return { success: false, error: "Failed to mark all notifications as read" };
    }

    const updatedCount = data?.length || 0;
    logInfo("NotificationEngine", "All notifications marked as read", { userId, updatedCount });

    return { success: true, data: updatedCount };
  } catch (error) {
    logError("NotificationEngine", "Error marking all notifications as read", { error: error.message });
    return { success: false, error: "Failed to mark all notifications as read" };
  }
}
...
export async function deleteNotification(notificationId, userId) {
  try {
    ...
    if (existing.user_id !== userId) {
      logWarn("NotificationEngine", "Unauthorized delete attempt", { notificationId, userId });
    ...
    if (error) {
      logError("NotificationEngine", "Failed to delete notification", { error: error.message, notificationId });
      return { success: false, error: "Failed to delete notification" };
    }

    logInfo("NotificationEngine", "Notification deleted", { notificationId, userId });

    return { success: true };
  } catch (error) {
    logError("NotificationEngine", "Error deleting notification", { error: error.message });
    return { success: false, error: "Failed to delete notification" };
  }
}
...
export async function getNotificationPreferences(userId) {
  try {
    ...
    if (error && isTableMissing(error)) {
      logWarn("NotificationEngine", "notification_preferences table missing (migration 014 not applied); using defaults", { userId });
      return { success: true, data: { user_id: userId, preferences: DEFAULT_PREFERENCES } };
    }

    if (error && error.code !== "PGRST116") {
      logError("NotificationEngine", "Failed to get preferences", { error: error.message, userId });
    ...
    if (!data) {
      ...
      if (createError) {
        logError("NotificationEngine", "Failed to create default preferences", { error: createError.message, userId });
    ...
    logError("NotificationEngine", "Error getting preferences", { error: error.message });
    return { success: false, error: "Failed to get notification preferences" };
  }
}
...
export async function updateNotificationPreferences(userId, preferences) {
  try {
    ...
    if (error) {
      logError("NotificationEngine", "Failed to update preferences", { error: error.message, userId });
    ...
    logError("NotificationEngine", "Error updating preferences", { error: error.message });
    return { success: false, error: "Failed to update notification preferences" };
  }
}
...
export async function sendNotification({
  ...
  try {
    ...
    logError("NotificationEngine", "Error sending notification", { error: error.message });

...
export async function createReview({
  ...
  try {
    if (!milestoneId || !reviewerId || !decision) {
      return {
        success: false,
        error: "milestoneId, reviewerId, and decision are required",
      };
    }
    ...
    if (checkError) {
      logError("MilestoneReview", "Check existing review error", {
    ...
    if (existingReview) {
      return {
        success: false,
        error: "You have already reviewed this milestone",
      };
    }
    ...
    if (milestone.status !== "submitted" && milestone.status !== "active") {
      return {
        success: false,
        error: `Cannot review milestone in '${milestone.status}' status`,
      };
    }
    ...
    if (donError) {
      logError("MilestoneReview", "Fetch donations error", {
        error: donError.message,
      });
    ...
    if (insertError) {
      ...
      logError("MilestoneReview", "Create review error", {
...
export async function getMilestoneReviews(milestoneId) {
  try {
    ...
    if (error) {
      logError("MilestoneReview", "Get milestone reviews error", {
        error: error.message,
      });
...
export async function getReviewStats(milestoneId) {
  try {
    ...
    if (error) {
      logError("MilestoneReview", "Get review stats error", {
        error: error.message,
      });
    ...
    stats.approvalPercentage =
      totalDecisionWeight > 0
        ? Math.round((stats.approveWeight / totalDecisionWeight) * 100)
        : 0;
...
export async function getUserReview(milestoneId, reviewerId) {
  try {
    if (!milestoneId || !reviewerId) {
      return {
        success: false,
        error: "milestoneId and reviewerId are required",
      };
    }
    ...
    if (error) {
      logError("MilestoneReview", "Get user review error", {
        error: error.message,
      });
...
export async function updateReview(reviewId, reviewerId, updates) {
  try {
    ...
    if (error) {
      logError("MilestoneReview", "Update review error", {
        error: error.message,
      });
...
export async function deleteReview(reviewId, reviewerId) {
  try {
    ...
    if (error) {
      logError("MilestoneReview", "Delete review error", {
        error: error.message,
      });
...
async function recalculateApproval(milestoneId) {
  try {
    ...
    if (fetchError) {
      logError("MilestoneReview", "Recalculate approval fetch error", {
    ...
    const approvalPercentage =
      decisionWeight > 0
        ? Math.round((approveWeight / decisionWeight) * 100)
        : 0;
    ...
    if (updateError) {
      logError("MilestoneReview", "Recalculate approval update error", {
    ...
    logError("MilestoneReview", "Recalculate approval error", {
      error: err.message,
    });
  }
}

...
export async function submitMilestone({
  ...
  try {
    if (!milestoneId || !creatorId || !title) {
      return { success: false, error: "milestoneId, creatorId, and title are required" };
    }
    ...
    if (milestone.creator_id !== creatorId) {
      return { success: false, error: "You can only submit evidence for your own milestones" };
    }

    if (milestone.status === "completed" || milestone.status === "cancelled") {
      return { success: false, error: `Cannot submit evidence for milestone in '${milestone.status}' status` };
    }
    ...
    if (insertError) {
      logError("MilestoneSubmission", "Create submission error", { error: insertError.message });
    ...
    if (updateError) {
      logError("MilestoneSubmission", "Update milestone status error", { error: updateError.message });
    ...
    logError("MilestoneSubmission", "Submit milestone error", { error: err.message });
...
export async function getSubmissions(milestoneId) {
  try {
    ...
    if (error) {
      logError("MilestoneSubmission", "Get submissions error", { error: error.message });
      return { success: false, error: "Failed to fetch submissions" };
    }

    return { success: true, data: data || [] };
  } catch (err) {
    logError("MilestoneSubmission", "Get submissions error", { error: err.message });
    return { success: false, error: "Failed to fetch submissions" };
  }
}
...
export async function getSubmission(submissionId) {
  try {
    ...
    logError("MilestoneSubmission", "Get submission error", { error: err.message });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function updateSubmissionStatus(submissionId, reviewerId, status, reviewNotes) {
  try {
    if (!submissionId || !reviewerId || !status) {
      return { success: false, error: "submissionId, reviewerId, and status are required" };
    }
    ...
    if (error) {
      logError("MilestoneSubmission", "Update submission status error", { error: error.message });
    ...
    await logAuditEvent({
      ...
      details: { previousStatus: existing.status, newStatus: status, milestoneId: existing.milestone_id },
    });
    ...
    logError("MilestoneSubmission", "Update submission status error", { error: err.message });
    return { success: false, error: "Failed to update submission status" };
  }
}
...
export async function getCreatorSubmissions(creatorId) {
  try {
    ...
    if (error) {
      logError("MilestoneSubmission", "Get creator submissions error", { error: error.message });
      return { success: false, error: "Failed to fetch creator submissions" };
    }

    return { success: true, data: data || [] };
  } catch (err) {
    logError("MilestoneSubmission", "Get creator submissions error", { error: err.message });
    return { success: false, error: "Failed to fetch creator submissions" };
  }
}

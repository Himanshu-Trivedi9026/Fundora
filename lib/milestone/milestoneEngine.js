...
export async function createMilestone({
  ...
  try {
    if (!campaignId || !creatorId || !title || !targetAmount) {
      return {
        success: false,
        error: "campaignId, creatorId, title, and targetAmount are required",
      };
    }
    ...
    if (releaseAmount && releaseAmount > targetAmount) {
      return {
        success: false,
        error: "releaseAmount cannot exceed targetAmount",
      };
    }
    ...
    if (error) {
      logError("MilestoneEngine", "Create milestone error", {
        error: error.message,
      });
...
export async function activateMilestone(milestoneId, creatorId) {
  try {
    if (!milestoneId || !creatorId) {
      return {
        success: false,
        error: "milestoneId and creatorId are required",
      };
    }
    ...
    if (existing.creator_id !== creatorId) {
      return {
        success: false,
        error: "You can only activate your own milestones",
      };
    }
    ...
    if (existing.status !== MILESTONE_STATUSES.DRAFT) {
      return {
        success: false,
        error: `Cannot activate milestone in '${existing.status}' status`,
      };
    }
    ...
    if (error) {
      logError("MilestoneEngine", "Activate milestone error", {
        error: error.message,
      });
    ...
    await logAuditEvent({
      ...
      action: "activate",
      details: {
        previousStatus: MILESTONE_STATUSES.DRAFT,
        newStatus: MILESTONE_STATUSES.ACTIVE,
      },
    });
...
export async function getCampaignMilestones(campaignId) {
  try {
    ...
    if (error) {
      logError("MilestoneEngine", "Get campaign milestones error", {
        error: error.message,
      });
...
export async function getCreatorMilestones(creatorId) {
  try {
    ...
    if (error) {
      logError("MilestoneEngine", "Get creator milestones error", {
        error: error.message,
      });
...
export async function updateMilestone(milestoneId, creatorId, updates) {
  try {
    ...
    if (existing.status !== MILESTONE_STATUSES.DRAFT) {
      return {
        success: false,
        error: `Cannot update milestone in '${existing.status}' status. Only draft milestones can be updated.`,
      };
    }
    ...
    if (error) {
      logError("MilestoneEngine", "Update milestone error", {
        error: error.message,
      });
...
export async function cancelMilestone(milestoneId, creatorId) {
  try {
    ...
    if (existing.creator_id !== creatorId) {
      return {
        success: false,
        error: "You can only cancel your own milestones",
      };
    }
    ...
    if (!validTransitions.includes(MILESTONE_STATUSES.CANCELLED)) {
      return {
        success: false,
        error: `Cannot cancel milestone in '${existing.status}' status`,
      };
    }
    ...
    if (error) {
      logError("MilestoneEngine", "Cancel milestone error", {
        error: error.message,
      });
...
export async function getMilestoneStats(campaignId) {
  try {
    ...
    if (error) {
      logError("MilestoneEngine", "Get milestone stats error", {
        error: error.message,
      });
    ...
    for (const m of milestones) {
      ...
      if (
        m.status === MILESTONE_STATUSES.ACTIVE ||
        m.status === MILESTONE_STATUSES.SUBMITTED
      ) {
    ...
    stats.averageApprovalPercentage =
      approvalCount > 0 ? Math.round(approvalSum / approvalCount) : 0;
...
export async function checkAutoApproval(milestoneId) {
  try {
    ...
    const { data: milestone, error: fetchError } = await supabaseAdmin
      .from("campaign_milestones")
      .select(
        "id, status, approval_percentage, auto_approve_threshold, total_reviews, approval_count",
      )
      .eq("id", milestoneId)
      .single();
    ...
    logError("MilestoneEngine", "Check auto-approval error", {
      error: err.message,
    });

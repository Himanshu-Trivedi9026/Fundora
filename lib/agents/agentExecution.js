...
export async function runAgent(agentId, input = {}, options = {}) {
  try {
    ...
    executeAgentRun(agent, run.id, input, options).catch((err) => {
      logError("Background agent execution failed", { agentId, runId: run.id, error: err.message });
    });
...
async function executeAgentRun(agent, runId, input, options) {
  ...
  try {
    ...
    const output = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error("Execution timeout")), maxTime)),
    ]);
    ...
    if (output.cost > (options.costThreshold || 0.1)) {
      const { checkThresholdAlert } = await import("../observability/alertManager.js");
      await checkThresholdAlert("agent_cost", output.cost, options.costThreshold || 0.1, {
        severity: "warning",
...
export async function approveAgentRun(runId, userId) {
  try {
    ...
    if (run.status !== "pending_approval") return { success: false, error: "Run is not awaiting approval" };

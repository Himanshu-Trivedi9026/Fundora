...
export function createAgentInstance(agentType, config = {}) {
  ...
  if (!typeDef) return { success: false, error: `Unknown agent type: ${agentType}` };
  ...
  const instance = {
    ...
    requiresApproval: config.requiresApproval !== undefined ? config.requiresApproval : typeDef.requiresApproval,

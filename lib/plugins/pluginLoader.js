...
class PluginLoader {
  ...
  async loadFromDb(pluginRecord) {
    ...
    if (!manifestResult.success) {
      return {
        success: false,
        error: `Invalid manifest for '${pluginRecord.name}': ${manifestResult.error}`,
      };
    }
  ...
  async loadFromManifest(raw) {
    ...
    const pluginId =
      `plugin_${manifestResult.data.name}_${manifestResult.data.version}`.replace(
        /[^a-zA-Z0-9_]/g,
        "_",
      );
  ...
  async executeHook(pluginId, hookName, context = {}) {
    ...
    if (!handler) {
      return {
        success: false,
        error: `Hook '${hookName}' not registered for '${pluginId}'`,
      };
    }

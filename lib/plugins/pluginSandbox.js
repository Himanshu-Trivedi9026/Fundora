...
class PluginSandbox {
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error("Plugin execution timeout")), this.timeout)
        ),
      ]);
  ...
  validateHookExecution(hookName) {
    ...
    if (!hooks[hookName]) {
      return { allowed: false, reason: `Hook '${hookName}' not implemented by plugin` };
    }
    return { allowed: true };
  }
  ...
  getRestrictedGlobals() {
    return [
      ...
      "global", "globalThis", "Buffer", "setImmediate",
    ];
  }
}

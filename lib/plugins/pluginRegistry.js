...
import { checkPluginPermission, PLUGIN_PERMISSIONS } from "./pluginPermissions.js";
...
class PluginRegistry {
  ...
  registerPlugin(pluginId, pluginInstance) {
    if (!pluginId || !pluginInstance) {
      return { success: false, error: "pluginId and pluginInstance are required" };
    }

    if (this._plugins.has(pluginId)) {
      return { success: false, error: `Plugin '${pluginId}' is already registered` };
    }
    ...
    return { success: true, data: { id: pluginId, manifest: manifestValidation.data } };
  }

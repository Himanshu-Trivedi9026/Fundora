...
const ALLOWED_TRANSITIONS = {
  [PLUGIN_STATUSES.DRAFT]: [
    PLUGIN_STATUSES.PENDING_REVIEW,
    PLUGIN_STATUSES.ARCHIVED,
  ],
  [PLUGIN_STATUSES.PENDING_REVIEW]: [
    PLUGIN_STATUSES.APPROVED,
    PLUGIN_STATUSES.REJECTED,
    PLUGIN_STATUSES.DRAFT,
  ],
  [PLUGIN_STATUSES.APPROVED]: [
    PLUGIN_STATUSES.PUBLISHED,
    PLUGIN_STATUSES.DISABLED,
  ],
...
export async function updatePluginStatus(pluginId, newStatus, performedBy) {
  ...
  try {
    ...
    return {
      success: true,
      data: { id: pluginId, status: newStatus, previousStatus: plugin.status },
    };
  } catch (err) {
...
export async function installPlugin(pluginId, userId, options = {}) {
  try {
    ...
    await supabaseAdmin.rpc("increment_plugin_downloads", {
      plugin_id: pluginId,
    });

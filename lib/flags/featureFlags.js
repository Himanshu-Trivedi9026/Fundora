...
export async function listFlags(options = {}) {
  try {
    ...
    if (options.search)
      query = query.or(
        `key.ilike.%${options.search}%,name.ilike.%${options.search}%`,
      );
...
function evaluateFlag(flag, context) {
  ...
  if (flag.organization_ids && flag.organization_ids.length > 0) {
    if (
      ...
      flag.organization_ids.includes(context.organizationId)
    ) {
      return true;
    }
...
export async function trackEvent(flagKey, userId, event, metadata = {}) {
  try {
    const { error } = await supabaseAdmin.from("feature_flag_events").insert({
      flag_key: flagKey,
      user_id: userId,
      event,
      metadata,

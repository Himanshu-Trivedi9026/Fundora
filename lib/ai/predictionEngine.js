...
function extractCampaignFeatures(campaign, creator) {
  ...
  const daysActive = Math.max(
    1,
    Math.floor(
      (Date.now() - new Date(campaign.created_at || Date.now()).getTime()) /
        86400000,
    ),
  );

  const earlyFundingRatio =
    goalAmount > 0 ? Math.min(currentAmount / goalAmount, 1) : 0;
...
function ruleBasedPrediction(features) {
  ...
  const probability = Math.max(
    0,
    Math.min(1, weightedSum / totalWeight + recencyBonus),
  );
...
function calculateConfidence(sampleSize, variance) {
  ...
  return Math.max(
    0.05,
    Math.min(1, baseConfidence + sizeContribution - variancePenalty),
  );
}
...
async function fetchCampaignWithCreator(campaignId) {
  const { data: campaign, error } = await supabaseAdmin
    .from("campaigns")
    .select(
      "*, creator:users(id, trust_score, reputation_score, total_raised, total_campaigns, created_at)",
    )
    .eq("id", campaignId)
    .single();

  if (error || !campaign) {
    logError("PredictionEngine", "Campaign fetch error", {
      error: error?.message,
      campaignId,
    });
  ...
  const creator = Array.isArray(campaign.creator)
    ? campaign.creator[0]
    : campaign.creator;
...
async function fetchDonationStats(campaignId, days = 30) {
  ...
  if (err1) {
    logError("PredictionEngine", "Donation stats fetch error", {
      error: err1.message,
      campaignId,
    });
    return {
...
export async function predictCampaignSuccess({ campaignId }) {
  try {
    ...
    const {
      ...
      creator,
      error: fetchErr,
    } = await fetchCampaignWithCreator(campaignId);
    if (fetchErr) {
    ...
    logError("PredictionEngine", "predictCampaignSuccess exception", {
      error: err.message,
      campaignId,
    });
    return {
      success: false,
      error: "Internal error predicting campaign success",
    };
  }
}
...
export async function predictFundingTimeline({ campaignId }) {
  try {
      ...
      const variance =
        amounts.length > 1
          ? amounts.reduce((sum, a) => sum + (a - mean) ** 2, 0) /
            (amounts.length - 1)
          : 1;
      const normalisedVariance =
        mean > 0 ? Math.min(variance / (mean * mean), 1) : 1;
    ...
    logError("PredictionEngine", "predictFundingTimeline exception", {
      error: err.message,
      campaignId,
    });
    return {
      success: false,
      error: "Internal error predicting funding timeline",
    };
  }
}
...
export async function predictDonationVelocity({ campaignId, windowDays = 30 }) {
  try {
    ...
    logInfo("PredictionEngine", "Predicting donation velocity", {
      campaignId,
      windowDays,
    });
    ...
    if (error) {
      logError("PredictionEngine", "Velocity donations fetch error", {
        error: error.message,
        campaignId,
      });
    ...
    const predictedVelocity = Math.max(
      0,
      Math.round((secondRate + velocityChange) * 1000) / 1000,
    );
    ...
    logError("PredictionEngine", "predictDonationVelocity exception", {
      error: err.message,
      campaignId,
    });
    return {
      success: false,
      error: "Internal error predicting donation velocity",
    };
  }
}
  ...
  try {
    ...
    if (features.earlyFundingRatio < 0.1 && features.daysActive > 14) {
      ...
      mitigationSuggestions.push(
        "Consider lowering the goal or increasing promotion",
      );
    }
    ...
    if (stats.recentDonations === 0 && features.daysActive > 7) {
      ...
      mitigationSuggestions.push(
        "Share campaign on social media and reach out to past supporters",
      );
    }

    // Declining donation velocity
    if (features.daysActive > 14) {
      const velocityResult = await predictDonationVelocity({
        campaignId,
        windowDays: 14,
      });
      if (
        velocityResult.success &&
        velocityResult.data?.trend === "decreasing"
      ) {
        riskScore += 0.15;
        keyRiskFactors.push("Donation velocity is declining");
        mitigationSuggestions.push(
          "Post a campaign update to re-engage donors",
        );
      }
    }
    ...
    if (features.updateCount === 0 && features.daysActive > 7) {
      ...
      mitigationSuggestions.push(
        "Post regular updates to maintain donor confidence",
      );
    }
    ...
    if (features.goalAmount > 50000 && features.earlyFundingRatio < 0.05) {
      ...
      mitigationSuggestions.push(
        "Consider a milestone-based approach with smaller sub-goals",
      );
    }
    ...
    if (features.donorCount < 3 && features.daysActive > 14) {
      ...
      mitigationSuggestions.push(
        "Ask friends and family for initial support to build momentum",
      );
    }
    ...
    logError("PredictionEngine", "predictFailureRisk exception", {
      error: err.message,
      campaignId,
    });
...
export async function predictRefundProbability({ donationId }) {
  try {
    ...
    logInfo("PredictionEngine", "Predicting refund probability", {
      donationId,
    });
    ...
    const { data: donation, error: donErr } = await supabaseAdmin
      .from("donations")
      .select(
        "id, amount, donor_id, campaign_id, created_at, donor:users(id, trust_score, refund_count, total_donations), campaign:campaigns(id, status, goal_amount, current_amount)",
      )
      .eq("id", donationId)
      .single();

    if (donErr || !donation) {
      logError("PredictionEngine", "Donation fetch error", {
    ...
    const donor = Array.isArray(donation.donor)
      ? donation.donor[0]
      : donation.donor;
    const campaign = Array.isArray(donation.campaign)
      ? donation.campaign[0]
      : donation.campaign;
    ...
    const donorFeatures = extractDonorFeatures(donor, {
      ...
      refundRate:
        donor?.refund_count && donor?.total_donations
    ...
    if (donorFeatures.refundRate > 0.3) {
      refundScore += 0.35;
      factors.push(
        `Donor has a ${Math.round(donorFeatures.refundRate * 100)}% historical refund rate`,
      );
    ...
    if (hoursSinceDonation < 24) {
      refundScore += 0.15;
      factors.push(
        "Donation was made within the last 24 hours (cooling-off period)",
      );
    ...
    if (donation.amount > 1000) {
      refundScore += 0.05;
      factors.push(
        `Large donation amount ($${donation.amount.toLocaleString()})`,
      );
    }
    ...
    logError("PredictionEngine", "predictRefundProbability exception", {
      error: err.message,
      donationId,
    });
    return {
      success: false,
      error: "Internal error predicting refund probability",
    };
  }
}
...
export async function predictMilestoneCompletion({ milestoneId }) {
  try {
    ...
    logInfo("PredictionEngine", "Predicting milestone completion", {
      milestoneId,
    });
    ...
    const { data: milestone, error: msErr } = await supabaseAdmin
      ...
      .select(
        "id, title, description, status, target_date, campaign_id, created_at, campaign:campaigns(id, status, creator_id, creator:users(id, trust_score, reputation_score, total_campaigns))",
      )
      .eq("id", milestoneId)
      .single();

    if (msErr || !milestone) {
      logError("PredictionEngine", "Milestone fetch error", {
        error: msErr?.message,
        milestoneId,
      });
    ...
    const campaign = Array.isArray(milestone.campaign)
      ? milestone.campaign[0]
      : milestone.campaign;
    const creator = campaign?.creator
      ? Array.isArray(campaign.creator)
        ? campaign.creator[0]
        : campaign.creator
      : null;
    ...
    if (milestone.status === "completed") {
      return {
        ...
        data: {
          probability: 1,
          estimatedCompletionDate:
            milestone.target_date || new Date().toISOString(),
          blockers: [],
        },
      };
    }
    ...
    const creatorReputation =
      creator?.reputation_score ?? creator?.trust_score ?? 0.5;
    ...
    if (creator?.id) {
      ...
      if (pastMilestones) {
        const creatorMilestones = pastMilestones.filter((m) => {
          const c = Array.isArray(m.campaign) ? m.campaign[0] : m.campaign;
          return c?.creator_id === creator.id;
        });
        const completed = creatorMilestones.filter(
        ...
        if (creatorMilestones.length > 0) {
          ...
          if (completionRate < 0.3) {
            blockers.push(
              `Creator has only ${Math.round(completionRate * 100)}% milestone completion rate`,
            );
          }
        }
      }
    ...
    if (!estimatedCompletionDate && probability > 0.3) {
      ...
      projected.setDate(
        projected.getDate() + Math.round(60 / Math.max(probability, 0.1)),
      );
    ...
    logError("PredictionEngine", "predictMilestoneCompletion exception", {
      error: err.message,
      milestoneId,
    });
    return {
      success: false,
      error: "Internal error predicting milestone completion",
    };
  }
}
...
export async function predictCreatorGrowth({ creatorId, windowDays = 90 }) {
  try {
    ...
    logInfo("PredictionEngine", "Predicting creator growth", {
      creatorId,
      windowDays,
    });
    ...
    const { data: creator, error: creatorErr } = await supabaseAdmin
      .from("users")
      .select(
        "id, follower_count, total_raised, total_campaigns, trust_score, created_at",
      )
      .eq("id", creatorId)
      .single();

    if (creatorErr || !creator) {
      logError("PredictionEngine", "Creator fetch error", {
    ...
    const recentAmount = recentDons.reduce(
    ...
    const accountAgeDays = Math.max(
      1,
      Math.floor(
        (now.getTime() - new Date(creator.created_at || now).getTime()) /
          86400000,
      ),
    );
    const currentFollowers = creator.follower_count || 0;
    const followerGrowthRate =
      accountAgeDays > 0 ? (currentFollowers / accountAgeDays) * 100 : 0;
    ...
    const projectedFollowers =
      currentFollowers +
      Math.round((projectedFollowerGrowth / 100) * futureDays);

    const projectedDonationRate =
      trend === "accelerating"
    ...
    logError("PredictionEngine", "predictCreatorGrowth exception", {
      error: err.message,
      creatorId,
    });
    return {
      success: false,
      error: "Internal error predicting creator growth",
    };
  }
}
...
export async function batchPredict({ entityType, entityIds, predictionType }) {
  try {
    if (
      ...
      return {
        success: false,
        error: "entityType and a non-empty entityIds array are required",
      };
    }
        ...
        switch (predictionType) {
          case PREDICTION_TYPES.CAMPAIGN_SUCCESS: {
            if (entityType !== "campaign") {
              prediction = {
                error: `Cannot predict campaign success for entity type: ${entityType}`,
              };
            } else {
          ...
          case PREDICTION_TYPES.FUNDING_TIMELINE: {
            if (entityType !== "campaign") {
              prediction = {
                error: `Cannot predict funding timeline for entity type: ${entityType}`,
              };
            } else {
          ...
          case PREDICTION_TYPES.DONATION_VELOCITY: {
            if (entityType !== "campaign") {
              prediction = {
                error: `Cannot predict donation velocity for entity type: ${entityType}`,
              };
            } else {
          ...
          case PREDICTION_TYPES.FAILURE_RISK: {
            if (entityType !== "campaign") {
              prediction = {
                error: `Cannot predict failure risk for entity type: ${entityType}`,
              };
            } else {
          ...
          case PREDICTION_TYPES.REFUND_PROBABILITY: {
            if (entityType !== "donation") {
              prediction = {
                error: `Cannot predict refund probability for entity type: ${entityType}`,
              };
            } else {
          ...
          case PREDICTION_TYPES.MILESTONE_COMPLETION: {
            if (entityType !== "milestone") {
              prediction = {
                error: `Cannot predict milestone completion for entity type: ${entityType}`,
              };
            } else {
          ...
          case PREDICTION_TYPES.CREATOR_GROWTH: {
            if (entityType !== "creator") {
              prediction = {
                error: `Cannot predict creator growth for entity type: ${entityType}`,
              };
            } else {
    ...
    logError("PredictionEngine", "batchPredict exception", {
      error: err.message,
      entityType,
      predictionType,
    });
    return {
      success: false,
      error: "Internal error running batch predictions",
    };
  }
}

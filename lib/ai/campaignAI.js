...
const POWER_WORDS = [
  "help", "support", "fund", "build", "create", "launch",
  "urgent", "critical", "essential", "transform", "empower",
  "community", "together", "make a difference", "change lives",
  "innovative", "breakthrough", "revolutionary", "impact",
];

const CATEGORY_KEYWORDS = {
  technology: ["tech", "software", "app", "ai", "machine learning", "blockchain", "saas", "platform", "digital"],
  health: ["health", "medical", "wellness", "fitness", "mental health", "therapy", "cure", "treatment", "clinical"],
  education: ["education", "learning", "school", "university", "student", "teach", "course", "training", "academic"],
  environment: ["environment", "climate", "green", "sustainability", "eco", "renewable", "carbon", "conservation"],
  arts: ["art", "music", "film", "creative", "design", "culture", "gallery", "exhibition", "performance"],
  community: ["community", "social", "neighborhood", "volunteer", "charity", "nonprofit", "cause", "movement"],
  business: ["business", "startup", "entrepreneur", "company", "product", "market", "revenue", "growth"],
  science: ["science", "research", "laboratory", "experiment", "discovery", "innovation", "data"],
};
...
function calculateDescriptionScore(description) {
  ...
  const hasHeadings = /#{1,3}\s/.test(description) || /^[A-Z][^\n]{5,}:$/m.test(description);
  ...
  const personalPronouns = (description.match(/\b(I|we|our|my|me)\b/gi) || []).length;
...
export async function scoreCampaignQuality({ campaignId }) {
  try {
    ...
    const { data: campaign, error: fetchError } = await supabaseAdmin
      .from("campaigns")
      .select("id, title, description, goal_amount, category, media_urls, creator_id")
      .eq("id", campaignId)
      .single();

    if (fetchError) {
      logError("CampaignAI", "Score campaign fetch error", { error: fetchError.message, campaignId });
    ...
    const mediaCount = Array.isArray(campaign.media_urls) ? campaign.media_urls.length : 0;
    const mediaScore = calculateMediaScore(mediaCount);
    const goalScore = calculateGoalScore(campaign.goal_amount, campaign.category);
    const categoryScore = calculateCategoryScore(campaign.category, campaign.title, campaign.description);
    ...
    const suggestions = [];
    if (titleScore < 60) suggestions.push("Improve your title — make it more descriptive and compelling");
    if (descriptionScore < 50) suggestions.push("Expand your description with more details about your project");
    if (mediaScore < 50) suggestions.push("Add more images or media to showcase your campaign");
    if (goalScore < 60) suggestions.push("Consider adjusting your funding goal to better match your category");
    if (categoryScore < 50) suggestions.push("Ensure your content aligns with your chosen category");

    logInfo("CampaignAI", "Campaign quality scored", { campaignId, overallScore });
    ...
    logError("CampaignAI", "Score campaign quality error", { error: err.message, campaignId });
...
export async function suggestCampaignTitles({ title, category, goal }) {
  try {
    ...
    if (wordCount > 10) {
      const shortened = title.split(/[,.]|\s+/).slice(0, 8).join(" ").trim();
      suggestions.push({
        title: shortened,
        score: 65,
    ...
    logInfo("CampaignAI", "Title suggestions generated", { title, suggestionsCount: suggestions.length });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function improveCampaignDescription({ title, description, category, goal }) {
  try {
    if (!description) {
    ...
    if (len < DESCRIPTION_MIN_LENGTH) {
      suggestions.push(`Your description is too short (${len} characters). Aim for at least ${DESCRIPTION_MIN_LENGTH} characters`);
    }

    // Check structure
    if (paragraphs.length < 2) {
      suggestions.push("Break your description into multiple paragraphs for better readability");
    }

    // Check for personal touch
    const hasPersonalPronouns = /\b(I|we|our|my|me)\b/i.test(description);
    if (!hasPersonalPronouns) {
      suggestions.push("Add a personal touch — tell your story using first-person language");
    }

    // Check for call to action
    const hasCTA = /\b(donate|support|contribute|help|fund|join)\b/i.test(description);
    if (!hasCTA) {
      suggestions.push("Include a clear call to action encouraging people to donate");
    }

    // Check for goal mention
    const hasGoalMention = /\$[\d,]+/.test(description);
    if (!hasGoalMention && goal) {
      suggestions.push(`Mention your funding goal ($${goal.toLocaleString()}) in the description`);
    }

    // Check for updates/benefits section
    const hasBenefits = /\b(benefit|impact|result|outcome|change|improve|achieve)\b/i.test(description);
    if (!hasBenefits) {
      suggestions.push("Describe the impact and benefits of your campaign");
    }

    // Check for media references
    const hasMediaRef = /\b(photo|image|video|watch|see|look)\b/i.test(description);
    if (!hasMediaRef) {
      suggestions.push("Reference your images or videos to engage readers visually");
    }
    ...
    logInfo("CampaignAI", "Description analysis completed", { title, suggestionsCount: suggestions.length });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function recommendFundingGoal({ category, campaignType, similarCampaigns = [] }) {
  try {
    ...
    if (similarCampaigns.length > 0) {
      const avgSimilar = similarCampaigns.reduce((sum, c) => sum + (c.goal_amount || 0), 0) / similarCampaigns.length;
      const similarSuccess = similarCampaigns.filter((c) => c.status === "funded" || c.raised >= c.goal_amount);

      if (similarSuccess.length > 0) {
        const avgSuccessful = similarSuccess.reduce((sum, c) => sum + (c.goal_amount || 0), 0) / similarSuccess.length;
    ...
    logInfo("CampaignAI", "Funding goal recommended", { category, recommended });
    ...
    logError("CampaignAI", "Recommend funding goal error", { error: err.message });
...
export async function predictCategory({ title, description }) {
  try {
    if (!title && !description) {
      return { success: false, error: "At least one of title or description is required" };
    }
    ...
    logInfo("CampaignAI", "Category predicted", { title, topCategory: results[0]?.category });
...
export async function observeCampaignRisk({ campaignId }) {
  try {
    ...
    const { data: campaign, error: fetchError } = await supabaseAdmin
      .from("campaigns")
      .select("id, title, description, goal_amount, category, media_urls, creator_id, status, created_at")
      .eq("id", campaignId)
      .single();
    ...
    if (campaign.goal_amount > categoryAvg * 5) {
      observations.push({
        ...
        suggestion: "Consider lowering the goal to a more achievable amount or providing strong justification",
      });
    }

    // Vague description
    if (!campaign.description || campaign.description.trim().length < DESCRIPTION_MIN_LENGTH) {
      observations.push({
        type: "vague_description",
        severity: "medium",
        description: "Campaign description is very short or missing",
        suggestion: "Add a detailed description including your goals, timeline, and how funds will be used",
      });
    }
    ...
    if (creatorProfile && creatorProfile.trust_score < 30) {
      observations.push({
        ...
        suggestion: "Creator should complete more verification steps and build a track record",
      });
    }
    ...
    if (mediaCount === 0) {
      observations.push({
        ...
        suggestion: "Add at least 2-3 images to make the campaign more engaging and trustworthy",
      });
    }

    // New creator with high goal
    if (creatorProfile && creatorProfile.campaigns_created <= 1 && campaign.goal_amount > 10000) {
      observations.push({
        type: "new_creator_high_goal",
        severity: "medium",
        description: "New creator with a relatively high funding goal",
        suggestion: "Consider starting with a smaller goal to build trust with the community",
      });
    }
    ...
    logInfo("CampaignAI", "Campaign risk observed", { campaignId, observationCount: observations.length });
    ...
    logError("CampaignAI", "Observe campaign risk error", { error: err.message, campaignId });
  ...
  try {
    ...
    if (title) {
      ...
      if (!title.includes(category)) {
        titleSuggestions.push(`${title} | ${category.charAt(0).toUpperCase() + category.slice(1)} Campaign`);
      }
      titleSuggestions.push(`${title} — Fund This Project on Fundora`);
    }

    logInfo("CampaignAI", "SEO suggestions generated", { title, keywordsCount: keywords.length });
    ...
    logError("CampaignAI", "Generate SEO suggestions error", { error: err.message });
...
export async function analyzeCompleteness({ campaignId }) {
  try {
    ...
    const required = [
      { field: "title", label: "Title", check: (c) => c.title && c.title.trim().length >= TITLE_MIN_LENGTH },
      { field: "description", label: "Description", check: (c) => c.description && c.description.trim().length >= DESCRIPTION_MIN_LENGTH },
      { field: "goal_amount", label: "Funding goal", check: (c) => c.goal_amount && c.goal_amount > 0 },
      { field: "category", label: "Category", check: (c) => c.category && c.category.length > 0 },
    ];

    const optional = [
      { field: "media_urls", label: "Media/images", check: (c) => Array.isArray(c.media_urls) && c.media_urls.length > 0 },
      { field: "end_date", label: "End date", check: (c) => c.end_date != null },
      { field: "tags", label: "Tags", check: (c) => Array.isArray(c.tags) && c.tags.length > 0 },
      { field: "short_description", label: "Short description", check: (c) => c.short_description && c.short_description.length > 0 },
      { field: "location", label: "Location", check: (c) => c.location && c.location.length > 0 },
      { field: "video_url", label: "Video URL", check: (c) => c.video_url && c.video_url.length > 0 },
    ];
    ...
    for (const field of optional) {
      if (field.check(campaign)) {
        ...
        improvements.push(`Add ${field.label.toLowerCase()} to improve your campaign`);
      }
    }

    const score = Math.round((completedFields / totalFields) * 100);

    logInfo("CampaignAI", "Completeness analyzed", { campaignId, score, missingCount: missing.length });
    ...
    logError("CampaignAI", "Analyze completeness error", { error: err.message, campaignId });
...
export async function batchQualityCheck({ campaignIds }) {
  try {
    if (!campaignIds || !Array.isArray(campaignIds) || campaignIds.length === 0) {
      return { success: false, error: "campaignIds array is required and must not be empty" };
    }
    ...
    for (let i = 0; i < campaignIds.length; i += batchSize) {
      ...
      if (fetchError) {
        logError("CampaignAI", "Batch quality check fetch error", { error: fetchError.message });
        return { success: false, error: "Failed to fetch campaigns for batch check" };
      }
    ...
    logInfo("CampaignAI", "Batch quality check completed", { totalChecked: results.length });

...
let routerConfig = {
  ...
  taskRoutes: { ...DEFAULT_ROUTER_CONFIG.taskRoutes },
};
...
function estimateModelCost(model, estimatedTokens = 1000) {
  ...
  return parseFloat(
    ((estimatedTokens / 1000) * pricing.per1k * 100).toFixed(4),
  );
}
...
export async function routeModel({
  ...
  try {
    ...
    if (!taskConfig) {
      throw new Error(
        `Unknown task type: '${taskType}'. Valid types: ${Object.values(TASK_TYPES).join(", ")}`,
      );
    }

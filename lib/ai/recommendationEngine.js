...
function collaborativeFilterScore(userHistory, allDonations, campaignId) {
  ...
  const frequencyScore = Math.min(
    matchCount / Math.max(allDonations.length, 1),
    1,
  );
  const recencyScore =
    allDonations.length > 0 ? weightedSum / allDonations.length : 0;
...
function trendingScore(stats, timeframe = "7d") {
  ...
  const acceleration =
    previous > 0
      ? (recent - previous) / Math.max(previous, 1)
      : recent > 0
        ? 1
        : 0;
...
function trustWeightedScore(campaignScore, trustScore) {
  const trust =
    typeof trustScore === "number" ? Math.max(0, Math.min(1, trustScore)) : 0.5;
...
async function fetchDonorHistory(donorId) {
  const { data, error } = await supabaseAdmin
    .from("donations")
    .select(
      "campaign_id, amount, created_at, campaign:campaigns(id, category, goal_amount, title)",
    )
    .eq("donor_id", donorId)
  ...
  if (error) {
    logError("RecommendationEngine", "Fetch donor history error", {
      error: error.message,
      donorId,
    });
    return {
...
async function fetchActiveCampaigns({
  ...
  let query = supabaseAdmin
    .from("campaigns")
    .select(
      "id, title, category, goal_amount, current_amount, status, created_at, creator_id, creator:users(id, trust_score)",
    )
    .eq("status", "active");
  ...
  if (error) {
    logError("RecommendationEngine", "Fetch active campaigns error", {
      error: error.message,
    });
...
async function fetchCampaignVelocity(campaignId, timeframe = "7d") {
  ...
  if (err1) {
    logError("RecommendationEngine", "Fetch velocity error (recent)", {
      error: err1.message,
      campaignId,
    });
    return {
  ...
  if (err2) {
    logError("RecommendationEngine", "Fetch velocity error (previous)", {
      error: err2.message,
      campaignId,
    });
    return {
 ...
 */
export async function getDonorRecommendations({
  donorId,
  limit = 10,
  excludeIds = [],
}) {
  try {
    ...
    logInfo("RecommendationEngine", "Generating donor recommendations", {
      donorId,
      limit,
    });
    ...
    const {
      ...
      donations: history,
      error: histErr,
    } = await fetchDonorHistory(donorId);
    if (histErr) {
    ...
    const { campaigns, error: campErr } = await fetchActiveCampaigns({
      limit: 100,
      excludeIds,
    });
    if (campErr) {
    ...
    if (simErr) {
      logError("RecommendationEngine", "Similar donors fetch error", {
    ...
    for (const campaign of campaigns) {
      ...
      if (contentScore > 0.6)
        reasons.push(
          `matches your interest in ${campaign.category || "this category"}`,
        );
    ...
    logError("RecommendationEngine", "getDonorRecommendations exception", {
      error: err.message,
      donorId,
    });
    return {
      success: false,
      error: "Internal error generating donor recommendations",
    };
  }
}
...
export async function getCampaignDonorSuggestions({ campaignId, limit = 20 }) {
  try {
    ...
    logInfo("RecommendationEngine", "Generating campaign donor suggestions", {
      campaignId,
      limit,
    });
    ...
    if (campErr || !campaign) {
      logError("RecommendationEngine", "Campaign fetch error", {
    ...
    if (catErr) {
      logError("RecommendationEngine", "Category donors fetch error", {
    ...
    for (const donation of categoryDonors || []) {
      ...
      if (campaign.goal_amount && donation.campaign.goal_amount) {
        const ratio =
          Math.min(donation.campaign.goal_amount, campaign.goal_amount) /
    ...
    for (const [donorId, stats] of Object.entries(donorScores)) {
      ...
      const frequencyScore = Math.min(
        stats.count / Math.max(totalDonors * 0.1, 1),
        1,
      );
      const categoryScore =
        stats.count > 0 ? stats.categoryMatches / stats.count : 0;
      ...
      if (categoryScore > 0.5)
        reasons.push(`funds ${campaign.category || "similar"} campaigns`);
    ...
    logError("RecommendationEngine", "getCampaignDonorSuggestions exception", {
      error: err.message,
      campaignId,
    });
    return {
      success: false,
      error: "Internal error generating donor suggestions",
    };
  }
}
...
export async function getSimilarCampaigns({ campaignId, limit = 5 }) {
  try {
    ...
    logInfo("RecommendationEngine", "Finding similar campaigns", {
      campaignId,
      limit,
    });
    ...
    if (campErr || !campaign) {
      logError("RecommendationEngine", "Reference campaign fetch error", {
        error: campErr?.message,
        campaignId,
      });
      return { success: false, error: "Campaign not found" };
    ...
    if (allErr) {
      logError("RecommendationEngine", "Fetch campaigns for similarity error", {
    ...
    for (const candidate of allCampaigns || []) {
      ...
      if (
        candidate.category &&
        campaign.category &&
        candidate.category === campaign.category
      ) {
      ...
      if (candidate.goal_amount && campaign.goal_amount) {
        const ratio =
          Math.min(candidate.goal_amount, campaign.goal_amount) /
      ...
      if (candidateTags.size > 0 && referenceTags.size > 0) {
        ...
        tagOverlap =
          matches /
          Math.max(candidateTags.size + referenceTags.size - matches, 1);
      }
      ...
      if (embScore > 0) {
        ...
        score =
          embScore * 0.6 +
          goalSimilarity * 0.15 +
          (sharedCategories.length > 0 ? 1 : 0) * 0.1 +
          tagOverlap * 0.15;
      } else {
        // Feature-based fallback
        score =
          (sharedCategories.length > 0 ? 1 : 0) * 0.4 +
          goalSimilarity * 0.3 +
          tagOverlap * 0.3;
      }
      ...
      if (sharedCategories.length > 0)
        reasons.push(`same category (${sharedCategories[0]})`);
    ...
    logError("RecommendationEngine", "getSimilarCampaigns exception", {
      error: err.message,
      campaignId,
    });
    return {
      success: false,
      error: "Internal error finding similar campaigns",
    };
  }
}
...
export async function getTrendingCampaigns({
  ...
  try {
    logInfo("RecommendationEngine", "Generating trending campaigns", {
      limit,
      timeframe,
      category,
    });
    ...
    for (const campaign of campaigns) {
      ...
      const reasons = [];
      if (recentDonations > 10)
        reasons.push(`${recentDonations} donations this period`);
      if (recentGrowth > 50)
        reasons.push(`${Math.round(recentGrowth)}% growth`);
    ...
    logError("RecommendationEngine", "getTrendingCampaigns exception", {
      error: err.message,
    });
    return {
      success: false,
      error: "Internal error generating trending campaigns",
    };
  }
}
...
export async function getCreatorRecommendations({ creatorId, limit = 5 }) {
  try {
    ...
    logInfo("RecommendationEngine", "Generating creator recommendations", {
      creatorId,
      limit,
    });
    ...
    if (campErr) {
      logError("RecommendationEngine", "Creator campaigns fetch error", {
        error: campErr.message,
        creatorId,
      });
    ...
    if (creatorErr) {
      logError("RecommendationEngine", "Creator profile fetch error", {
    ...
    for (const [category, stats] of Object.entries(categoryStats)) {
      ...
      if (successRate > 0.5)
        reasons.push(
          `${Math.round(successRate * 100)}% success rate in this category`,
        );
      if (avgRaised > avgGoal * 0.5)
        reasons.push(
          `averages ${Math.round((avgRaised / avgGoal) * 100)}% funding`,
        );
      ...
      results.push({
        ...
        reason:
          reasons.length > 0 ? reasons.join("; ") : "based on your profile",
    ...
    for (const cat of unexplored) {
      ...
      results.push({
        ...
        expectedSuccess:
          Math.round(Math.min(trustScore * 0.6 + 0.2, 1) * 1000) / 1000,
      });
    }
    ...
    logError("RecommendationEngine", "getCreatorRecommendations exception", {
      error: err.message,
      creatorId,
    });
    return {
      success: false,
      error: "Internal error generating creator recommendations",
    };
  }
}
...
export async function invalidateRecommendationCache({ userId, type } = {}) {
  try {
    logInfo("RecommendationEngine", "Invalidating recommendation cache", {
      userId,
      type,
    });
    ...
    if (!userId && !type) {
      return {
        success: false,
        error: "At least one of userId or type is required",
      };
    }
    ...
    if (error) {
      ...
      logError("RecommendationEngine", "Cache invalidation DB error", {
        error: error.message,
        userId,
        type,
      });
    ...
    logInfo("RecommendationEngine", "Recommendation cache invalidated", {
      userId,
      type,
    });
    ...
    logError(
      ...
      "invalidateRecommendationCache exception",
      { error: err.message, userId, type },
    );
    return {
      success: false,
      error: "Internal error invalidating recommendation cache",
    };
  }
}
  ...
  try {
    switch (type) {
      ...
      default:
        return {
          success: false,
          error: `Unknown recommendation type: ${type}`,
        };
    }
  } catch (err) {
    logError("RecommendationEngine", "getRecommendations failed", {
      userId,
      type,
      error: err.message,
    });

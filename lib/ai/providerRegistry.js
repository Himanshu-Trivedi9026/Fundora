...
export class MockModelProvider extends BaseModelProvider {
  ...
  async chatCompletion(params) {
    try {
      ...
      logError("ModelProvider", "Mock chatCompletion failed", {
        error: err.message,
      });
  ...
  async createEmbedding(params) {
    try {
      ...
      const totalTokens = input.reduce(
        (sum, text) => sum + Math.ceil((text?.length || 0) / 4),
        0,
      );
      ...
      logError("ModelProvider", "Mock createEmbedding failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
...
export class OpenAIModelProvider extends BaseModelProvider {
  ...
  async initialize(config = {}) {
    try {
      ...
      this.initialized = true;
      logInfo("ModelProvider", "OpenAI provider initialized", {
        model: this.model,
      });
    } catch (err) {
      logError("ModelProvider", "OpenAI initialization failed", {
        error: err.message,
      });
    }
  }
  ...
  async chatCompletion(params) {
    ...
    try {
      ...
      logError("ModelProvider", "OpenAI chatCompletion failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
  ...
  async createEmbedding(params) {
    ...
    try {
      ...
      logError("ModelProvider", "OpenAI createEmbedding failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
...
export class OpenRouterModelProvider extends BaseModelProvider {
  ...
  async initialize(config = {}) {
    try {
      ...
      this.initialized = true;
      logInfo("ModelProvider", "OpenRouter provider initialized", {
        model: this.model,
      });
    } catch (err) {
      logError("ModelProvider", "OpenRouter initialization failed", {
        error: err.message,
      });
    }
  }
  ...
  async chatCompletion(params) {
    ...
    try {
      ...
      logError("ModelProvider", "OpenRouter chatCompletion failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
  ...
  async createEmbedding(params) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorBody = await response.text();
        throw new Error(
          `OpenRouter embeddings API error ${response.status}: ${errorBody}`,
        );
      }
      ...
      logError("ModelProvider", "OpenRouter createEmbedding failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
...
export class GeminiModelProvider extends BaseModelProvider {
  ...
  async initialize(config = {}) {
    try {
      ...
      this.initialized = true;
      logInfo("ModelProvider", "Gemini provider initialized", {
        model: this.model,
      });
    } catch (err) {
      logError("ModelProvider", "Gemini initialization failed", {
        error: err.message,
      });
    }
  }
  ...
  async chatCompletion(params) {
    ...
    try {
      ...
      logInfo("ModelProvider", "Gemini chat completion", {
        model,
      ...
      logError("ModelProvider", "Gemini chatCompletion failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
  ...
  async createEmbedding(params) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorBody = await response.text();
        throw new Error(
          `Gemini embeddings error ${response.status}: ${errorBody}`,
        );
      }
      ...
      return {
        ...
        data: {
          ...
          usage: {
            prompt_tokens: Math.ceil((input[0]?.length || 0) / 4),
            total_tokens: Math.ceil((input[0]?.length || 0) / 4),
          },
        },
        error: null,
      };
    } catch (err) {
      logError("ModelProvider", "Gemini createEmbedding failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
...
export class AnthropicModelProvider extends BaseModelProvider {
  ...
  async initialize(config = {}) {
    try {
      ...
      this.initialized = true;
      logInfo("ModelProvider", "Anthropic provider initialized", {
        model: this.model,
      });
    } catch (err) {
      logError("ModelProvider", "Anthropic initialization failed", {
        error: err.message,
      });
    }
  }
  ...
  async chatCompletion(params) {
    if (!this.initialized) {
      return {
        success: false,
        data: null,
        error: "Anthropic provider not initialized",
      };
    }
    ...
    try {
      ...
      logError("ModelProvider", "Anthropic chatCompletion failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
  ...
  async createEmbedding(params) {
    ...
    logWarn(
      "ModelProvider",
      "Anthropic does not support embeddings — use another provider",
    );
    return {
      success: false,
      data: null,
...
export class LocalModelProvider extends BaseModelProvider {
  ...
  async initialize(config = {}) {
    try {
      this.baseUrl =
        config.baseUrl || process.env.LOCAL_AI_URL || "http://localhost:11434";
      this.model = config.model || this.defaultModel;
      ...
      logError("ModelProvider", "Local provider initialization failed", {
        error: err.message,
      });
    }
  }
  ...
  async chatCompletion(params) {
    ...
    try {
      ...
      logError("ModelProvider", "Local chatCompletion failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
    ...
    try {
      ...
      for (const text of input) {
        ...
        if (!response.ok) {
          const errorBody = await response.text();
          throw new Error(
            `Local embeddings error ${response.status}: ${errorBody}`,
          );
        }
      ...
      logError("ModelProvider", "Local createEmbedding failed", {
        error: err.message,
      });
      return { success: false, data: null, error: err.message };
    }
...
export function registerModelProvider(name, provider, config = {}) {
  try {
    ...
    logError("ModelRegistry", "Failed to register provider", {
      name,
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
...
export function getModelProvider(name) {
  try {
    ...
    logError("ModelRegistry", "Failed to get provider", {
      name,
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
...
export function setActiveModelProvider(name) {
  try {
    ...
    logError("ModelRegistry", "Failed to set active provider", {
      name,
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
...
export function getActiveModelProvider() {
  return (
    activeModelProvider ||
    modelProviders.get("mock")?.provider ||
...
export function listModelProviders() {
  try {
    const providers = Array.from(modelProviders.entries()).map(
      ([name, { provider }]) => ({
        name,
    ...
    logError("ModelRegistry", "Failed to list providers", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }
...
export async function initializeModelProviders() {
  try {
    ...
    logError("ModelRegistry", "Failed to initialize providers", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }

...
export async function getPromptTemplate(templateName, variables = {}) {
  try {
    ...
    try {
      ...
      logInfo("Prompt template not found in DB, using default", { templateName });
    }
    ...
    if (!systemResult.success) {
      return { success: false, error: `System prompt render failed: ${systemResult.error}` };
    }

    const userResult = renderPrompt(template.userTemplate, variables);
    if (!userResult.success) {
      return { success: false, error: `User prompt render failed: ${userResult.error}` };
    }

    logInfo("Prompt template loaded", { templateName, variablesUsed: Object.keys(variables) });
...
export async function createPromptTemplate({
  ...
  try {
    if (!name || !systemTemplate || !userTemplate) {
      return { success: false, error: "name, systemTemplate, and userTemplate are required" };
    }
...
export async function listPromptTemplates({ category, enabled } = {}) {
  try {
    ...
    const defaults = Object.values(DEFAULT_TEMPLATES)
      ...
      .filter((t) => (typeof enabled === "boolean" ? t.enabled === enabled : true))
      .map((t) => ({
        id: null,
        name: t.name,
        category: t.category,
    ...
    logInfo("Prompt templates listed", { count: allTemplates.length, category, enabled });
...
export async function updatePromptTemplate(templateId, updates, performedBy) {
  try {
    ...
    if (updates.systemTemplate !== undefined) dbUpdates.system_template = updates.systemTemplate;
    if (updates.userTemplate !== undefined) dbUpdates.user_template = updates.userTemplate;
    ...
    if (error) {
      logError("updatePromptTemplate DB error", { templateId, error: error.message });
      return { success: false, error: `Failed to update template: ${error.message}` };
    }

    logInfo("Prompt template updated", { templateId, updatedFields: Object.keys(dbUpdates) });

    return { success: true, data: { updated: true } };
  } catch (error) {
    logError("updatePromptTemplate error", { templateId, error: error.message });
    return { success: false, error: error.message };
  }
}
...
export async function suggestCampaignTitle({ title, category, goal, userId }) {
  try {
    ...
    logError("suggestCampaignTitle error", { title, category, error: err.message });

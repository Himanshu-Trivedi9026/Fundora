  ...
  try {
    ...
    if (!chunkResult.success) {
      return {
        success: false,
        data: null,
        error: `Chunking failed: ${chunkResult.error}`,
      };
    }
    ...
    if (articleError) {
      ...
      return {
        success: false,
        data: null,
        error: `Failed to store article: ${articleError.message}`,
      };
    }
    ...
    return {
      success: true,
      data: { id: article.id, chunkCount: chunks.length },
      error: null,
    };
  } catch (err) {
    logError("KnowledgeEngine", "indexKnowledgeArticle failed", {
      title,
      error: err.message,
    });
...
export async function searchKnowledge({
  query,
  category,
  matchCount = DEFAULT_SEARCH_MATCH_COUNT,
  ...
  try {
    ...
    logInfo("KnowledgeEngine", "No semantic results, using keyword fallback", {
      query,
    });
    return await keywordFallbackSearch({ query, category, matchCount });
  } catch (err) {
    logError("KnowledgeEngine", "searchKnowledge failed", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }
...
export async function getRelevantContext({
  ...
  try {
    ...
    if (entityTypes.includes("knowledge_article")) {
      ...
      if (knowledgeResult.success && knowledgeResult.data) {
        for (const article of knowledgeResult.data) {
          ...
          contextParts.push(`[Knowledge: ${article.title}]\n${snippetChars}`);
          sources.push({
    ...
    if (entityTypes.includes("campaign")) {
      const campaignResults = await searchCampaignContext({
        query,
        maxChars: maxChars - currentTokenEstimate,
      });
    ...
    if (entityTypes.includes("creator")) {
      const creatorResults = await searchCreatorContext({
        query,
        maxChars: maxChars - currentTokenEstimate,
      });
    ...
    logError("KnowledgeEngine", "getRelevantContext failed", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }
 ...
 */
export async function manageKnowledgeArticle(
  articleId,
  action,
  performedBy,
  updates = {},
  ...
  try {
    ...
    switch (action) {
      case "update": {
        ...
        if (updateError) {
          logError("KnowledgeEngine", "Failed to update article", {
            articleId,
        ...
        if (updates.content) {
          ...
          if (deleteErr) {
            logError(
              "KnowledgeEngine",
              "Failed to delete old embeddings on update",
              {
                articleId,
      ...
      case "delete": {
        ...
        if (embDeleteErr) {
          logError(
            "KnowledgeEngine",
            "Failed to delete embeddings on article delete",
            {
              articleId,
        ...
        if (articleDeleteErr) {
          ...
          return {
            success: false,
            data: null,
            error: `Failed to delete article: ${articleDeleteErr.message}`,
          };
        }
      ...
      case "archive": {
        ...
        if (archiveError) {
          ...
          return {
            success: false,
            data: null,
            error: `Failed to archive article: ${archiveError.message}`,
          };
        }
    ...
    logError("KnowledgeEngine", "manageKnowledgeArticle failed", {
      articleId,
      action,
      error: err.message,
    });
...
async function keywordFallbackSearch({ query, category, matchCount }) {
  try {
    ...
    if (dbError) {
      logError("KnowledgeEngine", "Keyword fallback search failed", {
        error: dbError.message,
      });
      return {
        success: false,
        data: null,
        error: `Keyword search failed: ${dbError.message}`,
      };
    }
    ...
    logInfo("KnowledgeEngine", "Keyword fallback search completed", {
      resultCount: results.length,
    });

    return { success: true, data: results, error: null };
  } catch (err) {
    logError("KnowledgeEngine", "keywordFallbackSearch failed", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }
...
async function searchCampaignContext({ query, maxChars }) {
  try {
    ...
    if (dbError) {
      logError("KnowledgeEngine", "Campaign context search failed", {
        error: dbError.message,
      });
      return { success: true, data: [], error: null };
    }
    ...
    logError("KnowledgeEngine", "searchCampaignContext failed", {
      error: err.message,
    });
    return { success: true, data: [], error: null };
  }
...
async function searchCreatorContext({ query, maxChars }) {
  try {
    ...
    if (dbError) {
      logError("KnowledgeEngine", "Creator context search failed", {
        error: dbError.message,
      });
      return { success: true, data: [], error: null };
    }
    ...
    logError("KnowledgeEngine", "searchCreatorContext failed", {
      error: err.message,
    });
    return { success: true, data: [], error: null };
  }
...
function findNaturalBreakPoint(text, startIndex, endIndex) {
  ...
  for (
    let i = endIndex - 1;
    i > startIndex + Math.floor((endIndex - startIndex) * 0.3);
    i--
  ) {
    const char = text[i];
    if (

  ...
  try {
    ...
    if (dbError) {
      ...
      return {
        success: false,
        data: null,
        error: `Failed to store embedding: ${dbError.message}`,
      };
    }
...
export async function batchCreateEmbeddings({ items, model }) {
  try {
    ...
    logError("EmbeddingEngine", "batchCreateEmbeddings failed", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }
...
export async function searchEmbeddings({
  ...
  try {
    ...
    if (rpcError || !rpcData || rpcData.length === 0) {
      ...
      logInfo(
        "EmbeddingEngine",
        "Vector search unavailable or empty, using text fallback",
        {
    ...
    logError("EmbeddingEngine", "searchEmbeddings failed", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }
  ...
  try {
    ...
    if (dbError) {
      ...
      return {
        success: false,
        data: null,
        error: `Failed to delete embeddings: ${dbError.message}`,
      };
    }
    ...
    logError("EmbeddingEngine", "deleteEmbeddings failed", {
      entityType,
      entityId,
      error: err.message,
    });
  ...
  try {
    ...
    if (fetchError) {
      ...
      return {
        success: false,
        data: null,
        error: `Failed to fetch stale embeddings: ${fetchError.message}`,
      };
    }
    ...
    logError("EmbeddingEngine", "refreshEmbeddings failed", {
      entityType,
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
...
async function textFallbackSearch({ query, entityType, matchCount }) {
  try {
    ...
    if (dbError) {
      logError("EmbeddingEngine", "Text fallback search failed", {
        error: dbError.message,
      });
      return {
        success: false,
        data: null,
        error: `Text search failed: ${dbError.message}`,
      };
    }
    ...
    logInfo("EmbeddingEngine", "Text fallback search completed", {
      resultCount: results.length,
    });

    return { success: true, data: results, error: null };
  } catch (err) {
    logError("EmbeddingEngine", "textFallbackSearch failed", {
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
  }

...
export async function createConversation({ userId, copilotType, title }) {
  try {
    ...
    if (!copilotType || !Object.values(COPILOT_TYPES).includes(copilotType)) {
      return {
        success: false,
        error: `copilotType must be one of: ${Object.values(COPILOT_TYPES).join(", ")}`,
      };
    }
    ...
    if (error) {
      logError("createConversation DB error", {
        userId,
        copilotType,
        error: error.message,
      });
    ...
    return {
      success: true,
      data: { id: data.id, conversationId: data.id, title: data.title },
    };
  } catch (error) {
...
export async function addMessage({
  ...
  try {
    ...
    if (!role || !Object.values(MESSAGE_ROLES).includes(role)) {
      return {
        success: false,
        error: `role must be one of: ${Object.values(MESSAGE_ROLES).join(", ")}`,
      };
    }
    if (!content && content !== "") {
...
export async function getConversationHistory(
  ...
  try {
    ...
    if (countError) {
      logError("getConversationHistory count error", {
        conversationId,
        error: countError.message,
      });
      return {
        success: false,
        error: `Failed to count messages: ${countError.message}`,
      };
    }
    ...
    if (error) {
      logError("getConversationHistory DB error", {
        conversationId,
        error: error.message,
      });
      return {
    ...
    logError("getConversationHistory error", {
      conversationId,
      error: error.message,
    });
    return { success: false, error: error.message };
...
export async function getActiveConversations(userId, { limit = 20 } = {}) {
  try {
    ...
    if (error) {
      logError("getActiveConversations DB error", {
        userId,
        error: error.message,
      });
      return {
...
export async function archiveConversation(conversationId, userId) {
  try {
    ...
    if (conversation.user_id !== userId) {
      ...
      return {
        success: false,
        error: "Unauthorized: you can only archive your own conversations",
      };
    }
    ...
    if (error) {
      logError("archiveConversation DB error", {
        conversationId,
        error: error.message,
      });
      return {
...
export async function summarizeConversation(conversationId) {
  try {
    ...
    if (error) {
      logError("summarizeConversation DB error", {
        conversationId,
        error: error.message,
      });
      return {
    ...
    try {
      ...
      logInfo("AI summary unavailable, using fallback", {
        conversationId,
    ...
    logError("summarizeConversation error", {
      conversationId,
      error: error.message,
    });
    return { success: false, error: error.message };
...
export async function getConversationContext(
  ...
  try {
    ...
    if (error) {
      logError("getConversationContext DB error", {
        conversationId,
        error: error.message,
      });
      return {
    ...
    for (let i = allMessages.length - 1; i >= 0; i--) {
      const msgTokens = Math.ceil(
        (allMessages[i].content?.length || 0) / CHARS_PER_TOKEN,
      );
    ...
    if (!summary && includedMessages.length < allMessages.length) {
      const excludedMessages = allMessages.slice(
        0,
        allMessages.length - includedMessages.length,
      );
    ...
    logError("getConversationContext error", {
      conversationId,
      error: error.message,
    });
    return { success: false, error: error.message };

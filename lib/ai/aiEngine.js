  ...
  try {
    ...
    if (!resolvedModel) {
      const routeResult = await routeModel({
        taskType,
        context,
        preferredProvider: configResult.data?.defaultProvider,
      });
      if (!routeResult.success) {
        logWarn("Model routing failed, falling back to default", {
          taskType,
    ...
    const maxRetries =
      configResult.data?.maxRetries ?? AI_CONFIG_DEFAULT.maxRetries;
    const timeoutMs =
      configResult.data?.timeoutMs ?? AI_CONFIG_DEFAULT.timeoutMs;
    ...
    logError("AI request failed after all retries", {
      taskType,
    ...
    logError("completeAIRequest unexpected error", {
      taskType,
      error: error.message,
    });
...
export async function getPrediction({
  ...
  try {
    if (!entityType || !entityId || !predictionType) {
      return {
        success: false,
        error: "entityType, entityId, and predictionType are required",
      };
    }
    ...
    logError("AIEngine", "getPrediction failed", {
      entityType,
      entityId,
      predictionType,
      error: err.message,
...
export async function scoreCampaign({ campaignId, scoredBy }) {
  try {
    ...
    logError("AIEngine", "scoreCampaign failed", {
      campaignId,
      error: err.message,
    });
    return { success: false, error: err.message };

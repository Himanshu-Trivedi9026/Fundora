...
export async function recordAICost({
  ...
  try {
    if (
      ...
      throw new Error(
        "userId, operation, provider, model, and costCents are required",
      );
    }
    ...
    const { data: existing, error: fetchError } = await supabaseAdmin
      .from("ai_usage")
      .select(
        "id, cost_cents, request_count, input_tokens, output_tokens, total_tokens",
      )
      .eq("user_id", userId)
      .eq("date", today)
    ...
    logError("CostTracker", "Failed to record cost", {
      userId,
      operation,
      model,
      error: err.message,
...
export async function getCostSummary({ userId, startDate, endDate }) {
  try {
    ...
    for (const key of Object.keys(byOperation)) {
      byOperation[key].costCents = parseFloat(
        byOperation[key].costCents.toFixed(4),
      );
    }
    ...
    logError("CostTracker", "Failed to get cost summary", {
      userId,
      error: err.message,
    });
    return { success: false, data: null, error: err.message };
...
export async function getPlatformAICosts({
  ...
  try {
    ...
    logError("CostTracker", "Failed to get platform costs", {
      startDate,
      endDate,
      error: err.message,
    });
...
export async function checkCostBudget({ entity, budgetType = "daily" }) {
  try {
    ...
    if (budgetRecord) {
      ...
      if (entity === "platform") {
        if (budgetType === "daily") {
          ...
          limitCents = parseFloat(
            process.env.AI_BUDGET_MONTHLY_CENTS || "200000",
          );
        }
      }
    ...
    const now = new Date();
    let spentQuery = supabaseAdmin.from("ai_usage").select("cost_cents");
    ...
    logError("CostTracker", "Failed to check budget", {
      entity,
      budgetType,
      error: err.message,
    });

...
export function calculateCost(model, inputTokens, outputTokens) {
  try {
    ...
    logError("TokenTracker", "Cost calculation failed", {
      model,
      error: err.message,
    });
    return 0;
...
export async function trackTokenUsage({
  ...
  try {
    ...
    const { data: existing, error: fetchError } = await supabaseAdmin
      .from("ai_usage")
      .select(
        "id, input_tokens, output_tokens, total_tokens, cost_cents, request_count",
      )
      .eq("user_id", userId)
      .eq("date", today)
    ...
    if (existing) {
      ...
      const newOutputTokens =
        (existing.output_tokens || 0) + (outputTokens || 0);
    ...
    logError("TokenTracker", "Failed to track usage", {
      userId,
      provider,
      model,
      error: err.message,
...
export async function getUserUsage({ userId, date }) {
  try {
    ...
    logError("TokenTracker", "Failed to get user usage", {
      userId,
      date,
      error: err.message,
    });
...
export async function getUsageStats({ startDate, endDate, groupBy = "date" }) {
  try {
    ...
    logError("TokenTracker", "Failed to get usage stats", {
      startDate,
      endDate,
      error: err.message,
    });
...
export async function checkUsageLimit({ userId, limit }) {
  try {
    ...
    logError("TokenTracker", "Failed to check usage limit", {
      userId,
      limit,
      error: err.message,
    });
...
export async function getUserAIUsage({ userId, timeframe }) {
  try {
    ...
    logError("TokenTracker", "Failed to get user AI usage", {
      userId,
      timeframe,
      error: err.message,
    });

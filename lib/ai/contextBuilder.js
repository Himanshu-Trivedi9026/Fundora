...
export async function buildCampaignContext(campaignId) {
  try {
    ...
    const { data: campaign, error: campaignError } = await supabaseAdmin
      .from("projects")
      .select("id, title, description, goal, pledged, category, creator_id, created_at, status")
      .eq("id", campaignId)
      .single();

    if (campaignError || !campaign) {
      logError("buildCampaignContext: campaign not found", { campaignId, error: campaignError?.message });
      return {
        success: true,
        data: {
          campaign: null,
    ...
    const [creatorResult, donationsResult, milestonesResult] = await Promise.allSettled([
    ...
    const avgDonation =
      donationCount > 0
        ? Math.round(donations.reduce((sum, d) => sum + (d.amount || 0), 0) / donationCount)
        : 0;
    ...
    logInfo("Campaign context built", { campaignId, donationCount, milestoneCompletion });
    ...
    logError("buildCampaignContext error", { campaignId, error: error.message });
...
export async function buildUserContext(userId) {
  try {
    ...
    const [profileResult, verificationResult, trustResult, reputationResult, statsResult] =
    ...
    const profile = profileData
      ? { id: profileData.id, email: profileData.email, created_at: profileData.created_at }
    ...
    // Process trust
    const trustData = trustResult.status === "fulfilled" ? trustResult.value?.data : null;
    ...
    const reputationData =
      reputationResult.status === "fulfilled" ? reputationResult.value?.data : null;
    ...
    const totalDonations =
      statsData?.[0]?.status === "fulfilled" ? (statsData[0].value?.data || []).length : 0;
    const campaignsCreated =
      statsData?.[1]?.status === "fulfilled" ? (statsData[1].value?.data || []).length : 0;
    const followerCount =
      statsData?.[2]?.status === "fulfilled" ? (statsData[2].value?.data || []).length : 0;

    const stats = { totalDonations, campaignsCreated, followerCount };

    logInfo("User context built", { userId, verificationLevel: verification.level });
...
export async function buildDonorContext(donorId) {
  try {
    ...
    const [profileResult, donationsResult, trustResult] = await Promise.allSettled([
      // Profile
      supabaseAdmin
        .from("auth.users")
        .select("id, email, created_at")
    ...
    const avgDonation = donationHistory.length > 0 ? Math.round(totalAmount / donationHistory.length) : 0;
    ...
    logInfo("Donor context built", { donorId, donationCount: donationHistory.length });
...
export async function buildPlatformContext() {
  try {
    ...
    const [usersResult, campaignsResult, donationsResult, activeResult, trendingResult, recentResult] =
      await Promise.allSettled([
        // Total users
        supabaseAdmin.from("auth.users").select("id", { count: "exact", head: true }),
    ...
    const totalCampaigns =
      campaignsResult.status === "fulfilled" ? (campaignsResult.value?.count ?? 0) : 0;
    const totalDonations =
      donationsResult.status === "fulfilled" ? (donationsResult.value?.count ?? 0) : 0;
    ...
    const stats = { totalUsers, totalCampaigns, totalDonations, activeCampaigns, recentActivity: recentActivity.length };

    logInfo("Platform context built", { totalUsers, totalCampaigns, activeCampaigns });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function buildConversationContext(conversationId, maxMessages = 20) {
  try {
    if (!conversationId) {
    ...
    if (messagesError) {
      logError("buildConversationContext DB error", { conversationId, error: messagesError.message });
      return { success: false, error: `Failed to load conversation: ${messagesError.message}` };
    }
    ...
    if (formattedMessages.length > 5) {
      const firstFew = formattedMessages.slice(0, 3).map((m) => `${m.role}: ${m.content?.substring(0, 100)}`).join("\n");
      summary = firstFew;
    }
    ...
    logInfo("Conversation context built", { conversationId, messageCount: formattedMessages.length });
    ...
    logError("buildConversationContext error", { conversationId, error: error.message });
    return { success: false, error: error.message };
  }
}

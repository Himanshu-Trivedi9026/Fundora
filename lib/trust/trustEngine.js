...
function scoreIdentity(verification) {
  ...
  if (verification.verified_at) {
    const daysSinceVerified =
      (Date.now() - new Date(verification.verified_at).getTime()) /
      (1000 * 60 * 60 * 24);
...
export async function calculateTrustScore(data = {}) {
  ...
  const confidence =
    totalWeight > 0 ? Math.round(confidenceSum / totalWeight) : 0;
...
export function getTrustLevel(score) {
  if (score >= 80)
    return {
      ...
      color: "success",
      description:
        "Exemplary creator with strong verification and track record",
    };
  if (score >= 60)
    return {
      label: "Moderate Trust",
      color: "primary",
      description: "Verified creator with positive platform history",
    };
  if (score >= 40)
    return {
      label: "Developing Trust",
      color: "warning",
      description: "Building verification and platform presence",
    };
  if (score >= 20)
    return {
      label: "Low Trust",
      color: "danger",
      description: "Limited verification and platform history",
    };
  return {
    label: "New Creator",
    color: "danger",
    description: "Just joined — verification recommended",
  };
}
...
export function calculateBusinessTrustBonus(businessData) {
  ...
  const multiplier =
    BUSINESS_TYPE_MULTIPLIERS[businessData.business_type] || 1.0;
...
export function calculateBankTrustBonus(bankData) {
  ...
  const pennyDropBonus =
    bankData.penny_drop_status === "success"
      ? VERIFICATION_WEIGHTS.penny_drop
      : 0;

...
export async function createOTP(userId, phone) {
  try {
    ...
    if (error) {
      logError("PhoneVerification", "Create OTP error", { error: error.message });
      return { success: false, error: "Failed to create OTP" };
    }

    logInfo("PhoneVerification", "OTP created", { userId: userId.substring(0, 8) + "..." });
...
export async function verifyOTP(userId, phone, otp) {
  try {
    ...
    if (fetchError) {
      logError("PhoneVerification", "Fetch OTP error", { error: fetchError.message });
      return { success: false, error: "Verification failed" };
    }

    if (!otpRecords || otpRecords.length === 0) {
      return { success: false, error: "No pending OTP found. Request a new one." };
    }
    ...
    const storedSalt = record.otp_salt || process.env.OTP_SALT || "fundora-otp-salt";
    ...
    logInfo("PhoneVerification", "OTP verified", { userId: userId.substring(0, 8) + "..." });

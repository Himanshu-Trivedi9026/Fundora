...
export function getProvider(name = "fundora_internal") {
  ...
  if (!provider) {
    logWarn("VerificationProvider", `Provider '${name}' not found, falling back to 'fundora_internal'`);
...
import { getOCRProvider, listOCRProviders, registerOCRProvider } from "./ocrProviderRegistry";

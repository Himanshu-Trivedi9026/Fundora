...
export async function createBankAccount(userId, accountData) {
  try {
    ...
    if (!accountData.accountHolderName || !accountData.accountNumber || !accountData.ifscCode) {
      return { success: false, error: "Account holder name, account number, and IFSC are required" };
    }
    ...
    const encryptedAccount = encryptMetadata({ account_number: accountData.accountNumber });
    ...
    const { data, error } = await supabaseAdmin
      ...
      .insert({
        ...
        account_number_encrypted: encryptedAccount?.ciphertext ? Buffer.from(encryptedAccount.ciphertext, "base64") : null,
...
export async function setPrimaryAccount(userId, accountId) {
  try {
    ...
    if (existing.status !== "verified") {
      return { success: false, error: "Only verified accounts can be set as primary" };
    }
    ...
    if (error) {
      logError("BankVerification", "Set primary error", { error: error.message });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function uploadBankDocument(userId, accountId, documentType, file, originalFilename) {
  try {
    ...
    const updateField = documentType === "cancelled_cheque" ? "cancelled_cheque_path" : "passbook_path";
    const { error } = await supabaseAdmin
      .from("bank_accounts")
    ...
    if (error) {
      logError("BankVerification", "Document update error", { error: error.message });
...
export async function getBankVerification(userId) {
  try {
    ...
    if (bkvError) {
      logError("BankVerification", "Get summary error", { error: bkvError.message });

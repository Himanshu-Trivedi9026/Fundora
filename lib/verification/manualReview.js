...
function sanitizeQueueItem(item) {
  ...
  if (safe.metadata && typeof safe.metadata === "object") {
    const { device_info, device_metadata, ip_address, provider_reference, ...safeMeta } = safe.metadata;
 ...
 */
export async function assignReviewer(requestId, reviewerId, priority = null, callerId = null) {
  try {
    if (!requestId || !reviewerId) {
      return { success: false, error: "Request ID and reviewer ID are required" };
    }

    // Authorization check
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized assign attempt", { callerId: callerId.substring(0, 8) + "..." });
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function approveRequest(requestId, reviewerId, notes = null, callerId = null) {
  try {
    if (!requestId || !reviewerId) {
      return { success: false, error: "Request ID and reviewer ID are required" };
    }
    ...
    // Authorization check
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized approve attempt", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    await updateCreatorVerification(request.user_id, request.verification_type, "approved");
    ...
    await sendNotification({
      ...
      actorId: reviewerId,
      entityId: requestId,
      data: { verificationType: request.verification_type, decision: "approved" },
    }).catch((err) => logError("ManualReview", "Approve notification failed", { error: err.message }));
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function rejectRequest(requestId, reviewerId, reason, callerId = null) {
  try {
    if (!requestId || !reviewerId || !reason) {
      return { success: false, error: "Request ID, reviewer ID, and reason are required" };
    }

    // Authorization check
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized reject attempt", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    await updateCreatorVerification(request.user_id, request.verification_type, "rejected");
    ...
    await sendNotification({
      ...
      actorId: reviewerId,
      entityId: requestId,
      data: { verificationType: request.verification_type, decision: "rejected", reason: sanitizedReason },
    }).catch((err) => logError("ManualReview", "Reject notification failed", { error: err.message }));
...
export async function getReviewQueue({
  ...
  try {
    // Authorization check
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized queue access", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    if (status) {
      ...
      const statuses = status === "pending" ? ["submitted", "pending"] : [status];
...
async function enrichQueueItems(items) {
  ...
  try {
    const { data: documents } = await supabaseAdmin
      ...
      .select("id, user_id, document_type, document_name, mime_type, status, uploaded_at, verified_at")
      .in("user_id", userIds)
  ...
  try {
    const { data: history } = await supabaseAdmin
      ...
      .select("id, action, new_status, new_level, created_at, reason, performed_by")
      .in("user_id", userIds)
  ...
  try {
    const { data: verifications } = await supabaseAdmin
      ...
      .select("user_id, verification_status, verification_level, identity_verified, verified_at")
      .in("user_id", userIds);
...
export async function getRequestDetails(requestId, callerId = null) {
  try {
    ...
    // Authorization check
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized details access", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function updateReviewPriority(requestId, priority, callerId = null) {
  try {
    ...
    // Authorization check
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized priority update", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }

    const validPriorities = ["low", "normal", "high", "urgent"];
    if (!validPriorities.includes(priority)) {
      return { success: false, error: `Invalid priority. Must be: ${validPriorities.join(", ")}` };
    }
    ...
    if (error) {
      logError("ManualReview", "Priority update error", { error: error.message });
...
async function updateCreatorVerification(userId, verificationType, decision) {
  ...
  updateData.verification_status = statusMap[decision] || updateData.verification_status;
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function approveBusinessVerification(verificationId, reviewerId, notes = null, callerId = null) {
  try {
    if (!verificationId || !reviewerId) {
      return { success: false, error: "Verification ID and reviewer ID are required" };
    }

    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized business approve", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    if (error) {
      logError("ManualReview", "Business approve error", { error: error.message });
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function rejectBusinessVerification(verificationId, reviewerId, reason, callerId = null) {
  try {
    if (!verificationId || !reviewerId || !reason) {
      return { success: false, error: "Verification ID, reviewer ID, and reason are required" };
    }

    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized business reject", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    if (error) {
      logError("ManualReview", "Business reject error", { error: error.message });
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function approveBankVerification(verificationId, reviewerId, notes = null, callerId = null) {
  try {
    if (!verificationId || !reviewerId) {
      return { success: false, error: "Verification ID and reviewer ID are required" };
    }
    ...
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized bank approve", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function rejectBankVerification(verificationId, reviewerId, reason, callerId = null) {
  try {
    if (!verificationId || !reviewerId || !reason) {
      return { success: false, error: "Verification ID, reviewer ID, and reason are required" };
    }
    ...
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized bank reject", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function requestResubmission(verificationId, reviewerId, reason, callerId = null) {
  try {
    if (!verificationId || !reviewerId || !reason) {
      return { success: false, error: "Verification ID, reviewer ID, and reason are required" };
    }
    ...
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized resubmission request", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    if (identityRecord) {
      ...
      await logAuditEvent({
        ...
        action: "resubmission_requested",
        details: { reviewerId, reason: sanitizedReason, verificationType: identityRecord.verification_type },
      });

      // Sync the creator's overall status back to pending for resubmission
      await updateCreatorVerification(identityRecord.user_id, identityRecord.verification_type, "resubmitted");

      // Notify the creator
      await sendNotification({
        userId: identityRecord.user_id,
        notificationType: NOTIFICATION_TYPES.VERIFICATION_RESUBMISSION_REQUESTED,
        actorId: reviewerId,
        entityId: verificationId,
        data: { verificationType: identityRecord.verification_type, reason: sanitizedReason },
      }).catch((err) => logError("ManualReview", "Resubmission notification failed", { error: err.message }));
 ...
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function suspendVerification(verificationId, reviewerId, reason = null, callerId = null) {
  try {
    if (!verificationId || !reviewerId) {
      return { success: false, error: "Verification ID and reviewer ID are required" };
    }
    ...
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized suspend attempt", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    await updateCreatorVerification(request.user_id, request.verification_type, "suspended");
    ...
    await logAuditEvent({
      ...
      details: { reviewerId, reason: sanitizedReason, verificationType: request.verification_type },
    });
    ...
    await sendNotification({
      ...
      actorId: reviewerId,
      entityId: verificationId,
      data: { verificationType: request.verification_type, reason: sanitizedReason },
    }).catch((err) => logError("ManualReview", "Suspend notification failed", { error: err.message }));
...
export async function getBusinessReviewQueue({
  ...
  try {
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized business queue access", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }
    ...
    if (error) {
      logError("ManualReview", "Business queue error", { error: error.message });
...
export async function getBankReviewQueue({
  ...
  try {
    if (callerId && !requireAdminRole(callerId)) {
      logError("ManualReview", "Unauthorized bank queue access", { callerId: callerId.substring(0, 8) + "..." });
      return { success: false, error: "Unauthorized: admin role required" };
    }

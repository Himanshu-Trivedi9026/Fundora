...
export function validateDocumentExtension(filename, documentType) {
  ...
  const req = DOCUMENT_REQUIREMENTS[documentType] || DOCUMENT_REQUIREMENTS[DOCUMENT_TYPES.OTHER];
...
export function validateImageDimensions(width, height, documentType) {
  ...
  if (width < req.minImageDimensions.width || height < req.minImageDimensions.height) {
    return {
      valid: false,
...
export function validateDocument({
  ...
  if (mimeType && mimeType.startsWith("image/") && imageWidth && imageHeight) {
    const dimResult = validateImageDimensions(imageWidth, imageHeight, documentType);
    if (!dimResult.valid) {

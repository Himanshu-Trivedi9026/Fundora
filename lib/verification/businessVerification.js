...
export async function createBusinessVerification(
  ...
  try {
    ...
    const updateData = {
      ...
      metadata_encrypted: encryptedMetadata?.ciphertext
        ? Buffer.from(encryptedMetadata.ciphertext, "base64")
        : null,
      metadata_hash: encryptedMetadata
    ...
    if (existing) {
      ...
      if (error) {
        logError("BusinessVerification", "Update error", {
          error: error.message,
        });
        return {
          success: false,
      ...
      if (error) {
        logError("BusinessVerification", "Create error", {
          error: error.message,
        });
        return {
          success: false,
    ...
    logError("BusinessVerification", "Create/update error", {
      error: err.message,
    });
...
export async function uploadBusinessDocument(
  ...
  try {
    ...
    if (error) {
      logError("BusinessVerification", "Document record error", {
        error: error.message,
      });
...
export async function getBusinessVerification(userId) {
  try {
    ...
    const completion = checkDocumentCompletion(
      providedTypes,
      data.business_type,
    );

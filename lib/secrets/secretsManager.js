...
export async function getSecret(key, options = {}) {
  ...
  try {
    ...
    secureLogger.error("Secret retrieval failed", { key, provider, error: err.message });
    return null;
  }
}
  ...
  try {
    ...
    return {
      ...
      data: (data || []).map((s) => ({
        ...
        daysRemaining: Math.ceil((new Date(s.expires_at) - new Date()) / 86400000),
      })),
    };
  } catch (err) {
    return { success: false, error: err.message };
...
export async function validateCredentials(provider, credentials) {
  switch (provider) {
    ...
    case "openai":
      return validateApiKey(credentials.apiKey, "https://api.openai.com/v1/models");
    case "stripe":
      return validateApiKey(credentials.apiKey, "https://api.stripe.com/v1/charges");
    default:
...
async function validateApiKey(apiKey, testUrl) {
  ...
  return { success: true, data: { valid: true, keyPrefix: apiKey.substring(0, 8) + "..." } };
}
...
export async function generateSecurityAudit(options = {}) {
  ...
  const warnings = audit.checks.filter((c) => c.severity === "warning" || c.status === "warning").length;
  const passed = audit.checks.filter((c) => c.status === "present" || c.status === "passed").length;

...
export async function createDeveloperApp({
  ...
  try {
    ...
    if (error) {
      logError("DeveloperApp", "createDeveloperApp insert error", { error: error.message });
    ...
    logInfo("DeveloperApp", "Developer app created", { appId: data.id, name, userId });
    ...
    logError("DeveloperApp", "createDeveloperApp unexpected error", { error: err.message });
...
export async function validateDeveloperApp(clientId, clientSecret) {
  try {
    ...
    logError("DeveloperApp", "validateDeveloperApp unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}
...
export async function revokeDeveloperApp(appId, userId) {
  try {
    ...
    if (error || !data) {
      return { success: false, error: error?.message || "App not found or not owned by user" };
    }
    ...
    logError("DeveloperApp", "revokeDeveloperApp unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}
 ...
 */
export async function listDeveloperApps({ userId, organizationId, limit = 50, offset = 0 } = {}) {
  try {
    ...
    logError("DeveloperApp", "listDeveloperApps unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}
...
export async function getDeveloperApp(clientId) {
  try {
    ...
    logError("DeveloperApp", "getDeveloperApp unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}

...
export async function logApiRequest({
  ...
  try {
    ...
    if (error) {
      logError("APILog", "logApiRequest insert error", { error: error.message });
    }
  } catch (err) {
    logError("APILog", "logApiRequest unexpected error", { error: err.message });
  }
}
 ...
 */
export async function getApiUsageSummary({ apiKeyId, organizationId, startDate, endDate, limit = 30 } = {}) {
  try {
    let query = supabaseAdmin
      .from("api_logs")
    ...
    logError("APILog", "getApiUsageSummary unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}

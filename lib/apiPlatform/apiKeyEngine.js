...
export async function validateApiKey(keyHash) {
  try {
    ...
    if (error) {
      logError("APIKey", "validateApiKey query error", {
        error: error.message,
      });
      return { success: false, error: error.message };
    }
    ...
    logError("APIKey", "validateApiKey unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }
...
export async function revokeApiKey(keyId, userId) {
  try {
    ...
    if (error || !data) {
      return {
        success: false,
        error: error?.message || "API key not found or not owned by user",
      };
    }
...
export async function getApiKeyUsage(
  ...
  try {
    ...
    logError("APIKey", "getApiKeyUsage unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }

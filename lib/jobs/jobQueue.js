...
export async function processQueue(queueName = "default", options = {}) {
  ...
  if (!rateCheck.success) return { success: true, data: { processed: 0, rateLimited: true } };
  ...
  try {
    ...
    if (error) return { success: false, error: error.message };
    if (!jobs || jobs.length === 0) return { success: true, data: { processed: 0 } };
...
export async function executeJob(job) {
  ...
  if (!handler) {
    await handleJobFailure(job, new Error(`No handler registered for job type: ${job.job_type}`));
    return;
  }
...
async function handleJobFailure(job, error) {
  ...
  if (retryCount >= maxRetries) {
    ...
    await logAuditEvent({
      ...
      targetId: job.id,
      metadata: { jobType: job.job_type, error: error.message, retries: retryCount },
    });
  } else {
    // Retry with backoff
...
export async function requeueDeadLetters(queueName, options = {}) {
  try {
    ...
    if (error) return { success: false, error: error.message };
    if (!jobs || jobs.length === 0) return { success: true, data: { requeued: 0 } };
...
export function registerHandler(jobType, handler) {
  if (_handlers.has(jobType)) {
    return { success: false, error: `Handler already registered for ${jobType}` };
  }
  ...
  try {
    const { data, error } = await supabaseAdmin
      ...
      .insert({
        ...
        next_run_at: options.nextRunAt ? new Date(options.nextRunAt).toISOString() : new Date().toISOString(),
...
export async function processScheduledJobs() {
  try {
    ...
    for (const schedule of schedules || []) {
      try {
        const enqueueResult = await enqueue(schedule.job_type, schedule.payload, {
...
export async function listSchedules(options = {}) {
  try {
    let query = supabaseAdmin.from("scheduled_jobs").select("*", { count: "exact" });

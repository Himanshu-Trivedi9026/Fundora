  ...
  try {
    ...
    for (let attempt = 0; attempt <= AI_CONFIG.maxRetries; attempt++) {
      try {
        ...
        logWarn("AIRiskAnalyzer", `AI call attempt ${attempt + 1} failed`, {
          error: err.message,
        });
...
export async function explainDecision({
  ...
  try {
    ...
    logError("AIRiskAnalyzer", "Explain decision error", {
      error: err.message,
    });
    return ruleBasedExplanation({ decision, riskScore, signals, factors });
  }
...
export async function detectAnomalies({ userId, behaviorData }) {
  ...
  try {
    ...
    logError("AIRiskAnalyzer", "Anomaly detection error", {
      error: err.message,
    });
...
function ruleBasedExplanation({ decision, riskScore, signals, factors }) {
  ...
  return {
    explanation:
      ...
      `Decision: ${decision}. Risk score: ${riskScore}.`,
    keyFactors: factors,
    source: "rule_engine",
  };
}
...
export async function analyzeFraud({ userId, signals, timeframe, analyzedBy }) {
  try {
    ...
    if (Array.isArray(signals)) {
      for (const signal of signals) {
        if (typeof signal === "object" && signal.type) {
          formattedSignals[signal.type] =
            signal.value !== undefined ? signal.value : true;
        }
      }
    ...
    logError("AIRiskAnalyzer", "analyzeFraud failed", {
      userId,
      error: err.message,
    });

...
export async function recordFingerprint({ userId, fingerprint }) {
  try {
    ...
    const fingerprintHash = fingerprint.hash || computeFingerprintHash(fingerprint);
 ...
 */
export async function getDeviceFingerprints(userId, { limit = 50, offset = 0 } = {}) {
  try {
    if (!userId) {
...
export async function getDeviceStats(userId) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("device_fingerprints")
      .select("browser, platform, is_known, first_seen_at, last_seen_at, session_count, risk_flags")
      .eq("user_id", userId);

    if (error) {
      logError("DeviceFingerprint", "Stats query error", { error: error.message });
    ...
    devices.forEach((d) => {
      browsers[d.browser || "unknown"] = (browsers[d.browser || "unknown"] || 0) + 1;
      platforms[d.platform || "unknown"] = (platforms[d.platform || "unknown"] || 0) + 1;
...
export async function getHighRiskDevices({ limit = 50, offset = 0 } = {}) {
  try {
    ...
    if (error) {
      logError("DeviceFingerprint", "High-risk query error", { error: error.message });
    ...
    logError("DeviceFingerprint", "High-risk query error", { error: err.message });
    return { success: false, error: "Failed to fetch high-risk devices" };
  }
}

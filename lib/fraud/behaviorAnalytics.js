...
export async function recordBehaviorEvent({
  ...
  try {
    ...
    if (!validCategories.includes(eventCategory)) {
      return {
        success: false,
        error: `Invalid event category. Must be: ${validCategories.join(", ")}`,
      };
    }

    // Calculate initial risk score for the event
    const riskScore = calculateEventRiskScore(
      eventType,
      eventCategory,
      eventData,
    );
...
export async function getBehaviorEvents(
  userId,
  { limit = 50, offset = 0, category, startDate, endDate } = {},
) {
  try {
    if (!userId) {
...
export async function getBehaviorSummary(userId, days = 30) {
  try {
    ...
    if (error) {
      logError("BehaviorAnalytics", "Summary query error", {
        error: error.message,
      });
    ...
    events.forEach((event) => {
      summary.byCategory[event.event_category] =
        (summary.byCategory[event.event_category] || 0) + 1;
      summary.byType[event.event_type] =
        (summary.byType[event.event_type] || 0) + 1;
      totalRisk += event.risk_score || 0;
      if ((event.risk_score || 0) > 50) summary.highRiskEvents++;
    });

    summary.avgRiskScore = events.length
...
export async function detectBehaviorAnomalies(userId) {
  try {
    ...
    logError("BehaviorAnalytics", "Anomaly detection error", {
      error: err.message,
    });
...
export async function getLoginPatterns(userId, days = 30) {
  try {
    ...
    if (error) {
      logError("BehaviorAnalytics", "Login patterns query error", {
        error: error.message,
      });
    ...
    return {
      ...
      patterns: {
        ...
        avgLoginsPerDay: uniqueDays.size
          ? Math.round(logins.length / uniqueDays.size)
          : 0,
        hourlyDistribution,
        dailyDistribution,
        peakHour,
    ...
    logError("BehaviorAnalytics", "Login patterns error", {
      error: err.message,
    });
    return { success: false, error: "Failed to fetch login patterns" };
  }

 ...
 */
export async function detectBehaviorAnomalies({ userId, behaviorData, historicalPatterns }) {
  try {
    if (!userId) {
    ...
    const velocityZScore = calculateZScore(actionsPerMinute, historicalAvg, historicalStdDev);
    if (velocityZScore > 2.5) {
      anomalies.push({
        type: ANOMALY_TYPES.VELOCITY_SPIKE,
        severity: velocityZScore > 4 ? SEVERITY_LEVELS.HIGH : SEVERITY_LEVELS.MEDIUM,
    ...
    if (behaviorData.deviceChanged && historicalPatterns?.knownDevices?.length > 0) {
      anomalies.push({
        type: ANOMALY_TYPES.DEVICE_ANOMALY,
        severity: SEVERITY_LEVELS.LOW,
        description: "Activity from a new device not previously associated with this account",
        confidence: 0.4,
      });
      overallRisk += 8;
    }
    ...
    logInfo("FraudAIEnhancer", "Behavior anomalies detected", { userId, anomalyCount: anomalies.length, overallRisk });
    ...
    logError("FraudAIEnhancer", "Detect behavior anomalies error", { error: err.message, userId });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function detectDonationAnomalies({ userId, recentDonations, donorHistory }) {
  try {
    if (!userId) {
      return { success: false, error: "userId is required" };
    }
    ...
    const recentHour = recentDonations.filter((d) => new Date(d.created_at).getTime() > oneHourAgo);
    ...
    for (const amount of recentAmounts) {
      if (historicalStdDev > 0 && historicalMean > 0) {
        const zScore = calculateZScore(amount, historicalMean, historicalStdDev);
        if (zScore > 3) {
          amountAnomaly = true;
          anomalies.push(
    ...
    logInfo("FraudAIEnhancer", "Donation anomalies detected", { userId, anomalyCount: anomalies.length });
    ...
    logError("FraudAIEnhancer", "Detect donation anomalies error", { error: err.message, userId });
...
export async function analyzeNetworkRelationships({ userId, connections }) {
  try {
    ...
    const sharedDeviceUsers = Object.entries(connectionMap)
      .filter(([, data]) => data.types.has("shared_device") || data.types.has("shared_ip"))
      .filter(([, data]) => data.types.size >= 2)
      .map(([id, data]) => ({ id, sharedCount: data.types.size }));

    if (sharedDeviceUsers.length >= 1) {
      const sybilCandidates = sharedDeviceUsers.filter((u) => u.sharedCount >= 3);
    ...
    const highTransferUsers = Object.entries(connectionMap)
      .filter(([, data]) => data.types.has("fund_transfer") && data.strength > 5)
      .map(([id]) => id);
    ...
    logInfo("FraudAIEnhancer", "Network relationships analyzed", { userId, relationshipCount: relationships.length, riskLevel });
    ...
    logError("FraudAIEnhancer", "Analyze network relationships error", { error: err.message, userId });
  ...
  try {
    ...
    if (signals.identityInconsistencies?.length > 0) {
      patterns.push({
        ...
        confidence: Math.min(1, 0.4 + signals.identityInconsistencies.length * 0.15),
        evidence: signals.identityInconsistencies.map((i) => i.description || String(i)),
      });
    }
    ...
    if (signals.velocityScore && signals.velocityScore > 70) {
      patterns.push({
        ...
        evidence: [
          ...
          signals.velocityDetails || "Multiple rapid actions in short timeframe",
        ],
      });
    }

    // Account farming pattern
    if (signals.accountAge && signals.accountAge < 24 * 60 * 60 * 1000 && signals.donationCount > 10) {
      patterns.push({
        type: "account_farming",
        description: "New account with unusually high activity — possible account farming",
    ...
    if (signals.uniqueDevices > 5 || signals.uniqueIps > 10) {
      patterns.push({
        ...
        confidence: Math.min(1, 0.3 + (signals.uniqueDevices || 0) * 0.05 + (signals.uniqueIps || 0) * 0.03),
        evidence: [
    ...
    if (signals.previousFlags && signals.previousFlags > 2) {
      patterns.push({
        ...
        evidence: signals.flagDetails || [`Previously flagged ${signals.previousFlags} time(s)`],
      });
    }

    // Generate recommendation
    let recommendation = "No action recommended — signals within normal parameters";
    const maxConfidence = patterns.reduce((max, p) => Math.max(max, p.confidence), 0);

    if (maxConfidence >= 0.8) {
      recommendation = "Immediate review required — high-confidence fraud indicators detected";
    } else if (maxConfidence >= 0.6) {
      recommendation = "Flag for priority review — moderate-to-high fraud indicators detected";
    } else if (maxConfidence >= 0.4) {
      recommendation = "Monitor closely — elevated fraud indicators detected";
    } else if (patterns.length > 0) {
      recommendation = "Low priority — minor anomalies detected, no immediate action needed";
    }

    logInfo("FraudAIEnhancer", "Fraud patterns detected", { userId, patternCount: patterns.length });
    ...
    logError("FraudAIEnhancer", "Detect fraud patterns error", { error: err.message, userId });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function explainRiskAssessment({ userId, riskScore, signals, decision }) {
  try {
    if (!userId) {
      return { success: false, error: "userId is required" };
    }
    ...
    if (decision === "block") {
      explanation += "Due to the high risk level, this account has been blocked from performing the requested action.";
    } else if (decision === "flag") {
      explanation += "This account has been flagged for manual review by our security team.";
    } else {
      explanation += "The risk indicators were within acceptable thresholds, so the action was allowed.";
    }

    // Identify key factors
    const keyFactors = [];
    if (signals.velocityScore > 60) keyFactors.push("Unusually high activity velocity");
    if (signals.identityInconsistencies?.length > 0) keyFactors.push(`${signals.identityInconsistencies.length} identity inconsistency(ies) detected`);
    if (signals.deviceAnomalies > 0) keyFactors.push(`${signals.deviceAnomalies} device anomaly(ies)`);
    if (signals.ipAnomalies > 0) keyFactors.push(`${signals.ipAnomalies} IP anomaly(ies)`);
    if (signals.previousFlags > 0) keyFactors.push(`${signals.previousFlags} previous fraud flag(s) on this account`);
    if (signals.networkRisk === "high") keyFactors.push("Connected to suspicious network of accounts");
    ...
    if (riskLevel === "high" || riskLevel === "critical") {
      suggestedActions.push("Escalate to senior security team for manual review");
      ...
      suggestedActions.push("Request identity verification if not already completed");
    ...
    logInfo("FraudAIEnhancer", "Risk assessment explained", { userId, riskScore, riskLevel, decision });
    ...
    logError("FraudAIEnhancer", "Explain risk assessment error", { error: err.message, userId });
...
export async function generateFraudSummary({ timeframe = "7d", filters = {} }) {
  try {
    ...
    if (fetchError) {
      logError("FraudAIEnhancer", "Fraud summary fetch error", { error: fetchError.message });
    ...
    const highSeverity = (cases || []).filter((c) => c.severity === "high" || c.severity === "critical").length;
    ...
    const trendDirection = totalCases > previousCount ? "increasing" : totalCases < previousCount ? "decreasing" : "stable";
    ...
    const trends = [
      ...
      openCases > 0 ? `${openCases} case(s) still open and need resolution` : "All cases have been addressed",
    ];

    // Recommendations
    const recommendations = [];
    if (highSeverity > 0) {
      recommendations.push("Prioritize resolution of high-severity fraud cases");
    }
    if (openCases > resolvedCases && totalCases > 5) {
      recommendations.push("Consider adding additional moderation resources — case volume is elevated");
    }
    if (trendDirection === "increasing") {
      recommendations.push("Review recent platform changes that may be contributing to increased fraud");
    }
    if (topPatterns.length > 0) {
      recommendations.push(`Most common fraud type: ${topPatterns[0].split(":")[0]} — consider targeted prevention measures`);
    }
    ...
    logInfo("FraudAIEnhancer", "Fraud summary generated", { timeframe, totalCases });
    ...
    logError("FraudAIEnhancer", "Generate fraud summary error", { error: err.message, timeframe });
 ...
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
export async function getFraudRecommendations({ userId, riskLevel, anomalies }) {
  try {
    if (!userId) {
      return { success: false, error: "userId is required" };
    }
    ...
    switch (riskLevel) {
      case "high":
        recommendations.push({
          ...
          reason: "High-risk accounts should be limited to prevent further potential damage",
          priority: "high",
        });
        recommendations.push({
          action: "Escalate to senior security team",
          reason: "High-risk cases require senior review for complex investigation",
          priority: "high",
        });
        recommendations.push({
          action: "Request enhanced identity verification",
          reason: "Additional verification can help confirm or clear the account",
          priority: "high",
        });
      ...
      case "medium":
        recommendations.push({
          ...
          reason: "Medium-risk accounts need timely review but don't require immediate suspension",
          priority: "medium",
        });
        recommendations.push({
      ...
      default:
        recommendations.push({
          ...
          reason: "Low-risk anomalies should be monitored for potential escalation",
          priority: "low",
        });
        recommendations.push({
        ...
        if (anomalies && anomalies.length > 0) {
          recommendations.push({
            ...
            reason: "Even low-risk anomalies contribute to long-term pattern detection",
            priority: "low",
          });
        }
        break;
    ...
    logInfo("FraudAIEnhancer", "Fraud recommendations generated", { userId, riskLevel, recommendationCount: recommendations.length });
    ...
    logError("FraudAIEnhancer", "Get fraud recommendations error", { error: err.message, userId });
    return { success: false, error: "Failed to generate fraud recommendations" };
  }
}

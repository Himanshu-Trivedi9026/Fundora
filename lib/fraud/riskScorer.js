 ...
 */
export function calculateRiskScore({ signals, ruleResults, trustScore, verificationLevel, trigger }) {
  try {
    // 1. Signal-based risk
    ...
    const breakdown = {
      signalRisk: { score: signalRisk.score, weight: RISK_WEIGHTS.signals, factors: signalRisk.factors },
      ruleRisk: { score: ruleRisk.score, weight: RISK_WEIGHTS.rules, factors: ruleRisk.factors },
      trustRisk: { score: trustRisk.score, weight: RISK_WEIGHTS.trustInversion, factors: trustRisk.factors },
      verificationRisk: { score: verificationRisk.score, weight: RISK_WEIGHTS.verificationGap, factors: verificationRisk.factors },
      finalScore,
      level,
      trigger,
    };
...
function calculateRuleRisk(ruleResults) {
  ...
  if (!ruleResults || !ruleResults.triggered || ruleResults.triggered.length === 0) {
  ...
  switch (level) {
    case "critical":
      return {
        label: "Critical Risk",
        color: "danger",
        description: "Immediate action required — high probability of fraudulent activity",
      };
    case "high":
      return {
        label: "High Risk",
    ...
    case "medium":
      return {
        label: "Medium Risk",
        color: "primary",
        description: "Moderate risk — monitoring and additional verification recommended",
      };
    case "low":
    default:
      return {

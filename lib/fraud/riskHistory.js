 ...
 */
export async function getRiskHistory(userId, { limit = 30, offset = 0, startDate, endDate } = {}) {
  try {
    if (!userId) {
...
export async function getRiskTrend(userId, days = 30) {
  try {
    ...
    const trend = Object.values(dailyData).map((day) => ({
      date: day.date,
      avgRiskScore: Math.round(day.scores.reduce((a, b) => a + b, 0) / day.scores.length),
...
export async function getAggregateStats({ days = 30 } = {}) {
  try {
    ...
    const stats = {
      ...
      avgRiskScore: scores.length ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 0,

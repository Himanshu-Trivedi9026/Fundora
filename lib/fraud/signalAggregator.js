...
async function getDeviceSignals(userId) {
  try {
    ...
    if (error || !devices || devices.length === 0) {
      return { knownDevice: true, deviceCount24h: 0, newDevice: false, riskFlags: [] };
    }
...
function calculateSignalsSummary(signals) {
  ...
  if (signals.recentVerificationAttempts > 3) negatives.push("verification_spam");
...
function getEmptySignals(trigger) {
  return {
    ...
    trigger,
    evaluatedAt: new Date().toISOString(),
    summary: { positiveSignals: [], negativeSignals: [], signalCount: 0, riskIndicator: "normal" },
  };
}

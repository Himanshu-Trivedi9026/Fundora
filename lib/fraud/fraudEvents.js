...
export async function getFraudEventSummary(userId, days = 30) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("fraud_events")
      .select(
        "event_type, event_category, severity, risk_contribution, created_at",
      )
      .eq("user_id", userId)
    ...
    events.forEach((event) => {
      ...
      summary.topEventTypes[event.event_type] =
        (summary.topEventTypes[event.event_type] || 0) + 1;
    });
 ...
 * @returns {Promise<{success: boolean, events?: Object[], total?: number, error?: string}>}
 */
export async function getAllFraudEvents({
  limit = 50,
  offset = 0,
  category,
  severity,

 ...
 */
export async function evaluateUser({ userId, trigger = "unknown", context = {}, ipAddress, userAgent }) {
  try {
    if (!userId) {
    ...
    try {
      ...
      if (aiResult.success && aiResult.data) {
        finalRiskScore = Math.max(finalRiskScore, aiResult.data.overallRisk * 0.3);
      }
    } catch {
...
export async function applyManualOverride({
  ...
  try {
    ...
    const validTypes = ["decision", "risk_score", "block", "unblock", "whitelist", "blacklist"];
    if (!validTypes.includes(overrideType)) {
      return { success: false, error: `Invalid override type. Must be: ${validTypes.join(", ")}` };
    }
    ...
    const { error } = await supabaseAdmin.from("manual_overrides").insert({
      ...
      previous_value: overrideType === "decision" ? profile?.decision : String(profile?.risk_score || 0),
      new_value: newValue,
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      if (updateError) {
        logError("RiskEngine", "Profile override error", { error: updateError.message });
      }
    }
 ...
 */
export async function getFraudDashboard({ limit = 50, offset = 0, riskLevel, decision } = {}) {
  try {
    ...
    (stats || []).forEach((row) => {
      summary.byRiskLevel[row.risk_level] = (summary.byRiskLevel[row.risk_level] || 0) + 1;
      summary.byDecision[row.decision] = (summary.byDecision[row.decision] || 0) + 1;
    });

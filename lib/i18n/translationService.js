...
export async function loadTranslations(locale, namespace = "default", force = false) {
  const cacheKey = `${locale}:${namespace}`;
  const now = Date.now();

  if (!force && _translationsCache.has(cacheKey) && (now - _lastCacheRefresh) < _cacheTTL) {
...
export function formatNumber(number, locale = _activeLocale, options = {}) {
  try {
    return new Intl.NumberFormat(locale.replace("_", "-"), options).format(number);
  } catch {
...
export function formatDate(date, locale = _activeLocale, options = {}) {
  try {
    return new Intl.DateTimeFormat(locale.replace("_", "-"), options).format(new Date(date));
  } catch {
    return String(date);
  }
}

export function formatCurrency(amount, currency = "INR", locale = _activeLocale) {
  try {
...
export function formatRelativeTime(date, locale = _activeLocale) {
  ...
  if (days > 30) return formatDate(date, locale, { month: "short", day: "numeric", year: "numeric" });

  ...
  try {
    if (
      ...
      return {
        success: false,
        error:
          "appealType, appellantId, originalAction, originalActionId, originalActionType, and reason are required",
      };
    }
    ...
    if (error) {
      logError("AppealsEngine", "Failed to create appeal", {
        error: error.message,
        appellantId,
      });
    ...
    logInfo("AppealsEngine", "Appeal created", {
      appealId: data.id,
      appealNumber,
      appellantId,
    });
    ...
    logError("AppealsEngine", "Error creating appeal", {
      error: error.message,
    });
    return { success: false, error: "Failed to create appeal" };
  }
...
export async function getAppeal(appealId) {
  try {
    ...
    if (error) {
      logError("AppealsEngine", "Failed to fetch appeal", {
        error: error.message,
        appealId,
      });
    ...
    logError("AppealsEngine", "Error fetching appeal", {
      error: error.message,
    });
    return { success: false, error: "Failed to fetch appeal" };
  }
...
export async function getAppealByNumber(appealNumber) {
  try {
    ...
    if (error) {
      logError("AppealsEngine", "Failed to fetch appeal by number", {
        error: error.message,
        appealNumber,
      });
    ...
    logError("AppealsEngine", "Error fetching appeal by number", {
      error: error.message,
    });
    return { success: false, error: "Failed to fetch appeal" };
  }
...
export async function getAppeals({
  ...
  try {
    let query = supabaseAdmin.from("appeals").select("*", { count: "exact" });
    ...
    if (error) {
      logError("AppealsEngine", "Failed to list appeals", {
        error: error.message,
      });
    ...
    logError("AppealsEngine", "Error listing appeals", {
      error: error.message,
    });
    return { success: false, error: "Failed to list appeals" };
  }
...
export async function assignAppealReviewer(appealId, reviewerId, assignedBy) {
  try {
    if (!appealId || !reviewerId || !assignedBy) {
      return {
        success: false,
        error: "appealId, reviewerId, and assignedBy are required",
      };
    }
    ...
    if (
      existing.status === APPEAL_STATUSES.CLOSED ||
      existing.status === APPEAL_STATUSES.WITHDRAWN
    ) {
      return {
        success: false,
        error: `Cannot assign reviewer to appeal in '${existing.status}' status`,
      };
    }
    ...
    if (error) {
      logError("AppealsEngine", "Failed to assign reviewer", {
        error: error.message,
        appealId,
      });
    ...
    logInfo("AppealsEngine", "Reviewer assigned", {
      appealId,
      reviewerId,
      assignedBy,
    });
    ...
    logError("AppealsEngine", "Error assigning reviewer", {
      error: error.message,
    });
    return { success: false, error: "Failed to assign reviewer" };
  }
...
export async function requestEvidence(appealId, reason, performedBy) {
  try {
    if (!appealId || !reason || !performedBy) {
      return {
        success: false,
        error: "appealId, reason, and performedBy are required",
      };
    }
    ...
    if (
      ...
      return {
        success: false,
        error: `Cannot request evidence for appeal in '${existing.status}' status`,
      };
    }
    ...
    if (error) {
      logError("AppealsEngine", "Failed to request evidence", {
        error: error.message,
        appealId,
      });
    ...
    logError("AppealsEngine", "Error requesting evidence", {
      error: error.message,
    });
    return { success: false, error: "Failed to request evidence" };
  }
  ...
  try {
    if (!appealId || !reviewerDecision || !decisionReason || !performedBy) {
      return {
        success: false,
        error:
          "appealId, reviewerDecision, decisionReason, and performedBy are required",
      };
    }
    ...
    if (
      ...
      return {
        success: false,
        error: `Cannot review appeal in '${existing.status}' status`,
      };
    }
    ...
    if (error) {
      logError("AppealsEngine", "Failed to review appeal", {
        error: error.message,
        appealId,
      });
    ...
    logInfo("AppealsEngine", "Appeal reviewed", {
      appealId,
    ...
    logError("AppealsEngine", "Error reviewing appeal", {
      error: error.message,
    });
    return { success: false, error: "Failed to review appeal" };
  }
...
export async function withdrawAppeal(appealId, performedBy) {
  try {
    ...
    if (existing.appellant_id !== performedBy) {
      logWarn("AppealsEngine", "Unauthorized withdrawal attempt", {
        appealId,
        performedBy,
        appellantId: existing.appellant_id,
      });
      return {
        success: false,
        error: "You can only withdraw your own appeals",
      };
    }
    ...
    if (
      ...
      return {
        success: false,
        error: `Cannot withdraw appeal in '${existing.status}' status`,
      };
    }
    ...
    if (error) {
      logError("AppealsEngine", "Failed to withdraw appeal", {
        error: error.message,
        appealId,
      });
    ...
    logError("AppealsEngine", "Error withdrawing appeal", {
      error: error.message,
    });
    return { success: false, error: "Failed to withdraw appeal" };
  }
...
export async function getAppealsStats() {
  try {
    ...
    if (error) {
      logError("AppealsEngine", "Failed to get appeals stats", {
        error: error.message,
      });
    ...
    const pending =
      (byStatus[APPEAL_STATUSES.SUBMITTED] || 0) +
    ...
    logError("AppealsEngine", "Error getting appeals stats", {
      error: error.message,
    });
    return { success: false, error: "Failed to get appeals stats" };
  }

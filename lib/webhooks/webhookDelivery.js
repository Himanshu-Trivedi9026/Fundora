  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          success: true,
          data: { status: "delivered", statusCode: response.status },
        };
      }
      ...
      if (newAttemptCount >= delivery.max_attempts) {
        ...
        await supabaseAdmin
          ...
          .update({
            ...
            status:
              newFailureCount >= MAX_WEBHOOK_FAILURES
                ? "failed"
                : webhook.status,
          })
          .eq("id", webhook.id);
      ...
      const retryDelay =
        RETRY_DELAYS_MS[Math.min(newAttemptCount, RETRY_DELAYS_MS.length - 1)];
    ...
    logError("WebhookDelivery", "deliverWebhook unexpected error", {
      error: err.message,
    });
...
export async function retryDelivery(deliveryId) {
  try {
    ...
    if (error || !data) {
      return {
        success: false,
        error: "Delivery not found or not in failed state",
      };
    }
    ...
    logError("WebhookDelivery", "retryDelivery unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }
...
export async function getWebhookDeliveries(
  ...
  try {
    ...
    logError("WebhookDelivery", "getWebhookDeliveries unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }
...
export async function getPendingRetries() {
  try {
    ...
    logError("WebhookDelivery", "getPendingRetries unexpected error", {
      error: err.message,
    });
    return { success: false, error: "Internal error" };
  }

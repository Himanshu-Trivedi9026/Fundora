 ...
 */
export async function createWebhook({ userId, organizationId, url, events = [], description }) {
  try {
    if (!userId || !url) {
    ...
    if (error) {
      logError("Webhook", "createWebhook insert error", { error: error.message });
    ...
    logError("Webhook", "createWebhook unexpected error", { error: err.message });
...
export async function updateWebhook(webhookId, updates, userId) {
  try {
    if (!webhookId || !updates || !userId) {
      return { success: false, error: "webhookId, updates, and userId are required" };
    }
    ...
    if (!existing || existing.user_id !== userId) {
      return { success: false, error: "Webhook not found or not owned by user" };
    }

    // Validate events if provided
    if (updates.events) {
      const allEvents = Object.values(WEBHOOK_EVENTS);
      const invalidEvents = updates.events.filter((e) => !allEvents.includes(e));
      if (invalidEvents.length > 0) {
        return { success: false, error: `Invalid events: ${invalidEvents.join(", ")}` };
      }
    }
    ...
    logError("Webhook", "updateWebhook unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}
...
export async function deleteWebhook(webhookId, userId) {
  try {
    ...
    logError("Webhook", "deleteWebhook unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}
 ...
 */
export async function getWebhooks({ organizationId, userId, limit = 50, offset = 0 } = {}) {
  try {
...
export async function triggerWebhook({ organizationId, eventType, payload }) {
  try {
    ...
    if (queryErr) {
      logError("Webhook", "triggerWebhook query error", { error: queryErr.message });
    ...
    if (insertErr) {
      logError("Webhook", "triggerWebhook insert error", { error: insertErr.message });
    ...
    logError("Webhook", "triggerWebhook unexpected error", { error: err.message });
    return { success: false, error: "Internal error" };
  }
}

...
class LocalProvider extends BaseStorageProvider {
  ...
  async download(bucket, path) {
    return { success: true, data: { path: `${this.basePath}/${bucket}/${path}` } };
  }
...
class S3CompatibleProvider extends BaseStorageProvider {
  ...
  async getMetadata(bucket, path) {
    return { success: true, data: { bucket, path, provider: "s3", region: this.region } };
  }
...
class GCSProvider extends BaseStorageProvider {
  ...
  async upload(bucket, path, file, options = {}) {
    return { success: true, data: { url: `https://storage.googleapis.com/${bucket}/${path}`, path, bucket } };
  }

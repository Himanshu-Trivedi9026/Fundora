...
export async function createPayoutRequest({
  ...
  try {
    ...
    if (escrow.creator_id !== creatorId) {
      return {
        success: false,
        error: "Escrow account does not belong to this creator",
      };
    }
    ...
    if (!fraudResult.success) {
      ...
      return {
        success: false,
        error: "Payout request could not be evaluated at this time",
      };
    }
    ...
    if (fraudDecision === "block") {
      ...
      await logAuditEvent({
        ...
        details: {
          amount,
          fraudDecision,
          riskScore: fraudResult.result?.riskScore,
        },
      });

      return {
        success: false,
        error: "Payout request denied. Please contact support.",
      };
    }
    ...
    if (lockError) {
      logError("PayoutEngine", "Lock funds error", {
    ...
    if (insertError) {
      ...
      logError("PayoutEngine", "Create payout request error", {
...
export async function getPayoutRequest(payoutRequestId) {
  try {
    ...
    logError("PayoutEngine", "Get payout request error", {
      error: err.message,
    });
...
export async function getCreatorPayoutRequests(creatorId) {
  try {
    ...
    if (error) {
      logError("PayoutEngine", "Get creator payout requests error", {
        error: error.message,
      });
...
export async function getPendingPayouts({
  ...
  try {
    ...
    if (error) {
      logError("PayoutEngine", "Get pending payouts error", {
        error: error.message,
      });
...
export async function approvePayout(payoutRequestId, adminId) {
  try {
    if (!payoutRequestId || !adminId) {
      return {
        success: false,
        error: "payoutRequestId and adminId are required",
      };
    }
    ...
    if (existing.status !== PAYOUT_STATUSES.PENDING) {
      return {
        success: false,
        error: `Cannot approve payout in '${existing.status}' status`,
      };
    }
    ...
    if (error) {
      logError("PayoutEngine", "Approve payout error", {
        error: error.message,
      });
...
export async function rejectPayout(payoutRequestId, adminId, reason) {
  try {
    ...
    if (
      existing.status !== PAYOUT_STATUSES.PENDING &&
      existing.status !== PAYOUT_STATUSES.APPROVED
    ) {
      return {
        success: false,
        error: `Cannot reject payout in '${existing.status}' status`,
      };
    }
    ...
    const { error: unlockError } = await supabaseAdmin
      ...
      .update({
        available_balance:
          (await getEscrowBalance(existing.escrow_account_id)).available +
          existing.amount,
        locked_balance: Math.max(
          0,
          (await getEscrowBalance(existing.escrow_account_id)).locked -
            existing.amount,
        ),
        updated_at: new Date().toISOString(),
      })
    ...
    if (unlockError) {
      logError("PayoutEngine", "Unlock funds on rejection error", {
    ...
    await supabaseAdmin.from("escrow_ledger").insert({
      ...
      balance_after: (await getEscrowBalance(existing.escrow_account_id))
        .available,
...
export async function cancelPayout(payoutRequestId, userId) {
  try {
    if (!payoutRequestId || !userId) {
      return {
        success: false,
        error: "payoutRequestId and userId are required",
      };
    }
    ...
    if (existing.creator_id !== userId) {
      return {
        success: false,
        error: "You can only cancel your own payout requests",
      };
    }
    ...
    if (existing.status !== PAYOUT_STATUSES.PENDING) {
      return {
        success: false,
        error: `Cannot cancel payout in '${existing.status}' status. Only pending payouts can be cancelled.`,
      };
    }
    ...
    if (unlockError) {
      logError("PayoutEngine", "Unlock funds on cancel error", {
        error: unlockError.message,
      });
    }
...
export async function processPayout(payoutRequestId) {
  try {
    ...
    if (existing.status !== PAYOUT_STATUSES.APPROVED) {
      return {
        success: false,
        error: `Cannot process payout in '${existing.status}' status. Must be approved first.`,
      };
    }
    ...
    if (txError) {
      logError("PayoutEngine", "Create transaction error", {
    ...
    const providerResult = await simulateProviderProcessing(
      existing,
      transaction,
    );
    ...
    if (providerResult.success) {
      ...
      return {
        success: false,
        error: `Payout failed: ${providerResult.error}`,
      };
    }
...
export async function retryPayout(payoutRequestId) {
  try {
    ...
    if (existing.status !== PAYOUT_STATUSES.FAILED) {
      return {
        success: false,
        error: `Can only retry failed payouts. Current status: '${existing.status}'`,
      };
    }
    ...
    const fraudResult = await evaluateUser({
      ...
      context: {
        payoutRequestId,
        amount: existing.amount,
        attempt: (existing.retry_count || 0) + 1,
      },
    });
    ...
    if (!fraudResult.success) {
      return {
        success: false,
        error: "Retry evaluation failed. Please try again later.",
      };
    }
    ...
    if (fraudDecision === "block") {
      return {
        success: false,
        error: "Payout retry blocked by security check",
      };
    }
  ...
  try {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .in("status", [
        ...
        PAYOUT_STATUSES.CANCELLED,
      ])
      .order("created_at", { ascending: false })
      .range(offset, offset + limit - 1);

    if (error) {
      logError("PayoutEngine", "Get payout history error", {
        error: error.message,
      });
...
export async function getCreatorBalance(creatorId) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("escrow_accounts")
      .select(
        "id, campaign_id, available_balance, locked_balance, released_balance, status",
      )
      .eq("creator_id", creatorId);

    if (error) {
      logError("PayoutEngine", "Get creator balance error", {
        error: error.message,
      });

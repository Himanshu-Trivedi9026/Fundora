...
export async function calculatePlatformHealth() {
  try {
    ...
    const totalRaised = (donationSum || []).reduce(
    ...
    const fraudRate =
      totalUsers > 0 ? ((fraudAlerts || 0) / totalUsers) * 100 : 0;
    ...
    const avgTrustScore =
      (trustScores || []).length > 0
        ? trustScores.reduce((sum, t) => sum + (t.score || 0), 0) /
          trustScores.length
        : 0;
    ...
    logError("AnalyticsEngine", "Failed to calculate platform health", {
      error: error.message,
    });
...
export async function calculateTrustDistribution() {
  try {
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get trust scores", {
        error: error.message,
      });
      return {
        success: false,
        error: "Failed to calculate trust distribution",
      };
    }
    ...
    logError("AnalyticsEngine", "Error calculating trust distribution", {
      error: error.message,
    });
...
export async function getFraudTrends({
  ...
  try {
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get fraud trends", {
        error: error.message,
      });
    ...
    logError("AnalyticsEngine", "Error getting fraud trends", {
      error: error.message,
    });
    return { success: false, error: "Failed to get fraud trends" };
  }
...
export async function getEscrowStats() {
  try {
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get escrow stats", {
        error: error.message,
      });
    ...
    const totalDonated = accounts.reduce(
      (sum, a) => sum + (a.total_donated || 0),
      0,
    );

    const utilizationRate =
      totalDonated > 0 ? (totalBalance / totalDonated) * 100 : 0;
    ...
    logError("AnalyticsEngine", "Error getting escrow stats", {
      error: error.message,
    });
    return { success: false, error: "Failed to get escrow stats" };
  }
...
export async function getMilestoneCompletionStats() {
  try {
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get milestone stats", {
        error: error.message,
      });
    ...
    logError("AnalyticsEngine", "Error getting milestone stats", {
      error: error.message,
    });
    return { success: false, error: "Failed to get milestone stats" };
  }
...
export async function getPayoutSuccessStats() {
  try {
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get payout stats", {
        error: error.message,
      });
    ...
    logError("AnalyticsEngine", "Error getting payout stats", {
      error: error.message,
    });
    return { success: false, error: "Failed to get payout stats" };
  }
...
export async function getUserGrowthStats({
  ...
  try {
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get user growth", {
        error: error.message,
      });
    ...
    logError("AnalyticsEngine", "Error getting user growth stats", {
      error: error.message,
    });
    return { success: false, error: "Failed to get user growth stats" };
  }
...
export async function getCampaignPerformanceStats() {
  try {
    ...
    if (campaignError) {
      logError("AnalyticsEngine", "Failed to get campaign stats", {
        error: campaignError.message,
      });
      return {
        success: false,
        error: "Failed to get campaign performance stats",
      };
    }
    ...
    const completed = campaignsList.filter(
      (c) => c.status === "completed",
    ).length;
    const failed = campaignsList.filter((c) => c.status === "failed").length;

    const totalGoalAmount = campaignsList.reduce(
      (sum, c) => sum + (c.goal_amount || 0),
      0,
    );
    const totalRaisedAmount = campaignsList.reduce(
      (sum, c) => sum + (c.raised_amount || 0),
      0,
    );
    const fundingRate =
      totalGoalAmount > 0 ? (totalRaisedAmount / totalGoalAmount) * 100 : 0;
    const successRate = total > 0 ? (completed / total) * 100 : 0;
    ...
    logError("AnalyticsEngine", "Error getting campaign performance stats", {
      error: error.message,
    });
    return {
      success: false,
...
export async function getVerificationStats() {
  try {
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get verification stats", {
        error: error.message,
      });
    ...
    const completed = verificationsList.filter(
      (v) => v.status === "verified" || v.status === "approved",
    ).length;
    const pending = verificationsList.filter(
      (v) => v.status === "pending" || v.status === "in_progress",
    ).length;
    const failed = verificationsList.filter(
    ...
    logError("AnalyticsEngine", "Error getting verification stats", {
      error: error.message,
    });
    return { success: false, error: "Failed to get verification stats" };
  }
...
export async function getEngagementMetrics() {
  try {
    ...
    logError("AnalyticsEngine", "Error getting engagement metrics", {
      error: error.message,
    });
...
export async function getModerationStats() {
  try {
    ...
    if (reportsError) {
      logError("AnalyticsEngine", "Failed to get moderation stats", {
    ...
    const dismissed = reportsList.filter(
    ...
    const resolutionRate =
      total > 0 ? ((resolved + dismissed) / total) * 100 : 0;
    ...
    logError("AnalyticsEngine", "Error getting moderation stats", {
      error: error.message,
    });
    return { success: false, error: "Failed to get moderation stats" };
  }
...
export async function storeMetric({
  ...
  try {
    if (!metricType || !metricDate || !metricData) {
      return {
        success: false,
        error: "metricType, metricDate, and metricData are required",
      };
    }
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to store metric", {
        error: error.message,
        metricType,
      });
    ...
    logInfo("AnalyticsEngine", "Metric stored", {
      metricType,
      metricDate,
      aggregationPeriod,
    });
    ...
    logError("AnalyticsEngine", "Error storing metric", {
      error: error.message,
    });
    return { success: false, error: "Failed to store metric" };
  }
...
export async function getStoredMetrics({
  ...
  try {
    ...
    if (aggregationPeriod)
      query = query.eq("aggregation_period", aggregationPeriod);
    ...
    if (error) {
      logError("AnalyticsEngine", "Failed to get stored metrics", {
        error: error.message,
        metricType,
      });
    ...
    logError("AnalyticsEngine", "Error getting stored metrics", {
      error: error.message,
    });
    return { success: false, error: "Failed to get stored metrics" };
  }

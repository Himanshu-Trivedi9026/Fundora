...
export async function initiateRestore(options = {}) {
  try {
    ...
    validateAndExecute(data.id, options).catch((err) => {
      logError("Restore execution failed", { restoreId: data.id, error: err.message });
    });
...
export async function listRestoreOperations(options = {}) {
  try {
    let query = supabaseAdmin.from("restore_operations").select("*", { count: "exact" });
...
export async function rollbackRestore(restoreId) {
  try {
    const op = await getRestoreOperation(restoreId);
    if (!op.success) return { success: false, error: "Restore operation not found" };
    ...
    await supabaseAdmin
      ...
      .update({
        status: "rolled_back",
        metadata: { ...op.data.metadata, rolledBackAt: new Date().toISOString() },
        updated_at: new Date().toISOString(),
      })
      .eq("id", restoreId);

...
import {
  ...
  GoogleWorkspaceConnector, GitHubConnector, JiraConnector, NotionConnector,
...
export async function connectConnector(connectorId) {
  try {
    ...
    if (error || !config) return { success: false, error: "Connector config not found" };

    const ConnectorClass = CONNECTOR_CLASSES[config.provider];
    if (!ConnectorClass) return { success: false, error: `Unknown provider: ${config.provider}` };
...
export async function getConnectorStatus(connectorId) {
  try {
    const { data, error } = await supabaseAdmin
      .from("connector_configs")
      .select("id, provider, label, status, is_active, last_connected_at, last_error, metadata")
      .eq("id", connectorId)
      .single();
...
export async function listConnectors(options = {}) {
  try {
    let query = supabaseAdmin.from("connector_configs").select("*", { count: "exact" });

...
export default withRole(async function handler(req, res, user) {
  ...
  if (req.method === "GET") {
    ...
    try {
      const { mode, status, caseType, priority, limit, offset, caseId } = req.query;
      ...
      if (mode === "events") {
        if (!caseId) {
          return res.status(400).json({ error: "caseId is required for events mode" });
        }
      ...
      return res.status(500).json({ error: "Failed to fetch compliance dashboard" });
    }
  }
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (action === "update_case") {
        if (!caseId || !updates) {
          return res.status(400).json({ error: "caseId and updates are required" });
        }
      ...
      if (action === "assign_case") {
        if (!caseId || !assignTo) {
          return res.status(400).json({ error: "caseId and assignTo are required" });
        }
      ...
      if (action === "resolve_case") {
        if (!caseId || !resolutionType || !resolution) {
          return res.status(400).json({ error: "caseId, resolutionType, and resolution are required" });
        }

        const result = await resolveComplianceCase(caseId, resolutionType, resolution, user.id);
      ...
      if (action === "escalate_case") {
        if (!caseId || !reason) {
          return res.status(400).json({ error: "caseId and reason are required" });
        }
      ...
      logError("ComplianceDashboardAPI", "POST error", { error: err.message });

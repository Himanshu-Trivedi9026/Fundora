...
import { approveBusinessVerification, rejectBusinessVerification, requestResubmission } from "../../../lib/verification/manualReview";
...
export default withRole(async function handler(req, res, user) {
  ...
  try {
    ...
    switch (action) {
      case "approve":
        result = await approveBusinessVerification(verificationId, user.id, notes, user.id);
        break;
      case "reject":
        if (!reason) {
          return res.status(400).json({ error: "Rejection reason is required" });
        }
        result = await rejectBusinessVerification(verificationId, user.id, reason, user.id);
        break;
      case "resubmit":
        if (!reason) {

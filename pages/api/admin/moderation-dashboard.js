  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      if (mode === "overview" || !mode) {
        ...
        (allCases || []).forEach((c) => {
          if (c.action_taken) {
            actionDistribution[c.action_taken] = (actionDistribution[c.action_taken] || 0) + 1;
          }
        });
      ...
      return res.status(500).json({ error: "Failed to fetch moderation dashboard" });
    }
  }
    ...
    try {
      ...
      if (action === "assign_case") {
        if (!caseId || !moderatorId) {
          return res.status(400).json({ error: "caseId and moderatorId are required" });
        }

        const result = await assignModerationCase(caseId, moderatorId, user.id);
      ...
      if (action === "resolve_case") {
        if (!caseId || !actionTaken || !resolution) {
          return res.status(400).json({ error: "caseId, actionTaken, and resolution are required" });
        }
      ...
      logError("ModerationDashboardAPI", "POST error", { error: err.message });

...
import { getFraudDashboard, applyManualOverride, getAllFraudEvents, getAggregateStats } from "../../../lib/fraud";
...
export default withRole(async function handler(req, res, user) {
  ...
  if (req.method === "GET") {
    ...
    try {
      const { mode, riskLevel, decision, limit, offset, userId, days, category, severity } = req.query;

      // Stats mode
      if (mode === "stats") {
        const result = await getAggregateStats({ days: parseInt(days, 10) || 30 });
        if (!result.success) {
      ...
      return res.status(500).json({ error: "Failed to fetch fraud dashboard" });
    }
  }
  ...
  if (req.method === "POST") {
    ...
    try {
      const { action, userId: targetUserId, overrideType, newValue, reason, isPermanent, expiresAt } = req.body;
      ...
      if (action === "getProfile") {
        ...
        const { data: events } = await supabaseAdmin
          .from("fraud_events")
          .select("id, event_type, event_category, severity, signal_name, risk_contribution, created_at")
          .eq("user_id", targetUserId)
        ...
        const { data: devices } = await supabaseAdmin
          ...
          .select("id, browser, platform, is_known, session_count, risk_flags, last_seen_at")
          .eq("user_id", targetUserId)
        ...
        const { data: overrides } = await supabaseAdmin
          ...
          .select("id, override_type, new_value, reason, is_permanent, created_at, revoked_at")
          .eq("user_id", targetUserId)
          .order("created_at", { ascending: false })
          .limit(10);

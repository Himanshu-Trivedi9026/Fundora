  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      if (mode === "overview" || !mode) {
        ...
        const pendingAppeals = await getAppeals({ status: "submitted", limit: parseInt(limit, 10) || 20, offset: 0 });

        return res.status(200).json({
          success: true,
          stats: stats.success ? stats.data : { total: 0, pending: 0 },
          pendingAppeals: pendingAppeals.success ? pendingAppeals.data.appeals : [],
        });
      }
      ...
      return res.status(500).json({ error: "Failed to fetch appeals dashboard" });
    }
  }
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (action === "assign_reviewer") {
        ...
        if (!appealId || !reviewerId) return res.status(400).json({ error: "appealId and reviewerId are required" });
        const result = await assignAppealReviewer(appealId, reviewerId, user.id);
        if (!result.success) return res.status(400).json({ error: result.error });
        return res.status(200).json({ success: true, data: result.data });
      }

      if (action === "review_appeal") {
        const { appealId, reviewerDecision, decisionReason, reviewerNotes } = params;
        if (!appealId || !reviewerDecision) return res.status(400).json({ error: "appealId and reviewerDecision are required" });
        const result = await reviewAppeal(appealId, reviewerDecision, decisionReason, reviewerNotes, user.id);
        if (!result.success) return res.status(400).json({ error: result.error });
        return res.status(200).json({ success: true, data: result.data });
      }
      ...
      if (action === "request_evidence") {
        ...
        if (!appealId || !reason) return res.status(400).json({ error: "appealId and reason are required" });

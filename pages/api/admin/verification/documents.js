...
import { getSignedUrl, sanitizeDoc } from "../../../../lib/verification/storage";
...
export default withRole(async function handler(req, res, user) {
  ...
  try {
    ...
    if (req.query.documentId) {
      ...
      query = query.in("status", ["pending", "uploaded", "verified", "rejected"]);
    ...
    return res.status(200).json({ success: true, documents: docs, count: docs.length });
  } catch (err) {

...
import { processSyncBatch, getChangesSince } from "../../../lib/mobile/offlineSync.js";
  ...
  try {
    switch (req.method) {
      case "GET": {
        ...
        if (!since) return res.status(400).json({ success: false, error: "since timestamp required" });
      ...
      case "POST": {
        ...
        const result = await processSyncBatch(operations, { conflictStrategy, userId: user?.id });
        return res.status(200).json(result);
      }

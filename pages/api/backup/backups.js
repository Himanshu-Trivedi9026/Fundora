...
import { createBackup, listBackups, deleteBackup, getBackupStats } from "../../../lib/backup/backupEngine.js";
  ...
  try {
    switch (req.method) {
      case "GET": {
        ...
        const result = await listBackups({ status, organizationId: req.user?.organizationId });
        return res.status(200).json(result);
      }

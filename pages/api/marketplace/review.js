...
async function handler(req, res) {
  ...
  try {
    ...
    if (!pluginId || !rating) {
      return res.status(400).json({ success: false, error: "Missing required fields: pluginId, rating" });
    }

    const result = await submitPluginReview({ pluginId, userId, rating, review });
    if (!result.success) {

...
import { useVerification, VERIFICATION_LEVELS } from "../../context/VerificationContext";
...
export default function CreatorVerification() {
  const {
    verification, history, loading: contextLoading, expiryStatus, daysUntilExpiry,
    businessVerification, businessDocuments, bankAccounts, bankVerification,
  ...
  if (loading) {
    return (
      <PageLayout>
        <main className="flex-1 flex items-center justify-center min-h-[60vh]" role="status" aria-label="Loading verification">
          <span className="material-symbols-outlined animate-spin text-primary text-4xl" aria-hidden="true">
            progress_activity
          </span>
        </main>
      </PageLayout>
  ...
  const currentLevelInfo = VERIFICATION_LEVELS.find(l => l.level === level) || VERIFICATION_LEVELS[0];
            ...
            <motion.div variants={fadeUp} className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-on-surface-variant font-inter text-sm max-w-md mx-auto">
                  Verify your identity to build trust with backers and unlock higher platform capabilities.
                </p>
              </div>
              ...
              {expiryStatus && expiryStatus !== "not_verified" && (
                <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-inter font-medium ${
                  expiryStatus === "valid"
                  ...
                  <span className={`material-symbols-outlined text-[14px] ${
                    expiryStatus === "valid" ? "text-success" : expiryStatus === "expiring_soon" ? "text-warning" : "text-danger"
                  }`} style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">
                    {expiryStatus === "valid" ? "verified" : expiryStatus === "expiring_soon" ? "warning" : "error"}
                  </span>
                  {expiryStatus === "valid" && "Verification Valid"}
                  {expiryStatus === "expiring_soon" && `Expires in ${daysUntilExpiry} days`}
                  {expiryStatus === "expired" && "Verification Expired — Renew Required"}
                </div>
              )}

              {/* Appeal Button — shown when verification is rejected */}
              {(status === "rejected" || businessVerification?.status === "rejected" || bankVerification?.status === "rejected") && (
                <div className="mt-4">
                  <button
                    onClick={() => { setAppealModal(true); setAppealError(null); setAppealSuccess(false); setAppealReason(""); }}
                ...
                <div className="glass-panel p-6 rounded-2xl border border-primary/20 bg-primary/[0.04] space-y-4">
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0">
                      <span className="material-symbols-outlined text-primary text-[24px]" aria-hidden="true">verified_user</span>
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-on-surface-variant font-inter mt-1">
                        ...
                        identity documents (PAN, Aadhaar, selfie). This unlocks
                        publishing and receiving funds on Fundora.
                      </p>
                    </div>
                  </div>
              ...
              <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-6">
                <div className="flex items-center justify-between border-b border-outline-variant/30 pb-3">
                  <h2 className="font-geist text-lg font-semibold flex items-center gap-2">
                    <span className="material-symbols-outlined text-primary text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">
                      trending_up
                    </span>
                ...
                <div className="flex items-center gap-4">
                  <div className="relative w-16 h-16">
                    <svg className="w-16 h-16 -rotate-90" viewBox="0 0 36 36" role="img" aria-label={`${completionPercentage}% verification completion`}>
                      <path
                  ...
                  <div>
                    <p className="text-sm font-semibold text-on-surface">Overall Completion</p>
                    <p className="text-xs text-on-surface-variant font-inter">
                      {completionPercentage === 100 ? "All verifications complete" : `${6 - Math.round(completionPercentage / 100 * 6)} steps remaining`}
                    </p>
                  </div>
                </div>
                ...
                {nextLevelInfo && (
                  <div className="p-4 bg-primary/5 border border-primary/10 rounded-xl">
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-primary text-[16px]" aria-hidden="true">arrow_upward</span>
                      <span className="text-sm font-inter text-on-surface">
                        Next: <span className="font-semibold text-primary">{nextLevelInfo.label}</span> — {nextLevelInfo.description}
                      </span>
                    </div>
                  </div>
                )}
              ...
              <motion.div variants={fadeUp}>
                <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-4">
                  <div className="flex items-center gap-2 border-b border-outline-variant/30 pb-3">
                    <span className="material-symbols-outlined text-warning text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">
                      pending_actions
                    </span>
                    <h2 className="font-geist text-lg font-semibold">Pending Actions</h2>
                  ...
                  <div className="space-y-2">
                    {pendingActions.map((action, i) => (
                      ...
                      >
                        <span className="material-symbols-outlined text-warning text-[16px]" aria-hidden="true">{action.icon}</span>
                        <span className="text-sm font-inter text-on-surface">{action.label}</span>
                        <span className="material-symbols-outlined text-on-surface-variant text-[16px] ml-auto" aria-hidden="true">chevron_right</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <motion.div variants={fadeUp}>
                <div className="glass-panel p-6 rounded-2xl border border-danger/10 space-y-4">
                  <div className="flex items-center gap-2 border-b border-danger/20 pb-3">
                    <span className="material-symbols-outlined text-danger text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">
                      error
                    </span>
                    <h2 className="font-geist text-lg font-semibold text-danger">Documents Need Attention</h2>
                  </div>
                  <div className="space-y-2">
                    {rejectedDocuments.map((doc) => (
                      ...
                      >
                        <span className="material-symbols-outlined text-danger text-[16px]" aria-hidden="true">upload_file</span>
                        <span className="text-sm font-inter text-on-surface">{doc.document_type?.replace(/_/g, " ")}</span>
            ...
            <motion.div variants={fadeUp}>
              <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-4">
                <div className="flex items-center gap-2 border-b border-outline-variant/30 pb-3">
                  ...
                  <h2 className="font-geist text-lg font-semibold">Business Verification</h2>
                  {businessVerification?.status === "verified" && (
                    <span className="ml-auto px-2 py-0.5 rounded-full bg-success/10 text-success text-xs font-bold">Verified</span>
                  )}
                  {businessVerification?.status === "pending" && (
                    <span className="ml-auto px-2 py-0.5 rounded-full bg-warning/10 text-warning text-xs font-bold">Pending</span>
                  )}
                  {businessVerification?.status === "rejected" && (
                    <span className="ml-auto px-2 py-0.5 rounded-full bg-danger/10 text-danger text-xs font-bold">Rejected</span>
                  )}
                </div>
                {businessVerification ? (
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3 text-sm font-inter">
                      <div>
                        ...
                        <p className="text-on-surface font-medium">{businessVerification.business_name || "—"}</p>
                      </div>
                      <div>
                        <span className="text-on-surface-variant">Type</span>
                        <p className="text-on-surface font-medium capitalize">{businessVerification.business_type?.replace(/_/g, " ") || "—"}</p>
                      </div>
                      <div>
                        <span className="text-on-surface-variant">GST Status</span>
                        <p className={`font-medium ${businessVerification.gst_status === "verified" ? "text-success" : "text-on-surface-variant"}`}>
                          {businessVerification.gst_status || "Not submitted"}
                        </p>
                      </div>
                      <div>
                        <span className="text-on-surface-variant">PAN Status</span>
                        <p className={`font-medium ${businessVerification.pan_status === "verified" ? "text-success" : "text-on-surface-variant"}`}>
                    ...
                    {businessDocuments.length > 0 && (
                      <div className="pt-2 border-t border-outline-variant/30">
                        <p className="text-xs text-on-surface-variant mb-2">Documents ({businessDocuments.length})</p>
                  ...
                  <p className="text-sm text-on-surface-variant font-inter">No business verification started yet.</p>
                )}
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp}>
              <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-4">
                <div className="flex items-center gap-2 border-b border-outline-variant/30 pb-3">
                  ...
                  <h2 className="font-geist text-lg font-semibold">Bank Verification</h2>
                ...
                {bankAccounts.length > 0 ? (
                    ...
                    {bankAccounts.map((account) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-medium text-on-surface">{account.bank_name || "Bank Account"}</span>
                          <div className="flex items-center gap-2">
                            {account.is_primary && (
                              <span className="px-1.5 py-0.5 rounded bg-primary/10 text-primary text-[10px] font-bold">Primary</span>
                            )}
                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                              account.status === "verified" ? "bg-success/10 text-success" :
                              account.status === "pending" ? "bg-warning/10 text-warning" :
                              account.status === "rejected" ? "bg-danger/10 text-danger" :
                              "bg-surface-container-high/50 text-on-surface-variant"
                            }`}>
                              {account.status}
                            </span>
                        ...
                        <p className="text-xs text-on-surface-variant font-inter">
                          {account.account_type || "Savings"} • {account.account_holder_name || "—"}
                        </p>
                        {account.penny_drop_status && (
                          <p className="text-xs text-on-surface-variant font-inter mt-1">
                            Penny Drop: <span className={account.penny_drop_status === "success" ? "text-success" : "text-on-surface-variant"}>
                              {account.penny_drop_status}
                            </span>
                          </p>
                        )}
                  ...
                  <p className="text-sm text-on-surface-variant font-inter">No bank accounts added yet.</p>
                )}
              </div>
            </motion.div>
              ...
              <motion.div variants={fadeUp}>
                <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-4">
                  <div className="flex items-center gap-2 border-b border-outline-variant/30 pb-3">
                    ...
                    <h2 className="font-geist text-lg font-semibold">Verification Timeline</h2>
                  </div>
                  <div className="space-y-3">
                    {verificationTimeline.slice(0, 10).map((event) => (
                      <div key={event.id} className="flex items-start gap-3">
                        <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                          event.status === "approved" || event.status === "verified" ? "bg-success" :
                          event.status === "rejected" ? "bg-danger" :
                          event.status === "pending" || event.status === "under_review" ? "bg-warning" :
                          "bg-primary"
                        }`} aria-hidden="true" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-inter text-on-surface">{event.action?.replace(/_/g, " ")}</p>
                          <p className="text-xs text-on-surface-variant font-inter">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold flex-shrink-0 ${
                          event.status === "approved" || event.status === "verified" ? "bg-success/10 text-success" :
                          event.status === "rejected" ? "bg-danger/10 text-danger" :
                          "bg-surface-container-high/50 text-on-surface-variant"
                        }`}>
                          {event.status}
                        </span>
            ...
            <motion.div variants={fadeUp}>
              <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-4">
                <div className="flex items-center gap-2 border-b border-outline-variant/30 pb-3">
                  ...
                  <h2 className="font-geist text-lg font-semibold">How We Protect Your Data</h2>
                </div>
                <div className="space-y-3 text-sm text-on-surface-variant font-inter">
                  <div className="flex items-start gap-3">
                    <span className="material-symbols-outlined text-success text-[16px] mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">check_circle</span>
                    <span>Your identity documents are encrypted and stored securely. We never share them with third parties.</span>
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="material-symbols-outlined text-success text-[16px] mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">check_circle</span>
                    <span>Verification data is only visible to you and our review team. It is never shown on your public profile.</span>
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="material-symbols-outlined text-success text-[16px] mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">check_circle</span>
                    <span>You can request data deletion at any time from your account settings.</span>
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="material-symbols-outlined text-success text-[16px] mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }} aria-hidden="true">check_circle</span>
                    <span>Our verification partners (when enabled) are SOC 2 Type II certified.</span>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp}>
              <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-4">
                <div className="flex items-center gap-2 border-b border-outline-variant/30 pb-3">
                  ...
                  <h2 className="font-geist text-lg font-semibold">Verification Requirements</h2>
                </div>
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`material-symbols-outlined text-[16px] ${
                            level >= lvl.level ? "text-success" : "text-on-surface-variant/50"
                          }`}
                          style={level >= lvl.level ? { fontVariationSettings: "'FILL' 1" } : {}}
                          aria-hidden="true"
                        >
                          {level >= lvl.level ? "check_circle" : lvl.icon}
                        </span>
                        <span className={`text-sm font-semibold ${
                          level >= lvl.level ? "text-success" : "text-on-surface"
                        }`}>
                          {lvl.label}
                        </span>
                      </div>
                      <p className="text-xs text-on-surface-variant font-inter">{lvl.description}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="bg-[#0d0d15] border border-white/[0.06] rounded-2xl p-6 max-w-lg w-full mx-4 shadow-xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => { setAppealModal(false); setAppealError(null); setAppealSuccess(false); }}
              ...
              {appealSuccess ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-green-300 text-sm mb-4">Your appeal has been submitted successfully.</p>
                  <button
                    onClick={() => { setAppealModal(false); setAppealSuccess(false); }}
                ...
                <form onSubmit={async (e) => {
                  ...
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">Reason for Appeal *</label>
                    <textarea
                      value={appealReason}
                  ...
                  <div className="flex gap-2 justify-end pt-2">
                    <button
                      type="button"
                      onClick={() => { setAppealModal(false); setAppealError(null); }}
